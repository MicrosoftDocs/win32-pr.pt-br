...
---
# <a name="determining-the-registry-size"></a><span data-ttu-id="91120-103">Determinando o tamanho do registro</span><span class="sxs-lookup"><span data-stu-id="91120-103">Determining the Registry Size</span></span>

<span data-ttu-id="91120-104">No Windows 2000, é comum um utilitário de instalação verificar o tamanho atual e o máximo do registro para determinar se há espaço disponível suficiente para os novos dados que serão adicionados.</span><span class="sxs-lookup"><span data-stu-id="91120-104">On Windows 2000, it is common for an installation utility to check the current and maximum size of the registry to determine whether there is enough available space for the new data it will be adding.</span></span> <span data-ttu-id="91120-105">Este exemplo demonstra como fazer isso programaticamente usando o contador de desempenho "% de cota de registro em uso" dentro do objeto do sistema.</span><span class="sxs-lookup"><span data-stu-id="91120-105">This sample demonstrates how to do this programmatically using the "% Registry Quota In Use" performance counter within the System object.</span></span>

<span data-ttu-id="91120-106">O exemplo a seguir usa o PDH (auxiliar de dados de desempenho) para obter o valor do contador; Ele deve ser vinculado a PDH. lib.</span><span class="sxs-lookup"><span data-stu-id="91120-106">The following sample uses performance data helper (PDH) to obtain the counter value; it must be linked with Pdh.lib.</span></span> <span data-ttu-id="91120-107">A PDH é um conjunto de APIs de alto nível usado para obter dados de desempenho.</span><span class="sxs-lookup"><span data-stu-id="91120-107">PDH is a high-level set of APIs used to obtain performance data.</span></span>

> [!Note]  
> <span data-ttu-id="91120-108">Não é necessário implementar esse tamanho de registro-Verifique no Windows Server 2003 ou no Windows XP porque eles não têm um limite de cota de registro.</span><span class="sxs-lookup"><span data-stu-id="91120-108">It is not necessary to implement this registry size-check on Windows Server 2003 or Windows XP because they do not have a registry quota limit.</span></span>
