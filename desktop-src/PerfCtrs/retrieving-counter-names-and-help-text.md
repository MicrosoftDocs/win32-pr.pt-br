...
ms.locfileid: "120033326"
---
# <a name="retrieving-counter-names-and-help-text"></a>Recuperando nomes de contadores e texto de ajuda

Os dados de desempenho contêm valores de índice que você usa para localizar os nomes e o texto de ajuda para cada objeto registrado e contador. Os membros **ObjectNameTitleIndex** e **ObjectHelpTitleIndex** da estrutura [**de \_ \_ tipo de objeto perf**](/windows/desktop/api/Winperf/ns-winperf-perf_object_type) contêm os valores de índice para o nome do objeto e o texto da ajuda, respectivamente, e os membros **CounterNameTitleIndex** e **CounterHelpTitleIndex** da estrutura de [**\_ \_ definição do contador de desempenho**](/windows/desktop/api/Winperf/ns-winperf-perf_counter_definition) contêm os valores de índice para o nome do contador e o texto de ajuda, respectivamente.

Para recuperar os nomes ou o texto da ajuda, chame a função [**RegQueryValueEx**](/windows/desktop/api/winreg/nf-winreg-regqueryvalueexa) . Defina o parâmetro *HKEY* para uma das seguintes chaves predefinidas. Normalmente, você usaria o **HKEY \_ performance \_ NLSTEXT** Key, portanto, não é necessário determinar o identificador de idioma do usuário.



| Chave                            | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **\_dados de desempenho de hKey \_**    | Cadeias de caracteres de consulta com base no valor do identificador de idioma que você especificar no parâmetro *lpValueName* . Defina o parâmetro *lpValueName* como "Counter &lt; langname &gt; " ou "Help &lt; LangID &gt; " para recuperar os nomes ou o texto da ajuda, respectivamente, em que " &lt; LangID &gt; " é o identificador de idioma do sistema formatado como um **número hexadecimal de 3 dígitos preenchido com zero**. O identificador de idioma é opcional. Se você não especificar um identificador de idioma, a função retornará cadeias de caracteres em inglês. Verifique a `HKEY_LOCAL_MACHINE\Software\Microsoft\Windows NT\CurrentVersion\Perflib` chave do registro para obter a lista de idiomas disponíveis no sistema.<br/>Para recuperar o texto na maioria dos idiomas, especifique apenas o identificador de idioma primário. Por exemplo, para recuperar cadeias de caracteres em inglês, especifique o identificador de idioma como 009, e não 1033 para inglês-US.<br/>Para recuperar texto em chinês e Português, especifique os identificadores primário e de subidioma.<br/>**Windows Server 2003 e Windows XP:** Especifique somente o identificador de idioma primário para Português.<br/>**Windows 10**: o texto "Help &lt; langid &gt; " com o identificador de idioma personalizado sempre retorna cadeias de caracteres em inglês, embora o valor localizado ainda possa ser recuperado do registro usando a chave mencionada anteriormente.<br/><br/> |
| **\_NLSTEXT de desempenho de hKey \_** | Cadeias de caracteres de consulta com base no idioma padrão da interface do usuário atual. Defina o parâmetro *lpValueName* como "Counter" ou "Help" para recuperar os nomes ou o texto da ajuda, respectivamente.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **\_texto de desempenho de hKey \_**    | Consultar cadeias de caracteres em inglês. Defina o parâmetro *lpValueName* como "Counter" ou "Help" para recuperar os nomes ou o texto da ajuda, respectivamente.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |





A função retorna os dados como uma lista de cadeias de caracteres. Cada cadeia de caracteres é terminada em nulo. A última cadeia de caracteres é seguida por um terminador nulo adicional. As cadeias de caracteres são listadas em pares. A primeira cadeia de cada par é o índice, e a segunda cadeia de caracteres é o texto associado ao índice. Os dados do contador usam apenas índices de número par, enquanto os dados de ajuda usam índices ímpares. Os pares são retornados em ordem de índice crescente.

As listas a seguir mostram exemplos de dados de contador e de ajuda. Incrementar um determinado valor de índice do contador por um fornece o índice para o texto de ajuda do contador. Por exemplo, 7 é o índice de ajuda associado ao índice de contador 6.

Pares de dados do contador.

<dl> 2 sistema 4 memória 6% tempo do processador
</dl>

Os pares de dados de ajuda.

<dl> 3 o tipo de objeto do sistema inclui os contadores que... 5 o tipo de objeto de memória inclui os contadores que... 7 o tempo do processador é expresso como uma porcentagem do...
</dl>

Observe que o primeiro par de cadeias de caracteres nos dados do contador não identifica um nome de contador e pode ser ignorado. O número de índice do primeiro par é 1 e a cadeia de caracteres é uma cadeia de caracteres numérica que representa o valor de índice máximo para os contadores do sistema.

Para obter informações sobre como um provedor carrega o nome e o texto de ajuda, consulte [adicionando nomes e descrições de contadores ao registro](adding-counter-names-and-descriptions-to-the-registry.md).

Não há informações no texto que indica se o texto identifica um contador ou objeto de desempenho. A única maneira de determinar isso, ou a relação entre contadores e objetos, é consultar os dados de desempenho em si. Por exemplo, se você quiser exibir uma lista de objetos e seus contadores em uma interface do usuário, deverá recuperar os dados de desempenho e, em seguida, usar os valores de índice para analisar os dados de texto das cadeias de caracteres. Para obter um exemplo que faz isso, consulte [exibindo nomes de objeto, instância e contador](displaying-object-instance-and-counter-names.md).

O exemplo a seguir mostra como usar **HKEY \_ performance \_ NLSTEXT** para recuperar o contador e o texto de ajuda e criar uma tabela para o acesso subsequente.
...
O exemplo a seguir mostra como usar **os \_ \_ dados de desempenho de hKey** para recuperar o texto do contador.
