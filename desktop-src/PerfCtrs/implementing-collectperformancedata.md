...
---
# <a name="implementing-collectperformancedata"></a>Implementando CollectPerformanceData

Depois que o sistema chama com êxito que você é a função [**OpenPerformanceData**](/previous-versions/windows/desktop/legacy/aa372200(v=vs.85)) , ele chama a função [**CollectPerformanceData**](/windows/win32/api/winperf/nc-winperf-pm_collect_proc) para coletar os dados do contador. Se o provedor oferecer suporte aos objetos consultados, ele entrará em contato com o serviço, o driver ou o aplicativo com o qual ele está associado e o solicitará pelos dados do contador.

O exemplo a seguir mostra uma implementação da função [*CollectPerformanceData*](/windows/win32/api/winperf/nc-winperf-pm_collect_proc) . O arquivo de cabeçalho que contém a definição dos contadores usados nessa função é mostrado em [implementando OpenPerformanceData](implementing-openperformancedata.md). Se você usar C++ para implementar essa função, certifique-se de usar o "C" externo ao declarar sua função.
