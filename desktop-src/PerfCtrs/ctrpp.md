...
|**-ch** *symFile*   |Especifica o nome do arquivo de símbolo opcional `.h` a ser gerado pelo CTRPP. Esse arquivo conterá símbolos C/C++ para os nomes e GUIDs de cada contador no provedor.
...
- O desenvolvedor edita o arquivo de manifesto do provedor ( ) para definir o atributo do provedor com o nome de um binário do provedor (.DLL, .SYS ou .EXE) que conterá os recursos de cadeia de caracteres para o provedor e será instalado como parte do componente do `.man` `applicationIdentity` provedor.
...
- Uma função de limpeza do counterset chamada <b> *prefix* Unregister *Counterset*</b>. Essa função invoca [PcwUnregister na](/windows-hardware/drivers/ddi/wdm/nf-wdm-pcwunregister) alça de registro do counterset na variável global ***Counterset.***
...
> [!NOTE]
> A função <b>Add *Counterset do* *prefixo*</b> gerado é usada quando você tem um retorno de chamada de counterset. As funções Create Counterset e Close <b>*Counterset*</b> do <b> *prefixo*</b> gerados são usadas quando você não tem um retorno de chamada de counterset.

### <a name="using-the-generated-symbol-file"></a>Usando o arquivo de símbolo gerado

Se o **parâmetro -ch** for especificado na linha de comando, a ferramenta CTRPP gerará um arquivo `.h` de símbolo. Esse arquivo contém os símbolos C/C++ para os nomes e GUIDs de cada contador no provedor. Os símbolos podem ser usados ao escrever programas que são codificados para consumir os dados desse contador usando as funções de Consumidor [PerfLib V2](using-the-perflib-functions-to-consume-counter-data.md).
