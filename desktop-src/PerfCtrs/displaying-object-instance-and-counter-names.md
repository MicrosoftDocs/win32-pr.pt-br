---
description: Se desejar exibir uma lista de objetos e seus contadores em uma interface do usuário, você deverá recuperar os dados de desempenho.
ms.assetid: 0d122fa8-2ed8-4bd8-a52e-6cb20fe81741
title: Exibindo nomes de objeto, instância e contador
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103917767"
---
# <a name="displaying-object-instance-and-counter-names"></a><span data-ttu-id="048f3-103">Exibindo nomes de objeto, instância e contador</span><span class="sxs-lookup"><span data-stu-id="048f3-103">Displaying Object, Instance, and Counter Names</span></span>

<span data-ttu-id="048f3-104">Se desejar exibir uma lista de objetos e seus contadores em uma interface do usuário, você deverá recuperar os dados de desempenho.</span><span class="sxs-lookup"><span data-stu-id="048f3-104">If you want to display a list of objects and their counters in a user interface, you must retrieve the performance data.</span></span> <span data-ttu-id="048f3-105">Os dados de desempenho contêm um número variável de objetos de desempenho e suas instâncias e contadores.</span><span class="sxs-lookup"><span data-stu-id="048f3-105">The performance data contains a variable number of performance objects and their instances and counters.</span></span> <span data-ttu-id="048f3-106">Para obter informações sobre o formato dos dados de desempenho, consulte [formato de dados de desempenho](performance-data-format.md).</span><span class="sxs-lookup"><span data-stu-id="048f3-106">For information on the format of the performance data, see [Performance Data Format](performance-data-format.md).</span></span>

<span data-ttu-id="048f3-107">Os dados de desempenho não contêm os nomes de objeto e contador.</span><span class="sxs-lookup"><span data-stu-id="048f3-107">The performance data does not contain the object and counter names.</span></span> <span data-ttu-id="048f3-108">Em vez disso, os dados contêm valores de índice que você usa para recuperar os nomes dos objetos e contadores, que são armazenados em recursos conhecidos da infraestrutura de contadores de desempenho.</span><span class="sxs-lookup"><span data-stu-id="048f3-108">Instead, the data contains index values that you use to retrieve the names of the objects and counters, which are stored in resources known to the performance counters infrastructure.</span></span> <span data-ttu-id="048f3-109">No entanto, os nomes de instância são armazenados em dados de desempenho.</span><span class="sxs-lookup"><span data-stu-id="048f3-109">However, the instance names are stored in performance data.</span></span> <span data-ttu-id="048f3-110">Para obter um exemplo que lê os nomes de objeto e contador do registro e cria uma tabela de índice para acessar os nomes posteriormente, consulte [recuperando nomes de contador e texto de ajuda](retrieving-counter-names-and-help-text.md).</span><span class="sxs-lookup"><span data-stu-id="048f3-110">For an example that reads the object and counter names from the registry and creates an index table for accessing the names later, see [Retrieving Counter Names and Help Text](retrieving-counter-names-and-help-text.md).</span></span>

<span data-ttu-id="048f3-111">O exemplo a seguir mostra como recuperar o objeto, a instância e os blocos de contadores dos dados de desempenho.</span><span class="sxs-lookup"><span data-stu-id="048f3-111">The following example shows how to retrieve the object, instance, and counter blocks from the performance data.</span></span> <span data-ttu-id="048f3-112">Depois de recuperar os nomes de objeto, nomes de instância e nomes de contadores, o exemplo classifica os objetos por nome e para cada objeto, classifica suas instâncias e contadores por nome.</span><span class="sxs-lookup"><span data-stu-id="048f3-112">After retrieving the object names, instance names, and counter names, the example sorts the objects by name and for each object, sorts its instances and counters by name.</span></span> <span data-ttu-id="048f3-113">Em seguida, o exemplo imprime as listas classificadas.</span><span class="sxs-lookup"><span data-stu-id="048f3-113">The example then prints the sorted lists.</span></span>
