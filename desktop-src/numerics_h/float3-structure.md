...
Esse tipo está disponível apenas em C++. Seu equivalente em .NET é [System. Numerics. Vector3](/dotnet/api/system.numerics.vector3?view=netframework-4.8).
...
| `float3 normalize(float3 const& value)` | Cria um vetor de unidade a partir do vetor especificado. |
| `float3 cross(float3 const& vector1, float3 const& vector2)` | Calcula o produto cruzado de dois vetores. |
| `float3 reflect(float3 const& vector, float3 const& normal)` | Determina o vetor de reflexão do vetor e normal específico. |
...
| `float3 transform(float3 const& value, quaternion const& rotation)` | Transforma um float3 pelo Quaternion especificado. |
...
| `float3 operator* (float3 const& value1, float3 const& value2)` | Multiplica os componentes de dois vetores entre si. |
...
| `float3 operator- (float3 const& value)` | Retorna um ponto de vetor na direção oposta. |
| `float3& operator+= (float3& value1, float3 const& value2)` | O in-loco adiciona dois vetores. |
| `float3& operator-= (float3& value1, float3 const& value2)` | O in-loco subtrai um vetor de um vetor. |
| `float3& operator*= (float3& value1, float3 const& value2)` | O in-loco multiplica os componentes de dois vetores entre si. |
