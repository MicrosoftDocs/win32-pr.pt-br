---
description: O exemplo a seguir mostra como usar a função GetNPPBlobTable para recuperar uma tabela de BLOB NPP que representa as placas de interface de rede registradas no computador local.
ms.assetid: 7267f658-103d-4290-8ebf-b78866bd1fe8
title: Usando GetNPPBlobTable
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "105768585"
---
# <a name="using-getnppblobtable"></a><span data-ttu-id="db18e-103">Usando GetNPPBlobTable</span><span class="sxs-lookup"><span data-stu-id="db18e-103">Using GetNPPBlobTable</span></span>

<span data-ttu-id="db18e-104">O exemplo a seguir mostra como usar a função [**GetNPPBlobTable**](getnppblobtable.md) para recuperar uma tabela de blob NPP que representa as placas de interface de rede registradas no computador local.</span><span class="sxs-lookup"><span data-stu-id="db18e-104">The following example shows how to use the [**GetNPPBlobTable**](getnppblobtable.md) function to retrieve a NPP BLOB table that represents the registered network interface cards on the local computer.</span></span>
