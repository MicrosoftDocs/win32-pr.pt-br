...
keywords:
- Windows SDK do formato de mídia, habilitando o DXVA
- Windows SDK do formato de mídia, DXVA (aceleração de vídeo do DirectX)
- Windows SDK de formato de mídia, aceleração de vídeo
...
a partir do SDK do Windows Media Format 9 Series, o filtro Wrapper DMO dá suporte ao DirectX VA. isso significa que, para reprodução local, os aplicativos podem usar o filtro de leitor ASF do WM para reproduzir Windows conteúdo baseado em mídia e a aceleração de hardware DirectX VA será invocada automaticamente se a placa gráfica oferecer suporte a ela. No entanto, o filtro de leitor ASF do WM não oferece suporte à reprodução de conteúdo transmitido. portanto, se você quiser dar suporte ao DirectX VA ao reproduzir conteúdo transmitido em um player personalizado, deverá usar um mecanismo alternativo, que é aquele usado pelo Windows Media Player a partir da série Windows Media 9.

como Windows Media Player foi desenvolvido antes de os filtros QASF terem sido desenvolvidos, Windows Media Player tem seu próprio filtro de origem, com base no SDK do formato de mídia Windows, para executar Windows conteúdo baseado em mídia. o filtro de origem de mídia do WMP Windows entrega dados descompactados downstream diretamente para os renderizadores de áudio e vídeo. por outro lado, o leitor de ASF do WM fornece conteúdo compactado downstream para os DMOs (objetos de mídia do DirectX) do decodificador de mídia Windows, que são hospedados dentro do Wrapper de DMO. os diagramas a seguir ilustram as diferenças entre o leitor ASF do WM e o Windows o filtro de origem de mídia do WMP.
...
Para habilitar o DirectX VA para conteúdo transmitido, você deve criar um filtro de origem personalizado como aquele no diagrama superior. basicamente, esse filtro usará o SDK do formato de mídia Windows para criar uma instância de um objeto do WM Reader, descompactar os exemplos e enviá-los downstream em seus pins de saída. Esta discussão pressupõe que você já criou o filtro de origem e agora está pronto para implementar o suporte do DirectX VA.

para habilitar o DirectX VA, a tarefa básica do filtro de origem é fornecer o processador de vídeo e o decodificador WMV DMO com as interfaces de que eles precisarão para negociar a conexão do DirectX VA. O filtro de origem não participa dessas negociações. Após o streaming começar, a única tarefa relacionada ao DirectX VA que o filtro de origem pode executar é modificar os carimbos de data/hora nos exemplos de vídeo antes que o decodificador WMV os entregue ao processador de vídeo. o principal motivo para fazer isso é fornecer controle de linha do tempo personalizado além do que as interfaces de® de DirectShow padrão habilitam.

três interfaces são definidas para habilitar as comunicações necessárias entre o SDK do formato de mídia Windows, o filtro de origem do player, o DMO do decodificador de vídeo de mídia Windows e a Mixer de sobreposição ou o processador de mixagem de vídeo. Essas interfaces são descritas na tabela a seguir.



| Interface                                                        | Descrição                                                                                                                                                                                        |
|------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**IWMCodecAMVideoAccelerator**](/previous-versions/windows/desktop/api/wmdxva/nn-wmdxva-iwmcodecamvideoaccelerator) | exposto pelo decodificador de mídia Windows DMO e chamado pelo filtro de origem do player de mídia para configurar as várias conexões necessárias para habilitar o DirectX VA para a decodificação de conteúdo de vídeo de mídia Windows. |
...
-   um filtro de origem personalizado, instanciado pelo player, que usa o SDK do formato de mídia Windows para descompactar Windows conteúdo baseado em mídia.
-   O pino de saída do vídeo do filtro de origem do Player, conhecido como o pino de saída.
-   o grafo de filtro de reprodução de vídeo DirectShow, conhecido como grafo.
-   O processador de mixagem de vídeo, chamado de VMR.
-   o Windows objeto leitor assíncrono do Media Format SDK, conhecido como leitor.
-   o objeto de mídia DirectX do decodificador de vídeo de mídia Windows, chamado de DMO de decodificador.
...
2.  O Player chama **IGraphBuilder:: render**, fornecendo a ele o pino de saída do filtro de fonte de vídeo. neste ponto, o gerenciador do grafo de filtro de DirectShow tenta conectar o VMR ao filtro de origem de vídeo do jogador.
3.  o gerenciador do grafo de filtro chama **IPin:: Conexão** no pino de saída do filtro de origem de vídeo do jogador.
...
2.  o filtro de origem passa o ponteiro **IAMVideoAccelerator** do pin passado para **Conexão** para o decodificador DMO por meio de **IWMCodecAMVideoAccelerator:: SetAcceleratorInterface**.
3.  em seguida, o filtro de origem delega o restante da operação **IPin:: Conexão** ao método **CBaseOutputPin:: Conexão** . A enumeração de formato com o SDK continua como faz hoje. se o codec der suporte ao directx va para o conteúdo que está sendo conectado, o codec DMO apresentará esses subtipos do directx VA primeiro, antes dos tipos YUV e RGB com suporte. Se o suporte do DirectX VA estiver disponível, serão tentadas as etapas 7 a 11 no contexto de um subtipo DirectX VA. O trecho de código a seguir mostra como identificar um subtipo de mídia DirectX VA.
    ```C++
...
4.  a implementação **CBaseOutputPin:: Conexão** chama **IPin:: CompleteConnect** durante a etapa 3. Se um subtipo DirectX VA estiver sendo considerado, a negociação do DirectX VA será tentada. O pino de saída chama **IWMCodecAMVideoAccelerator:: NegotiateConnection**, passando-o para o tipo de mídia de saída atual.
5.  o decodificador DMO executa a negociação necessária com o VMR por meio da interface **IAMVideoAccelerator** e retorna o GUID de subtipo de vídeo que os dois concordaram. o pino de saída delega todas as chamadas de **IAMVideoAcceleratorNotify** recebidas durante esse processo para o decodificador DMO interface **IAMVideoAcceleratorNotify** , que também pode ser obtida por meio do método **IWMReaderAccelerator:: GetCodecInterface** .
