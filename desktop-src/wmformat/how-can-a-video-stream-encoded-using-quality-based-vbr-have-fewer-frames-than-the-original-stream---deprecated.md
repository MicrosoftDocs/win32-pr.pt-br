---
title: Como um fluxo de vídeo codificado usando a taxa de bits baseada em qualidade tem menos quadros do que o fluxo original
description: Para obter a documentação atual sobre os codecs de mídia do Windows e os processadores de sinal digital, consulte codec de áudio e vídeo do Windows Media e APIs do DSP. | Como um fluxo de vídeo codificado usando a taxa de bits baseada em qualidade tem menos quadros do que o fluxo original
ms.assetid: b33078aa-14d8-4b13-8c50-a6cff079871a
keywords:
- Como um fluxo de vídeo codificado usando a taxa de bits baseada em qualidade tem menos quadros que o formato de mídia do Windows de fluxo original
topic_type:
- apiref
api_name:
- How can a video stream encoded using quality-based VBR have fewer frames than the original stream
api_type:
- NA
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: daba486222cc5e8befaf4e5292ef6e018c0ebe70
ms.sourcegitcommit: 92e74c99f8f4d097676959d0c317f533c2400a80
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 03/09/2021
ms.locfileid: "103663882"
---
# <a name="how-can-a-video-stream-encoded-using-quality-based-vbr-have-fewer-frames-than-the-original-stream"></a>Como um fluxo de vídeo codificado usando a taxa de bits baseada em qualidade tem menos quadros do que o fluxo original

Para obter a documentação atual sobre os codecs de mídia do Windows e os processadores de sinal digital, consulte [codec de áudio e vídeo do Windows Media e APIs do DSP.](/previous-versions//dd464626(v=vs.85))

 

 
