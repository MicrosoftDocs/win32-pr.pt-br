...
- Windows SDK do formato de mídia, recompactação inteligente
- Windows SDK do formato de mídia, recompactação
- Windows media Format SDK, Windows codecs de áudio de mídia
...
- formato de sistema avançado (ASF), Windows codecs de áudio de mídia
- ASF (formato de sistemas avançados), Windows codecs de áudio de mídia
- transcodificação de conteúdo
- recompactação inteligente
- recompactação
- Windows Codecs de áudio de mídia, transcodificação de conteúdo
- codecs, Windows codecs de áudio de mídia
ms.topic: article
ms.date: 05/31/2018
...
você pode transcodificar o conteúdo de uma taxa de bits para outra usando o SDK do formato de mídia Windows. Normalmente, isso envolve simplesmente decodificar o conteúdo e codificá-lo novamente para a taxa de bits desejada. o codec Windows Media Audio 9 dá suporte à recompactação inteligente, o que permite transcodificação que atinge uma qualidade melhor do que o normal.

para a recompactação inteligente, o fluxo de áudio original deve ser codificado com o codec de áudio de mídia Windows. todas as versões do codec têm suporte, mas os codecs de áudio especializados (Windows mídia de áudio 9 Professional e Windows voz do áudio 9 mídia) não são. se o áudio original foi codificado com o Windows codec de áudio de mídia 9 sem perdas, não há necessidade de usar a recompactação inteligente, pois nenhuma informação foi perdida na codificação original.
