...
Você pode usar o Windows Media Format SDK para criar arquivos ASF que contêm mídia digital codificada com qualquer codec escolhido. Ao usar um codec diferente de um incluído neste SDK, você deve executar as etapas a seguir.
...
> [!Note]  
> Este tópico se aplica a aplicativos que usam o Windows Media Format SDK para colocar MPEG-2 (ou outros formatos de compactação que usam quadros B) no contêiner de arquivo ASF.
...
-   Para cada exemplo de leitura do arquivo, examine a conclusão. Se ele contiver uma hora de início maior ou igual a zero, copie esse valor para o carimbo de data/hora do exemplo de saída antes que ele seja entregue ao decodificador. Defina todos os outros carimbos de data/hora na saída exemplos como **NULL**. No DirectShow, isso é feito chamando **IMediaSample:: SetTime**(**NULL**,**NULL**).
