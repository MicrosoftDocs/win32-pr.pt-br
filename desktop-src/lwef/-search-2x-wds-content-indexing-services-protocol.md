...
\[SALTON Salton, G., "Processamento automático de texto: a análise de transformação e a recuperação de informações por \] computador", 1988, ISBN 0-201-2227-8.

\[UNICODE \] O Consórcio Unicode, "O Padrão Unicode, versão 2.0", 1996, https://www.unicode.org
...
### <a name="13-protocol-overview-synopsis"></a>1.3 Visão geral do protocolo (Synopsis)
...
Ao processar arquivos, um serviço de indexação analisa um conjunto de documentos e  reorganiza seu conteúdo de forma que as propriedades desses documentos possam ser retornadas com eficiência em resposta a consultas. Uma coleção de documentos que podem ser consultados é composta por um **catálogo** . Um catálogo pode conter um **índice** (para referência rápida ao texto) e um **cache** de propriedade (para recuperação rápida de valores de propriedade).

Conceitualmente, um catálogo consiste em uma "tabela" lógica de propriedades com o texto ou o valor e a localidade correspondente armazenadas em **colunas** da tabela. Cada "linha" da tabela corresponde a um documento separado no escopo do catálogo e cada "coluna" da tabela corresponde a uma propriedade .

As tarefas específicas executadas pelo CISP são agrupadas em duas áreas funcionais:

-   Administração remota de catálogos de serviços de indexação,
-   Consulta remota de catálogos de serviços de indexação.

### <a name="131-remote-administration-tasks"></a>1.3.1 Tarefas de Administração Remota

O CISP habilita as seguintes tarefas de gerenciamento do catálogo de serviços de indexação de um cliente:

-   Consulte o estado atual de um catálogo de serviços de indexação no servidor.
-   Atualize o estado de um catálogo de serviços de indexação.
-   Iniciar o processo de indexação para um conjunto específico de arquivos.
...
1.  O cliente solicita uma conexão com um servidor que hospeda um serviço de indexação.
2.  O cliente envia os parâmetros para a consulta de pesquisa, que incluem:
    ...
    -   O tempo máximo para execução da consulta.
...
3.  Em seguida, o cliente especifica quais propriedades o servidor deve incluir nos resultados da pesquisa.
4.  O cliente solicita um conjunto de resultados do servidor e o servidor responde enviando ao cliente os valores de propriedade para arquivos que foram incluídos nos resultados da consulta de pesquisa do cliente. Se o valor de uma propriedade for muito grande para caber em um único buffer de resposta, o servidor não enviará a propriedade, mas definirá o status da propriedade como adiado. Em seguida, o cliente solicita o valor da propriedade separadamente usando uma série de solicitações para partes sucessivas do valor e, em seguida, continua solicitando outros valores.
5.  Depois que o cliente terminar a consulta de pesquisa e não exigir mais resultados, o cliente entrará em contato com o servidor para liberar a consulta.
6.  Depois que o servidor tiver liberado a consulta, o cliente poderá enviar uma solicitação para se desconectar do servidor. Em seguida, a conexão é fechada. Como alternativa, o cliente pode emitir outra consulta e repetir a sequência da etapa 2.

Windows Comportamento: esse protocolo é implementado no Windows 2000, Windows XP, Windows Server 2003 e Windows Vista.

### <a name="14-relationship-to-other-protocols"></a>1.4 Relacionamento com outros protocolos

O CISP depende do protocolo SMB \[ MS-SMB \] para transporte de mensagens. Nenhum outro protocolo depende diretamente do Protocolo de Serviço de Indexação de Conteúdo.

*Windows Comportamento: os aplicativos normalmente interagem com um wrapper de interface do \[ OLE DB MSDN-OLEDB (por exemplo, um cliente de protocolo) e não diretamente \] com o protocolo.*

### <a name="15-prerequisites-and-preconditions"></a>1.5 Pré-requisitos e pré-condições

Supõe-se que o cliente tenha obtido o nome do servidor e um nome de catálogo antes que esse protocolo seja invocado. Como um cliente faz isso está fora do escopo dessa especificação.

Supõe-se também que o cliente e o servidor tenham uma associação de segurança que pode ser usável com pipes \[ nomeados MS-SMB. \]

### <a name="16-applicability-statement"></a>1.6 Applicability Statement

O CISP foi projetado para consultar e gerenciar catálogos em um servidor remoto de um cliente. O CISP foi preterido no Windows Vista.

### <a name="17-versioning-and-capability-negotiation"></a>1.7 Controle de versão e negociação de capacidade

Esse protocolo não tem mecanismos de negociação de capacidade ou controle de versão.

### <a name="18-vendor-extensible-fields"></a>1.8 Campos extensíveis para fornecedor

Esse protocolo usa HRESULTs que são extensíveis para o fornecedor. Os fornecedores podem escolher seus próprios valores para esse campo, desde que o bit C (0x20000000) seja definido conforme especificado na Seção 4.1.1 do \[ MS-SYS, indicando que ele é um código do \] cliente.

Esse protocolo também usa valores NTSTATUS retirados do espaço de número NTSTATUS definido no \[ MS-SYS \] . Os fornecedores DEVEM reutilizar esses valores com o significado indicado. Escolher qualquer outro valor correrá o risco de uma colisão no futuro.

*Windows Comportamento: Windows usa apenas os valores especificados na Seção 4.1.3 \[ do MS-SYS. \]*

### <a name="181-property-ids"></a>1.8.1 IDs de propriedade

As propriedades são representadas por IDs conhecidas como IDs de propriedade. Cada propriedade deve ter um identificador global exclusivo. Esse identificador consiste em um **GUID** que representa uma coleção de propriedades chamada conjunto de propriedades mais uma cadeia de caracteres ou inteiro de 32 bits para identificar a propriedade dentro do conjunto. Se a forma de inteiro da ID for usada, os valores 0x00000000, 0xFFFFFFFF e 0xFFFFFFFE serão considerados inválidos.
...
Esse protocolo não tem atribuições de padrões, somente atribuições privadas feitas pela Microsoft usando procedimentos de alocação especificados em outros protocolos.

A Microsoft alocau a esse protocolo um pipe nomeado, conforme especificado \[ em MS-SMB. \] A atribuição é:



| Parâmetro | Valor             | Referência  |
|-----------|-------------------|------------|
| Nome do pipe | \\\\ \_ SKADS de CI de pipe | \[MS-SMB\] |
...
-   \\\\ \_ SKADS de CI de pipe

Esse protocolo usa o protocolo de pipe nomeado SMB subjacente para recuperar a identidade do chamador que fez a conexão conforme especificado na Seção 2.2.8 do MS-SMB ; o cliente DEVE definir IDENTIFICAÇÃO DE SEGURANÇA como \[ \] ImpersonationLevel na solicitação para abrir o \_ pipe nomeado.

### <a name="22-message-syntax"></a>2.2 Sintaxe da mensagem

Várias estruturas e mensagens nas seções a seguir referem-se a alças de capítulo ou indicador. Um identificador é uma estrutura opaca de 32 bits que identifica exclusivamente um capítulo ou indicador. Normalmente, os aplicativos cliente recebem valores de alça por meio de chamadas de método; no entanto, há vários valores conhecidos que não precisam ser obtidos de um servidor, o significado do qual é especificado na tabela a seguir:



| Valor                         | Significado                                                                      |
|-------------------------------|------------------------------------------------------------------------------|
| BANCO \_ DE DADOS NULL \_ HCHAPTER 0x00000000 | Um handle de capítulo para o conjuntos de linhas não enchapterados, contendo todos os resultados da consulta.    |
| DBBMK \_ FIRST 0x00000001       | Um identificador de indicador para um indicador que identifica a primeira linha no conjuntos de linhas. |
...
| BDKIND \_ GUID \_ PROPID 0x00000001  | ulId contém um inteiro de 4 byte que indica a ID da propriedade.                                                      |
...
**\_ aBookmarks:** uma matriz de alças de indicador (cada um representado por 4 bytes), conforme obtido de uma mensagem CPMGetRowsOut.
...
**\_ cCmdTimeout:** um inteiro sem sinal de 32 bits, especificando o número de segundos em que uma consulta deve ser encerrada e encerrada automaticamente, contando a partir do momento em que a consulta começa a ser executada no servidor. Um valor de 0x00000000 significa que a consulta não deve ter tempo de vida.
...
**vType**: especifica o tipo de valor de dados contido na coluna. Consulte o campo vType na Seção 2.2.1.1 para ver a lista de valores desse campo.
...
**\_ padding1:** um campo de um byte que DEVE ser inserido antes de ValueOffset se, sem ele, ValueOffset não começar em um deslocamento de mesmo valor desde o início da mensagem. O valor desse byte é arbitrário e DEVE ser ignorado. Se ValueUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.

**ValueOffset:** um inteiro de 2 byte sem sinal especificando o deslocamento do valor da coluna na linha. Se ValueUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.

**ValueSize:** um inteiro sem sinal de 2 bytes especificando o tamanho do valor da coluna em bytes. Se ValueUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.
...
**\_ padding2:** um campo de um byte que DEVE ser inserido antes de StatusOffset se, sem ele, o campo StatusOffset não começar em um deslocamento de até mesmo do início da mensagem. O valor desse byte é arbitrário e DEVE ser ignorado. Se StatusUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.

**StatusOffset:** um inteiro sem sinal de 2 byte que especifica o deslocamento do status da coluna na linha. Se StatusUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.

**LengthUsed:** um campo de um byte que DEVE ser definido como 0x01 ou 0x00. Se definido como 0x01, o comprimento da coluna será transferido dentro da linha. Se 0x00, o comprimento da coluna NÃO DEVE ser transferido dentro da linha.

**\_ padding3:** um campo de um byte que DEVE ser inserido antes de LengthOffset se, sem ele, LengthOffset não começar em um deslocamento de até mesmo do início de uma mensagem. O valor desse byte é arbitrário e DEVE ser ignorado. Se LengthUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.

**LengthOffset:** um inteiro de 2 byte sem sinal que especifica o deslocamento do comprimento da coluna na linha. Se LengthUsed estiver definido como 0x00, esse campo NÃO DEVERÁ estar presente.
...
**dwType**: um dos tipos variantes, definidos na seção 2.2.1.1, que pode ser combinado com modificadores de tipo variante. Para todos os tipos variantes, exceto aqueles combinados com VT ARRAY, SERIALIZEDPROPERTYVALUE tem o mesmo layout que \_ CBaseStorageVariant, conforme especificado na Seção 2.2.1.1. Se o tipo de variante for combinado com o modificador de tipo VT ARRAY, SAFEARRAY2, especificado na Seção \_ 2.2.1.2.1.1, será usado em vez de SAFEARRAY no campo vValue de CBaseStorageVariant.

**rgb:** valor serializado. Consulte detalhes de serialização para vValue na versão 2.2.1.1.

### <a name="222-message-headers"></a>2.2.2 Headers de Mensagem

Todas as mensagens do Protocolo do Serviço de Indexação de Conteúdo têm um título de 16 byte.

Veja abaixo um diagrama que mostra o formato de título da mensagem do Protocolo de Serviço de Indexação de Conteúdo.
...
\_**msg:** um inteiro de 32 bits que identifica o tipo de mensagem após o header. A tabela a seguir lista as mensagens do Protocolo de Serviço de Indexação de Conteúdo e os valores inteiros especificados para cada mensagem. Conforme mostrado na tabela, alguns valores identificam duas mensagens na tabela. Nesses casos, a mensagem após o header pode ser identificada pela direção do fluxo de mensagem. Se a direção for de cliente para servidor, a mensagem com "In" anexada ao nome da mensagem será indicada. Se a direção for de servidor para cliente, a mensagem com "Out" anexada ao nome da mensagem será indicada.
...
\_**status**: um HRESULT \[ MS-SYS \] , indicando o status da operação solicitada.

\* *

*Windows Comportamento: o cliente sempre define o \_ campo de status como 0x00000000.*

**\_ ulChecksum:** o ulChecksum DEVE ser calculado conforme especificado na Seção \_ 3.2.4 para as seguintes mensagens:
...
**\_ ulReserved2:** DEVE ser definido como 0 e ignorado pelo receptor.

### <a name="223-messages"></a>2.2.3 Mensagens

### <a name="2231-cpmcistateinout"></a>2.2.3.1 CPMCiStateInOut

A mensagem CPMCiStateInOut contém informações sobre o estado do serviço de indexação. O formato da mensagem CPMCiStateInOut que segue o header é:
...
*comportamento de Windows*: esse valor geralmente é zero, exceto imediatamente após a indexação ter sido iniciada ou após o estouro de uma fila de notificação.
...
O formato da mensagem CPMConnectIn que segue o header é:
...
**\_ iClientVersion:** um inteiro de 32 bits que indica se o servidor deve validar o valor de verificação especificado no campo ulChecksum dos headers de mensagem para mensagens enviadas pelo \_ cliente. Se o campo iClientVersion estiver definido como 0x00000008 ou superior, o servidor DEVERÁ validar o valor do campo \_ \_ ulChecksum para as seguintes mensagens:
...
Se o valor for maior que 0x00000008 o cliente será presumido capaz de lidar com deslocamentos de 64 bits em mensagens CPMGetRowsOut. Consulte a seção 2.2.3.16 para obter detalhes.

*Windows comportamento: em Windows, o iClientVersion é definido da seguinte forma:*



| Valor      | Significado                                                              |
|------------|----------------------------------------------------------------------|
| 0x00000005 | O sistema operacional cliente Windows 2000.                                           |
| 0x00000008 | O sistema operacional cliente é um Windows XP de 32 bits ou 32 bits Windows Server 2003. |
| 0x00010008 | O sistema operacional cliente é um Windows XP de 64 bits ou 64 bits Windows Server 2003. |



 

\_**fClientIsRemote:** um valor booliana que indica se o cliente está em execução em um computador diferente do servidor. DEVE ser definido como 0x00000001.

\_**cbBlob1:** um inteiro sem sinal de 32 bits que indica o tamanho em bytes dos campos cPropSet, PropertySet1 e PropertySet2, combinados.

\_**cbBlob2:** um inteiro sem sinal de 32 bits que indica o tamanho em bytes dos campos cExPropSet e aPropertySet, combinados.

\_**preenchimento:** 12 bytes de preenchimento que podem conter valores arbitrários e DEVEM ser ignorados.

**MachineName:** o nome do computador do cliente. A cadeia de caracteres de nome DEVE ser uma matriz terminada em nulo de menos de 512 caracteres Unicode, incluindo o terminador NULL.

**UserName:** uma cadeia de caracteres que representa o nome de usuário da pessoa que está executando o aplicativo que invocou esse protocolo. A cadeia de caracteres de nome DEVE ser uma matriz terminada em nulo de menos de 512 caracteres Unicode quando concatenada com MachineName.

**\_ paddingcPropSets:** esse campo DEVE ter de 0 a 7 bytes de comprimento. O número de bytes DEVE ser o número necessário para fazer com que o deslocamento de bytes do campo cPropSets do início da mensagem que contém essa estrutura seja um múltiplo de 8. O valor dos bytes pode ser qualquer valor arbitrário e DEVE ser ignorado pelo receptor.

**cPropSets:** um inteiro sem sinal de 32 bits que indica o número de estruturas CDbPropSet após esse campo. Esse valor DEVE ser definido como 0x0000002.

**PropertySet1:** uma estrutura CDbPropSet com guidPropertySet que contém DBPROPSET \_ FSCIFRMWRK EXT (consulte a seção \_ 2.2.1.22).

**PropertySet2:** uma estrutura CDbPropSet com guidPropertySet que contém DBPROPSET \_ CIFRMWRKCORE EXT (consulte a seção \_ 2.2.1.22).
...
**\_ hCursor:** um inteiro sem sinal de 32 bits que representa o identificador da mensagem CPMCreateQueryOut identificando a consulta para a qual recuperar informações de status.

### <a name="22311-cpmgetquerystatusout"></a>2.2.3.11 CPMGetQueryStatusOut

A mensagem CPMGetQueryStatusOut responde a uma mensagem CPMGetQueryStatusIn com o status da consulta. O formato da mensagem CPMGetQueryStatusOut que segue o header é:
...
**\_ Status:** um bitmask de valores definidos nas tabelas abaixo, que descreve a consulta.

A tabela a seguir lista os valores STAT obtidos executando uma operação AND bit a bit \_ \* em Status com \_ 0x00000007. O resultado DEVE ser um dos seguintes:



| Constante                 | Significado                                                                           |
|--------------------------|-----------------------------------------------------------------------------------|
| ESTATÍSTICAS \_ OCUPADAS 0X00000000    | A consulta assíncrona ainda está em execução.                                          |
| ESTATÍSTICAS \_ DE ERRO 0X00000001   | A consulta está em um estado de erro.                                                   |
| STAT \_ DONE 0X00000002    | A consulta está concluída.                                                            |
| ATUALIZAÇÃO \_ DE ESTATÍSTICAS 0x00000003 | A consulta foi concluída, mas as atualizações estão resultando em computação de consulta adicional. |



 

A tabela a seguir lista os bits STAT \_ \* adicionais que podem ser definidos independentemente.



| Constante                                    | Significado                                                                                                                             |
|---------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| PALAVRAS \_ DE RUÍDO DE ESTATÍSTICAS \_ 0X00000010               | Palavras de ruído foram substituídas por caracteres curinga na consulta de conteúdo.                                                              |
| CONTEÚDO \_ DE \_ ESTATÍSTICAS \_ \_ DESA 0X00000020     | Os resultados da consulta podem estar incorretos porque a consulta envolvida foi modificada, mas não indexada, arquivos.                             |
| ATUALIZAÇÃO \_ DE ESTATÍSTICAS INCOMPLETA \_ 0X00000040        | Os resultados da consulta podem estar incorretos porque a consulta envolvida modificou e re indexou arquivos cujo conteúdo não foi incluído. |
| CONSULTA \_ DE CONTEÚDO DE ESTATÍSTICAS INCOMPLETA \_ \_ 0X00000080 | A consulta de conteúdo era muito complexa para concluir ou a enumeração necessária em vez de usar o índice de conteúdo.                          |
| TEMPO \_ \_ LIMITE DE ESTATÍSTICAS \_ EXCEDIDO 0X00000100      | Os resultados da consulta podem estar incorretos porque o tempo de execução da consulta atingiu o tempo máximo permitida.                    |
...
A mensagem CPMGetQueryStatusExIn solicita o status de uma consulta e informações adicionais, como o número de documentos que foram indexados, o número de documentos restantes a serem indexados e assim por diante. O formato da mensagem CPMGetQueryStatusExIn que segue o header é:
...
**\_ bmk:** um valor de 32 bits que indica a alça de um indicador cuja posição deve ser recuperada.

### <a name="22313-cpmgetquerystatusexout"></a>2.2.3.13 CPMGetQueryStatusExOut

A mensagem CPMGetQueryStatusExOut responde a uma mensagem CPMGetQueryStatusExIn com o status da consulta e outras informações de status, conforme descrito no diagrama abaixo. O formato da mensagem CPMGetQueryStatusExOut que segue o header é:
...
**\_ Status**: uma das estatísticas \_ \* valores especificados na Seção 2.2.3.11.

**\_ cFilteredDocuments:** um inteiro sem sinal de 32 bits que indica o número de documentos que foram indexados

**\_ cDocumentsToFilter:** um inteiro sem sinal de 32 bits que indica o número de documentos que ainda permanecem a ser indexados.

**\_ dwRatioFinishedDenominator:** um inteiro sem sinal de 32 bits que indica o denominador da taxa de documentos que a consulta concluiu o processamento.

**\_ dwRatioFinishedNumerator:** um inteiro sem sinal de 32 bits que indica o numerador da proporção de documentos que a consulta concluiu o processamento.

**\_ iRowBmk:** um inteiro sem sinal de 32 bits que indica a posição aproximada do indicador no conjuntos de linhas em termos de linhas.

**\_ cRowsTotal:** um inteiro sem sinal de 32 bits que especifica o número total de linhas no conjuntos de linhas.

### <a name="22314-cpmsetbindingsin"></a>2.2.3.14 CPMSetBindingsIn

A mensagem CPMSetBindingsIn solicita a associação de colunas a um conjuntos de linhas. O servidor responderá à mensagem de solicitação CPMSetBindingsIn usando a seção de header da mensagem CPMBindingsIn com os resultados da solicitação contida no campo \_ de status. O formato da mensagem CPMSetBindingsIn que segue o header é:
...
**\_ fBwdFetch**: um inteiro de 32 bits sem sinal indicando a ordem na qual buscar as linhas. Se definido como 0x00000001, as linhas serão buscadas na ordem inversa. Se definido como 0x00000000, as linhas serão buscadas na ordem de encaminhamento. NÃO DEVE ser definido como nenhum outro valor.

**eType:** um inteiro sem sinal de 32 bits que contém um dos seguintes valores para indicar o tipo de operação a ser realizada.
...
**\_ chapt:** um valor de 32 bits que representa o handle do capítulo de conjuntos de linhas.
...
A mensagem CPMGetRowsOut responde a uma mensagem CPMGetRowsIn com as linhas de uma consulta. Os servidores DEVEM formatar deslocamentos para tipos de dados de comprimento variável no campo Linhas da seguinte forma:
...
-   O cliente indicou que era um sistema de 64 bits ( iClientVersion > 0x00000008 no CPMConnectIn) e o servidor indicou que era um sistema de 32 bits ( serverVersion definido como \_ \_ 0x00000007 em CPMConnectOut): deslocamentos são inteiros de 32 bits
-   O cliente indicou que era um sistema de 64 bits ( iClientVersion > 0x00000008 no CPMConnectIn) e o servidor indicou que era um sistema de 64 bits ( serverVersion definido como \_ \_ 0x00010007 em CPMConnectOut): os deslocamentos são inteiros de 64 bits

O formato da mensagem CPMGetRowsOut que segue o header é:
...
**\_ cRowsReturned:** um inteiro sem sinal de 32 bits que indica o número de linhas retornadas em Linhas.

**eType:** um inteiro sem sinal de 32 bits que contém um dos seguintes valores para indicar o tipo de operação rowseek a ser realizada
...
**paddingRows:** esse campo DEVE ser de comprimento suficiente (0 para cbReserved-1 bytes) para pad do campo Linhas para o deslocamento cbReserved do início de uma mensagem, em que cbReserved é o valor na mensagem \_ \_ \_ CPMGetRowsIn. Bytes de preenchimento usados neste campo podem ser qualquer valor arbitrário. Esse campo DEVE ser ignorado pelo receptor.

**Linhas:** os dados de linha são formatados conforme prescrito pelas informações de coluna na mensagem CPMSetBindingsIn mais recente. As linhas DEVEM ser armazenadas em ordem de avanço (por exemplo, linha 1 antes da linha 2).
...
Colunas de tamanho variável (por exemplo, cadeias de caracteres) DEVEM ser armazenadas da seguinte forma:

-   Os dados da variável em si (por exemplo, a cadeia de caracteres) são armazenados próximos ao final do buffer em ordem decrescente (por exemplo, a coleção de todos os dados variáveis da linha 1 está no final, a linha 2 mais próxima, etc.).
-   A área de tamanho fixo (no início do buffer de linha) DEVE conter um CRowVariant para cada coluna, armazenado no deslocamento especificado na mensagem CPMSetBindingsIn mais recente. vType DEVE conter o tipo de dados (por ex. VT \_ LPWSTR). Se, conforme determinado pelas regras no início da seção desta seção, deslocamentos de 32 bits estão sendo usados, o campo Offset em CRowVariant DEVE conter um valor de 32 bits que é o deslocamento dos dados variáveis do início da mensagem CPMGetRowsOut, mais o valor de ulClientBase especificado na mensagem \_ CPMGetRowsIn mais recente. Se deslocamentos de 64 bits estão sendo usados, o campo Offset em CRowVariant DEVE conter um valor de 64 bits que é o deslocamento do início da mensagem CPMGetRowsOut, adicionado a um valor de 64 bits composto usando ulClientBase como os 32 bits baixos e ulReserved2 como os \_ \_ 32 bits altos.

Por exemplo, se a mensagem CPMSetBindingsIn tiver especificado duas colunas-Size (VT \_ I4) e Title (VT \_ LPWSTR) e \_ ulClientBase de CPMGetRowsIn for 0x10000, duas linhas aparecerão da seguinte forma. A seção marcada em cinza é a parte de comprimento fixo do buffer.
...
A mensagem CPMRatioFinishedIn solicita o percentual de conclusão de uma consulta. O formato da mensagem CPMRatioFinishedIn que segue o header é:
...
-   Verifique se o catálogo associado ao cliente está no estado que permite que a consulta seja processada (CICAT \_ READONLY ou CICAT \_ WRITABLE). Se esse não for o caso, o servidor DEVERÁ relatar um erro QUERY \_ S \_ NO \_ QUERY (0X8004160C).
...
4.  O servidor verifica se o **\_ ulChecksum** está correto, verifica se o usuário está autorizado a fazer essa solicitação e responde com uma mensagem CPMConnectOut.
    ...
    -   o campo **\_ serverVersion** é definido como 0x00000007 (32-bit Windows XP ou 32-bit Windows Server 2003).
...
5.  O cliente prepara uma mensagem CPMCreateQueryIn.
    ...
    -   **PidMapper** é definido como:
        ...
        -   **\_ aPropSpec** é definido como GUID B725F130-47EF-101A-A5-F1-02608C9EEBAC/0x00000001 (para PRSPEC \_ PROPID)/0x0000000c que representa a propriedade de tamanho de arquivo Windows.
...
7.  O cliente emite uma mensagem de solicitação CPMSetBindingsIn para definir o formato de uma linha.
    ...
    -   -   **\_ PropSpec** é definido como GUID b725f130-47ef-101a-a5-f1-02608c9eebac/0x00000001 (para PRSPEC \_ PROPID)/0x0000000c que representa a propriedade de tamanho de arquivo Windows.
...
| Os aplicativos normalmente interagem com um wrapper OLE DB interface                                  | 1.4       | X            | X          | X                   |
| Valores NTSTATUS                                                                                   | 1.8       | X            | X          | X                   |
| O cliente define o \_ campo de status em cada Header de Mensagem.                                        | 2.2.2     | X            | X          | X                   |
