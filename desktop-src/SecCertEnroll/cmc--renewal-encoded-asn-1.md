---
description: O exemplo a seguir contém uma solicitação de renovação de CMC mostrada no formato ASN. 1. O exemplo foi gerado usando as ferramentas Certreq.exe e Certutil.exe. O arquivo. inf usado como entrada para Certreq.exe contém a configuração a seguir.
...
O exemplo a seguir contém uma solicitação de renovação de CMC mostrada no formato ASN. 1. O exemplo foi gerado usando as ferramentas Certreq.exe e Certutil.exe. O arquivo. inf usado como entrada para Certreq.exe contém a configuração a seguir.
