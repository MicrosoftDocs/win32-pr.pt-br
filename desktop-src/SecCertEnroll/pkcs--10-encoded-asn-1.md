---
description: O exemplo a seguir contém uma \# solicitação PKCS 10 mostrada no formato ASN. 1. O exemplo foi gerado usando as ferramentas Certreq.exe e Certutil.exe. O arquivo. inf usado como entrada para Certreq.exe contém a configuração a seguir.
ms.assetid: 5001d641-a121-421a-90bf-f2e6e5f88dc8
title: '\#ASN codificado no PKCS 10.1'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104172151"
---
# <a name="pkcs-10-encoded-asn1"></a>\#ASN codificado no PKCS 10.1

O exemplo a seguir contém uma \# solicitação PKCS 10 mostrada no formato ASN. 1. O exemplo foi gerado usando as ferramentas Certreq.exe e Certutil.exe. O arquivo. inf usado como entrada para Certreq.exe contém a configuração a seguir.
...
Essa configuração gera a seguinte saída de exemplo. A configuração especifica o assunto, o tipo de solicitação (PKCS \# 10) e o modelo de certificado (usuário). O modelo de usuário especifica que:
