...
---
# <a name="testing-for-the-end-of-a-file"></a><span data-ttu-id="3213d-103">Teste para o final de um arquivo</span><span class="sxs-lookup"><span data-stu-id="3213d-103">Testing for the End of a File</span></span>

<span data-ttu-id="3213d-104">A função [**ReadFile**](/windows/desktop/api/FileAPI/nf-fileapi-readfile) verifica a condição de fim de arquivo (EOF) de forma diferente para operações de leitura síncronas e assíncronas.</span><span class="sxs-lookup"><span data-stu-id="3213d-104">The [**ReadFile**](/windows/desktop/api/FileAPI/nf-fileapi-readfile) function checks for the end-of-file condition (EOF) differently for synchronous and asynchronous read operations.</span></span> <span data-ttu-id="3213d-105">Quando uma operação de leitura síncrona chega ao final de um arquivo, o **ReadFile** retorna **true** e define a variável apontada pelo parâmetro *lpNumberOfBytesRead* como zero.</span><span class="sxs-lookup"><span data-stu-id="3213d-105">When a synchronous read operation gets to the end of a file, **ReadFile** returns **TRUE** and sets the variable pointed to by the *lpNumberOfBytesRead* parameter to zero.</span></span> <span data-ttu-id="3213d-106">Uma operação de leitura assíncrona pode encontrar o final de um arquivo durante a chamada de inicialização para **ReadFile** ou durante operações assíncronas subsequentes se o ponteiro do arquivo for avançado de forma programática além do final do arquivo.</span><span class="sxs-lookup"><span data-stu-id="3213d-106">An asynchronous read operation can encounter the end of a file during the initiating call to **ReadFile** or during subsequent asynchronous operations if the file pointer is programmatically advanced beyond the end of the file.</span></span>

<span data-ttu-id="3213d-107">O exemplo de C++ a seguir mostra como testar o final de um arquivo durante uma operação de leitura síncrona.</span><span class="sxs-lookup"><span data-stu-id="3213d-107">The following C++ example shows how to test for the end of a file during a synchronous read operation.</span></span>
...
<span data-ttu-id="3213d-108">O teste de fim de arquivo durante uma operação de leitura assíncrona é um pouco mais envolvido do que para uma operação de leitura síncrona semelhante.</span><span class="sxs-lookup"><span data-stu-id="3213d-108">The test for end-of-file during an asynchronous read operation is slightly more involved than for a similar synchronous read operation.</span></span> <span data-ttu-id="3213d-109">O indicador de fim de arquivo para operações de leitura assíncronas é quando [**GetOverlappedResult**](/windows/desktop/api/ioapiset/nf-ioapiset-getoverlappedresult) retorna **false** e [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) retorna o **identificador de erro \_ \_ EOF**.</span><span class="sxs-lookup"><span data-stu-id="3213d-109">The end-of-file indicator for asynchronous read operations is when [**GetOverlappedResult**](/windows/desktop/api/ioapiset/nf-ioapiset-getoverlappedresult) returns **FALSE** and [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) returns **ERROR\_HANDLE\_EOF**.</span></span>

<span data-ttu-id="3213d-110">O exemplo de C++ a seguir mostra como testar o final do arquivo durante uma operação de leitura assíncrona.</span><span class="sxs-lookup"><span data-stu-id="3213d-110">The following C++ example shows how to test for the end of file during an asynchronous read operation.</span></span>
...
<span data-ttu-id="3213d-111">A saída desse código de exemplo é a seguinte:</span><span class="sxs-lookup"><span data-stu-id="3213d-111">The output from this sample code is as follows.</span></span>
