...
Telefone de livros contém as informações necessárias para estabelecer uma conexão RAS. Um usuário ou administrador pode usar a **Sistema de Rede de Conexão Discada** caixa de diálogo para criar, editar e discar entradas de lista telefônica.

A partir do Windows NT Server 4.0, o sistema dá suporte às funções descritas para o Windows 95, bem como várias funções adicionais que um aplicativo pode usar para trabalhar com livros telefônicas e entradas de livros telefônicas.
...
A [**função RasGetCountryInfo**](/windows/desktop/api/Ras/nf-ras-rasgetcountryinfoa) recupera informações de discagem específicas de país/região da lista Windows telefonia de países. **RasGetCountryInfo** usa a [**estrutura RASCTRYINFO.**](/previous-versions/windows/desktop/legacy/aa376731(v=vs.85))
