...
Existem vários tipos de texturas: 1D, 2D, 3D, e cada uma pode ser criada com ou sem mipmaps. O Direct3D 11 também dá suporte a matrizes de textura e texturas multisampled.
...
Uma textura 1D em sua forma mais simples contém dados de textura que podem ser tratados com uma coordenada de textura única; pode ser visualizada como uma matriz de texels, conforme mostrado na ilustração a seguir. Texturas 1D são representadas pela interface [**ID3D11Texture1D.**](/windows/desktop/api/D3D11/nn-d3d11-id3d11texture1d)
...
Um recurso Texture2D contém uma grade 2D de texels. Cada texel é endereçável por um vetor u, v. Como é um recurso de textura, ele pode conter níveis de mipmap e sub-recursos. Texturas 2D são representadas pela interface [**ID3D11Texture2D.**](/windows/desktop/api/D3D11/nn-d3d11-id3d11texture2d) Um recurso de textura 2D totalmente preenchida é semelhante à ilustração a seguir.
...
![ilustração de uma matriz de texturas 2d que representam um cubo de textura](images/d3d10-resource-texturecube.png)
...
> [!Note]  
> Os dispositivos que você cria com [**10 \_ 1**](/windows/desktop/api/D3DCommon/ne-d3dcommon-d3d_feature_level) nível de recurso e superior podem dar suporte a matrizes de cubos de textura em que o número de texturas é igual ao número de cubos de textura em uma matriz vezes seis. Os dispositivos que você cria com [**nível de recurso \_ 10 0**](/windows/desktop/api/D3DCommon/ne-d3dcommon-d3d_feature_level) são compatíveis apenas com um único cubo de textura de seis faces. Além disso, o Direct3D 11 não dá suporte a cubemaps parciais.
