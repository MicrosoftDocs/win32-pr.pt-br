...
-   [Regras de rasterização de linha (com alias, sem multiamostragem)](#line-rasterization-rules-aliased-without-multisampling)
-   [Regras de rasterização de linha (com alias sem multiamostragem)](#line-rasterization-rules-antialiased-without-multisampling)
-   [Regras de rasterização de ponto (sem multiamostragem)](#point-rasterization-rules-without-multisampling)
-   [Regras de rasterização de suavização de multiamostra](#multisample-anti-aliasing-rasterization-rules)
    -   [Suporte a hardware](#hardware-support)
    -   [Amostragem de atributos de centróide quando a suavização de multiamostras](#centroid-sampling-of-attributes-when-multisample-antialiasing)
    -   [Cálculos derivativos quando multiamostra](#derivative-calculations-when-multisampling)
-   [Tópicos relacionados](#related-topics)

## <a name="triangle-rasterization-rules-without-multisampling"></a>Regras de rasterização de triângulo (sem várias amostragens)
...
-   Uma borda esquerda, é uma borda que não é exatamente horizontal e está no lado esquerdo do triângulo. Um triângulo pode ter uma ou duas bordas à esquerda.
...
## <a name="line-rasterization-rules-aliased-without-multisampling"></a>Regras de rasterização de linha (serrilhado, sem várias amostras)

Regras de rasterização de linha usam uma área de teste de diamante para determinar se uma linha abrange um pixel. Para as linhas principais x (linhas com-1 <= inclinação <= + 1), a área de teste de losango inclui (mostrada sólida) a borda inferior esquerda, a borda inferior direita e o canto inferior; o losango exclui (mostrado pontilhado) a borda superior esquerda, a borda superior direita, o fio superior, o canto esquerdo e o canto direito. Uma linha de y grande é qualquer linha que não seja uma linha de x grande; a área de diamante de teste é a mesma conforme descrito para a linha de x grande, exceto pelo canto direito também estar incluído.
...
## <a name="line-rasterization-rules-antialiased-without-multisampling"></a>Regras de rasterização de linha (com suavização, sem várias amostras)
...
Será considerado que não há uma "melhor" e única maneira para realizar uma renderização de linha com suavização. O Direct3D 10 adota como uma diretriz o método mostrado na ilustração a seguir. Esse método foi derivado empiricamente, apresentando um número de propriedades visuais considerado desejável. O hardware não precisa corresponder exatamente a esse algoritmo; testes em relação a essa referência deverão ter tolerâncias "razoáveis" orientadas por alguns dos princípios listados mais abaixo, permitindo várias implementações de hardware e tamanhos de kernel do filtro. No entanto, nenhuma dessas flexibilidade permitida na implementação de hardware pode ser comunicada por meio do Direct3D 10 para aplicativos, além de simplesmente desenhar linhas e observar/avaliar como elas parecem.

![ilustração de exemplos de rasterização de linha AntiAlias](images/d3d10-rasterruleslineaa.png)

Esse algoritmo gera linhas relativamente suaves, com intensidade uniforme, com mínimo de bordas com dentes ou trançados. O padrão moiré para linhas próximas é minimizado. Há uma boa cobertura de junções entre os segmentos de linha colocados de ponta a ponta. O kernel do filtro é um equilíbrio razoável entre a quantidade de desfoque de borda e as alterações na intensidade causadas por correções de gama. O valor de cobertura é multiplicado no sombreador de pixel o0. a (srcAlpha) de acordo com a fórmula a seguir pelo estágio output-merger: srcColor \* srcAlpha + destColor \* (1-srcAlpha).

## <a name="point-rasterization-rules-without-multisampling"></a>Regras de rasterização de pontos (sem várias amostragens)
...
## <a name="multisample-anti-aliasing-rasterization-rules"></a>Regras de rasterização de suavização de várias amostras
...
Muitos formatos dão suporte a várias amostras (consulte [suporte de hardware para formatos de Direct3D 10](/previous-versions//cc627090(v=vs.85))), alguns formatos podem ser resolvidos ([**ResolveSubresource**](/windows/desktop/api/d3d10/nf-d3d10-id3d10device-resolvesubresource); que downsamples um formato de várias amostras para um tamanho de amostra de 1). Formatos de várias amostras podem ser usados em destinos de renderização que podem ser lidas novamente em sombreadores usando [carregar](/windows/desktop/direct3dhlsl/dx-graphics-hlsl-to-load), já que nenhuma resolução é necessária para amostras individuais acessadas pelo sombreador. Não há suporte para formatos de profundidade para recurso de várias amostras. Portanto, os formatos de profundidade são restritos a somente destinos de renderização.

Os formatos sem tipo (R8G8B8A8, \_ por exemplo) oferecem suporte a várias amostras para permitir que uma exibição de recurso interprete dados de maneiras diferentes. Por exemplo, você pode criar um recurso de multiamostragem usando R8G8B8A8 \_ sem tipo, renderizar para ele usando um recurso render-Target-View com um \_ formato R8G8B8A8 uint e, em seguida, resolver o conteúdo para outro recurso com um \_ formato de dados R8G8B8A8 UNORM.
...
-   Cada formato que dá suporte a multiamostrabilidade e tem os \_ formatos UNORM, \_ sRGB, \_ SNORM ou \_ float, também dá suporte à resolução.

### <a name="centroid-sampling-of-attributes-when-multisample-antialiasing"></a>Amostragem de centroides de atributos com suavização de várias amostras
...
### <a name="derivative-calculations-when-multisampling"></a>Cálculos derivados com várias amostras
