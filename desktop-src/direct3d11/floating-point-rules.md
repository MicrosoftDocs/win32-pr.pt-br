...
-   [Regras de ponto flutuante de 32 bits](#32-bit-floating-point-rules)
    -   [Regras do IEEE-754 de acordo](#honored-ieee-754-rules)
...
-   [Regras de ponto flutuante de 11 bits e 10 bits](#11-bit-and-10-bit-floating-point-rules)
...
-   O IEEE-754 exibe que as operações de ponto flutuante produzam um resultado que seja o valor representável mais próximo a um resultado infinitamente preciso, conhecido como valor arredondado para o número par mais próximo. O Direct3D 11 define o mesmo requisito: operações de ponto flutuante de 32 bits produzem um resultado que está dentro de 0,5 ULP (unidade de último lugar) do resultado infinitamente preciso. Isso significa que, por exemplo, o hardware tem permissão para truncar os resultados para 32 bits em vez de executar round-to-near-even, pois isso resultaria em erro de, no máximo, 0,5 ULP. Essa regra se aplica somente à adição, subtração e multiplicação.
...
Drivers de hardware e exibição opcionalmente suportam ponto flutuante de precisão dupla. Para indicar o suporte, quando você chama [**ID3D11Device::CheckFeatureSupport**](/windows/desktop/api/D3D11/nf-d3d11-id3d11device-checkfeaturesupport) com [**D3D11 \_ FEATURE \_ DOUBLES**](/windows/desktop/api/D3D11/ne-d3d11-d3d11_feature), o driver define **DoublePrecisionFloatShaderOps** [**de D3D11 \_ FEATURE DATA \_ \_ DOUBLES**](/windows/desktop/api/D3D11/ns-d3d11-d3d11_feature_data_doubles) como TRUE. Em seguida, o driver e o hardware devem dar suporte a todas as instruções de ponto flutuante de precisão dupla.
...
-   se e == 31 e f == 0, então v = (-1)s \* infinito (infinito com assinatura)
-   se e estiver entre 0 e 31, v = (-1)s \* 2(e-15) \* (1.f)
-   se e == 0 e f != 0, v = (-1)s \* 2(e-14) \* (0.f) (números desnormalizados)
-   se e == 0 e f == 0, v = (-1)s \* 0 (com assinatura zero)
