---
title: SHV de exemplo
description: O exemplo a seguir configura um SHV (validador de integridade do sistema) em um servidor de diretiva de integridade de NAP.
...
ms.locfileid: "104453522"
---
# <a name="example-shv"></a><span data-ttu-id="cbff6-103">SHV de exemplo</span><span class="sxs-lookup"><span data-stu-id="cbff6-103">Example SHV</span></span>

> [!Note]  
> <span data-ttu-id="cbff6-104">A plataforma de proteção de acesso à rede não está disponível a partir do Windows 10</span><span class="sxs-lookup"><span data-stu-id="cbff6-104">The Network Access Protection platform is not available starting with Windows 10</span></span>

 

<span data-ttu-id="cbff6-105">O exemplo a seguir configura um SHV (validador de integridade do sistema) em um servidor de diretiva de integridade de NAP.</span><span class="sxs-lookup"><span data-stu-id="cbff6-105">The following example sets up a system health validator (SHV) on a NAP health policy server.</span></span>

> [!Note]  
> <span data-ttu-id="cbff6-106">O SDK do NAP também contém um conjunto completo de códigos de exemplo que podem ser encontrados no. \\ .. Exemplos \\ de \\ NAP NetDS... diretório da instalação do SDK.</span><span class="sxs-lookup"><span data-stu-id="cbff6-106">The NAP SDK also contains a full set of sample code that can be found in the ...\\Samples\\NetDS\\NAP... directory of your SDK installation.</span></span> <span data-ttu-id="cbff6-107">Este conjunto de exemplo inclui o e o agente de integridade do sistema (SHA), SHV e cliente de imposição (EC).</span><span class="sxs-lookup"><span data-stu-id="cbff6-107">This sample set includes and system health agent (SHA), SHV, and enforcement client (EC).</span></span> <span data-ttu-id="cbff6-108">Ele tem cenários NAP de trabalho completo Configurando a comunicação entre SHA-SHV e SHA-EC.</span><span class="sxs-lookup"><span data-stu-id="cbff6-108">It has full working NAP scenarios setting up communication between SHA-SHV and SHA-EC.</span></span>
