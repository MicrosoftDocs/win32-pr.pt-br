...
Essas constantes e valores correspondentes indicam códigos de status HTTP retornados por servidores na Internet.
...
<span id="HTTP_STATUS_WEBDAV_MULTI_STATUS"></span><span id="http_status_webdav_multi_status"></span>**\_status http \_ \_ vários \_ status do WebDAV**
</dt> <dd> <dl> <dt>
...
Durante uma World Wide Web operação de criação e controle de versão distribuída (WebDAV), isso indica vários códigos de status para uma única resposta. O corpo da resposta contém linguagem XML (XML) que descreve os códigos de status. Para obter mais informações, consulte [extensões http para criação distribuída](../webdav/webdav-portal.md).
...
O recurso solicitado foi atribuído a um novo Uniform Resource Identifier permanente (URI) e todas as referências futuras a esse recurso devem ser feitas usando um dos URIs retornados.
...
A resposta à solicitação pode ser encontrada em um URI diferente e deve ser recuperada usando um [*verbo http*](glossary.md) Get nesse recurso.
...
A solicitação redirecionada mantém o mesmo [*verbo http*](glossary.md). Comportamento de HTTP/1.1.
...
O [*verbo http*](glossary.md) usado não é permitido.
