...
A partir do Microsoft Windows XP, um mecanismo de assemblies lado a lado foi fornecido para controlar a vinculação de tempo de execução para evitar conflitos de controle de versão da DLL (biblioteca de vínculos dinâmicos). Para obter informações sobre assemblies lado a lado, consulte [sobre aplicativos isolados e assemblies](/windows/desktop/SbsCs/about-isolated-applications-and-side-by-side-assemblies)lado a lado.
