...
O comando RPC netsh pode ser usado para definir um ponto de extremidade estático para qualquer interface dinâmica ou estática. Isso é útil ao restringir o acesso a um computador que executa uma solução de firewall baseada em porta. Se a solução de firewall do Windows for usada, a interface RPC poderá ser bloqueada ou habilitada sem a necessidade de atribuí-la a uma porta específica. Para obter mais informações, consulte a [referência de comando RPC netsh firewall](/previous-versions/windows/it-pro/windows-server-2003/cc784909(v=ws.10)).
