---
description: Você pode usar a função CryptSignMessage para coassinar uma mensagem.
ms.assetid: b400436f-a71f-426a-ac8a-7fdcfa6d7575
title: 'Programa de exemplo C: coatribuindo e decodificando uma mensagem'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104296367"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="4a173-103">Programa de exemplo C: coatribuindo e decodificando uma mensagem</span><span class="sxs-lookup"><span data-stu-id="4a173-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="4a173-104">Você pode usar a função [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) para coassinar uma mensagem.</span><span class="sxs-lookup"><span data-stu-id="4a173-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="4a173-105">Isso é feito chamando [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) uma vez para assinar a mensagem original e, em seguida, chamar **CryptSignMessage** novamente para coassinar a mensagem assinada.</span><span class="sxs-lookup"><span data-stu-id="4a173-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="4a173-106">Ao verificar a assinatura de uma mensagem coassinado, use a função [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) para obter o número de assinantes da mensagem e, em seguida, chame o [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) para cada assinatura.</span><span class="sxs-lookup"><span data-stu-id="4a173-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="4a173-107">Se todas as assinaturas forem verificadas, você saberá que a mensagem coassinado é válida.</span><span class="sxs-lookup"><span data-stu-id="4a173-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="4a173-108">O exemplo a seguir mostra como assinar uma mensagem por mais de uma pessoa (entrar na mensagem), verificar todas as assinaturas e decodificar a mensagem.</span><span class="sxs-lookup"><span data-stu-id="4a173-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
