---
description: Mostra a relação entre esses parâmetros de função que apontam para estruturas ou matrizes e seus dados inicializados, com um exemplo de código.
ms.assetid: b1ad0f13-fb4d-421f-b054-a99c8ad9c83a
title: 'Exemplo de programa C: usando CryptEncryptMessage e CryptDecryptMessage'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "112410029"
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="2e001-103">Exemplo de programa C: usando CryptEncryptMessage e CryptDecryptMessage</span><span class="sxs-lookup"><span data-stu-id="2e001-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="2e001-104">[**CryptEncryptMessage é**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) a única chamada de função necessária para realizar todas as tarefas listadas em [Criptografando uma Mensagem](../secauthn/encrypting-a-message.md).</span><span class="sxs-lookup"><span data-stu-id="2e001-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="2e001-105">A inicialização de estruturas de dados é necessária.</span><span class="sxs-lookup"><span data-stu-id="2e001-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="2e001-106">A ilustração a seguir mostra a relação entre esses parâmetros de função que apontam para estruturas ou matrizes e seus dados inicializados.</span><span class="sxs-lookup"><span data-stu-id="2e001-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="2e001-107">Este exemplo também descriptografa a mensagem usando [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span><span class="sxs-lookup"><span data-stu-id="2e001-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![mapa de inicialização para uma chamada para cryptencryptmessage](images/crypenc.png)

<span data-ttu-id="2e001-109">**Para criptografar dados**</span><span class="sxs-lookup"><span data-stu-id="2e001-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="2e001-110">Obter um ponteiro para o conteúdo a ser criptografado.</span><span class="sxs-lookup"><span data-stu-id="2e001-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="2e001-111">Determine o tamanho do conteúdo a ser criptografado.</span><span class="sxs-lookup"><span data-stu-id="2e001-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="2e001-112">Adquirir um alça para um provedor criptográfico.</span><span class="sxs-lookup"><span data-stu-id="2e001-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="2e001-113">Abra um [*armazenamento de certificados*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="2e001-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="2e001-114">Obter o certificado do destinatário.</span><span class="sxs-lookup"><span data-stu-id="2e001-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="2e001-115">Crie a matriz de certificados do destinatário.</span><span class="sxs-lookup"><span data-stu-id="2e001-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="2e001-116">Inicialize a [**estrutura CRYPT \_ ALGORITHM \_ IDENTIFIER.**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier)</span><span class="sxs-lookup"><span data-stu-id="2e001-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="2e001-117">Inicialize a [**estrutura CRYPT ENCRYPT MESSAGE \_ \_ \_ PARA.**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para)</span><span class="sxs-lookup"><span data-stu-id="2e001-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="2e001-118">Chame [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) para criptografar o conteúdo e criar uma mensagem enveloada digitalmente.</span><span class="sxs-lookup"><span data-stu-id="2e001-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="2e001-119">O exemplo a seguir implementa este procedimento.</span><span class="sxs-lookup"><span data-stu-id="2e001-119">The following example implements this procedure.</span></span>

<span data-ttu-id="2e001-120">Os comentários relacionam fragmentos de código a cada etapa do procedimento.</span><span class="sxs-lookup"><span data-stu-id="2e001-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="2e001-121">Para obter mais informações sobre a função, consulte [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="2e001-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="2e001-122">Para obter mais informações sobre as estruturas de dados, consulte [**CRYPT \_ ALGORITHM \_ IDENTIFIER e**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) CRYPT ENCRYPT MESSAGE [**\_ \_ \_ PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="2e001-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="2e001-123">Este exemplo usa as funções [**MyHandleError**](myhandleerror.md) e [**ByteToStr**](bytetostr.md).</span><span class="sxs-lookup"><span data-stu-id="2e001-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="2e001-124">O código para essa função está incluído no exemplo.</span><span class="sxs-lookup"><span data-stu-id="2e001-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="2e001-125">O código para essa e outras funções auxiliares também é listado [em Uso Geral Functions](general-purpose-functions.md).</span><span class="sxs-lookup"><span data-stu-id="2e001-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
