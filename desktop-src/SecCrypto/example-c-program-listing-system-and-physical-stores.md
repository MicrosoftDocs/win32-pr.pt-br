...
---
# <a name="example-c-program-listing-system-and-physical-stores"></a>Programa de exemplo C: listando o sistema e os repositórios físicos

O exemplo a seguir enumera os locais do [*repositório de certificados*](../secgloss/c-gly.md) do sistema, os repositórios de certificados do sistema e os repositórios físicos associados a cada repositório do sistema. Este exemplo demonstra a criação de funções de retorno de chamada e funções de retorno de chamada que chamam outras funções de retorno de chamada.

Este exemplo ilustra as seguintes funções de [*CryptoAPI*](../secgloss/c-gly.md) :
...
Este exemplo também usa a função [**MyHandleError**](myhandleerror.md). O código para essa função está incluído no exemplo. O código para essa e outras funções auxiliares também está listado em [funções uso geral](general-purpose-functions.md).
