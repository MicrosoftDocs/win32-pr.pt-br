...
O exemplo a seguir demonstra um processo alternativo de codificar uma mensagem assinada, usando essa mensagem assinada como o [*conteúdo interno*](../secgloss/i-gly.md) de uma mensagem envelopada. Na preparação para a decodificação, o conteúdo interno é testado para determinar seu tipo de conteúdo interno.
...
Este exemplo também usa as funções [**MyHandleError**](myhandleerror.md) e [**GetSignerCert**](getsignercert.md). O código C para essas funções está incluído no exemplo. Para ver o código que demonstra essas e outras funções auxiliares, consulte [funções de uso geral](general-purpose-functions.md).
