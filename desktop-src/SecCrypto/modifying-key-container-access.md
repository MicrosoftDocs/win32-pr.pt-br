...
ms.locfileid: "119797136"
---
# <a name="modifying-key-container-access"></a>Modificando o acesso de contêiner de chave

O contêiner de chave padrão criado pelo CryptoAPI não permite o acesso às chaves das contas LocalService ou NetworkService. Isso pode ser corrigido programaticamente usando a função [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) para modificar o parâmetro PP-conjunto de **\_ chaves \_ \_** .

O exemplo a seguir mostra como usar a função [**CryptSetProvParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetprovparam) para modificar o PP-conjunto de contêineres **\_ \_ SEC \_** para permitir o acesso a um contêiner de chave para as contas LocalService ou NetworkService.

> [!Note]  
> O código a seguir é fornecido como uma ferramenta e só deve ser usado se for absolutamente necessário. Você só deve executar esse código uma vez em cada computador para permitir o acesso às chaves.
