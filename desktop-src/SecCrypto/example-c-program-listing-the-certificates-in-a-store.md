...
ms.locfileid: "119007594"
---
# <a name="example-c-program-listing-the-certificates-in-a-store"></a>Programa de exemplo C: listando os certificados em um repositório

O código de exemplo a seguir lista todos os certificados em um [*repositório de certificados*](../secgloss/c-gly.md) do sistema e o nome do assunto e todas as propriedades do [*contexto de certificado*](../secgloss/c-gly.md) de cada um desses certificados. O exemplo obtém o nome do repositório de certificados do usuário e, portanto, pode ser usado para listar o conteúdo de qualquer repositório de certificados do sistema. Além disso, este exemplo mostra o uso de duas novas funções de interface do usuário, uma que exibe um certificado e a outra, a interface do usuário que permite que ele selecione um certificado de uma lista de certificados em um repositório.

Este código de exemplo ilustra as seguintes tarefas e funções de [*CryptoAPI*](../secgloss/c-gly.md) :
...
-   Em um loop, enumerando todos os certificados no armazenamento aberto usando [**CertEnumCertificatesInStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatesinstore).
-   Exibindo um certificado usando [**CryptUIDlgViewContext**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgviewcontext).
-   Obtendo o nome da entidade do certificado usando [**CertGetNameString**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetnamestringa).
-   Em um loop, usando [**CertEnumCertificateContextProperties**](/windows/desktop/api/Wincrypt/nf-wincrypt-certenumcertificatecontextproperties) para obter os identificadores de propriedade de todas as propriedades associadas ao certificado.
-   Usando [**CertGetCertificateContextProperty**](/windows/desktop/api/Wincrypt/nf-wincrypt-certgetcertificatecontextproperty) para obter cada uma das propriedades.
-   Exibir uma lista de certificados em um repositório e permitir que um usuário selecione um deles usando [**CryptUIDlgSelectCertificateFromStore**](/windows/desktop/api/Cryptuiapi/nf-cryptuiapi-cryptuidlgselectcertificatefromstore).
-   Fechando o repositório de certificados usando [**CertCloseStore**](/windows/desktop/api/Wincrypt/nf-wincrypt-certclosestore).
...
O exemplo a seguir mostra a enumeração e a exibição dos certificados em um repositório. Para compilar este exemplo, você deve configurar seu compilador para usar um conjunto de caracteres de vários bytes.
