...
---
# <a name="example-c-program-duplicating-a-session-key"></a>Programa de exemplo C: duplicando uma chave de sessão

O exemplo a seguir cria uma [*chave de sessão*](../secgloss/s-gly.md)aleatória, duplica a chave, define alguns parâmetros adicionais na chave original e destrói as chaves original e duplicada. Este exemplo ilustra o uso de [**CryptDuplicateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptduplicatekey) e funções relacionadas.

Este exemplo ilustra as seguintes tarefas e funções de CryptoAPI:

-   Acessando um CSP ( [*provedor de serviços de criptografia*](../secgloss/c-gly.md) ) usando [**CryptAcquireContext**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptacquirecontexta).
-   Criando uma chave de sessão usando [**CryptGenKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgenkey).
-   Duplicando a chave criada usando [**CryptDuplicateKey**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptduplicatekey).
-   Usando [**CryptSetKeyParam**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsetkeyparam) para alterar o processo de geração de chave de duas maneiras diferentes.
-   Preenchendo um buffer com bytes aleatórios usando [**CryptGenRandom**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgenrandom).
