...
O exemplo a seguir demonstra como usar as funções [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid) e [**CertCompareIntegerBlob**](/windows/desktop/api/Wincrypt/nf-wincrypt-certcompareintegerblob) .

Primeiro, todos os OIDs disponíveis são enumerados usando [**CryptEnumOIDInfo**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptenumoidinfo). O código usado com essa função também demonstra o uso de uma função de retorno de chamada. A função de retorno de chamada demonstra a lógica de interrupção para pausar entre cada grupo de OID e depois de apresentar informações sobre um número definido de OIDs.

Segundo, as três cadeias de caracteres de OID ( [*identificador de objeto*](../secgloss/o-gly.md) ) são convertidas em números inteiros de identificador de algoritmo **DWORD** usando [**CertOIDToAlgId**](/windows/desktop/api/Wincrypt/nf-wincrypt-certoidtoalgid). O código também demonstra que todas as cadeias de caracteres de OID não têm identificadores de algoritmo relacionados.

Por fim, o exemplo demonstra a comparação de BLOBs inteiros. Este exemplo demonstra o truncamento de 0x00's à esquerda de números positivos e 0xFF's à esquerda de números negativos.

Ele também mostra que os inteiros são comparados como se estivessem armazenados no formato [*little-endian*](../secgloss/l-gly.md) com os dígitos mais significativos à direita.
