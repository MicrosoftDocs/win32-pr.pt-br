...
---
# <a name="determining-if-a-driver-can-handle-the-input-format"></a><span data-ttu-id="cf8e4-106">Determinando se um driver pode manipular o formato de entrada</span><span class="sxs-lookup"><span data-stu-id="cf8e4-106">Determining If a Driver Can Handle the Input Format</span></span>

<span data-ttu-id="cf8e4-107">O exemplo a seguir mostra como verificar o formato de entrada com a macro [**ICDrawQuery**](/windows/desktop/api/Vfw/nf-vfw-icdrawquery) .</span><span class="sxs-lookup"><span data-stu-id="cf8e4-107">The following example shows how to check the input format with the [**ICDrawQuery**](/windows/desktop/api/Vfw/nf-vfw-icdrawquery) macro.</span></span>
