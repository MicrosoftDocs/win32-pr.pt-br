...
---
# <a name="locating-and-opening-compressors-and-decompressors"></a>Localizando e abrindo compactadores e descompactadores

O exemplo a seguir usa a função [**ICLocate**](/windows/desktop/api/Vfw/nf-vfw-iclocate) para encontrar um compressor que pode compactar um bitmap de 8 bits por pixel.
...
O exemplo a seguir enumera os decompactadores no sistema para encontrar um que possa manipular o formato de suas imagens. Este exemplo usa **o \_ vídeo ICTYPE** (que é equivalente ao código de quatro caracteres "VIDC") e a macro [**ICDecompressQuery**](/windows/desktop/api/Vfw/nf-vfw-icdecompressquery) para determinar se um compressor ou descompactador dá suporte ao formato de imagem.
...
O exemplo a seguir tenta localizar um compressor específico para compactar o formato RGB de 8 bits para um formato RLE de 8 bits.
