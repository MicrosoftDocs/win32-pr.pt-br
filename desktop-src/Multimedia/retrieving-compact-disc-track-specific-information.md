...
---
# <a name="retrieving-compact-disc-track-specific-information"></a><span data-ttu-id="4ddfb-104">Recuperando informações de Track-Specific de CD</span><span class="sxs-lookup"><span data-stu-id="4ddfb-104">Retrieving Compact Disc Track-Specific Information</span></span>

<span data-ttu-id="4ddfb-105">Para dispositivos de áudio de CD, você pode obter o local inicial e o comprimento de uma faixa especificando o \_ sinalizador de faixa MCI e definindo o membro **dwTrack** de [**\_ \_ parâmetros de status MCI**](mci-status-parms.md) para o número de faixa desejado.</span><span class="sxs-lookup"><span data-stu-id="4ddfb-105">For CD audio devices, you can get the starting location and length of a track by specifying the MCI\_TRACK flag and setting the **dwTrack** member of [**MCI\_STATUS\_PARMS**](mci-status-parms.md) to the desired track number.</span></span> <span data-ttu-id="4ddfb-106">Para obter o local inicial de uma faixa, defina o membro **dwItem** para a \_ posição do status MCI \_ .</span><span class="sxs-lookup"><span data-stu-id="4ddfb-106">To get the starting location of a track, set the **dwItem** member to MCI\_STATUS\_POSITION.</span></span> <span data-ttu-id="4ddfb-107">Para obter o comprimento de uma faixa, defina **dwItem** como o \_ comprimento do status MCI \_ .</span><span class="sxs-lookup"><span data-stu-id="4ddfb-107">To get the length of a track, set **dwItem** to MCI\_STATUS\_LENGTH.</span></span> <span data-ttu-id="4ddfb-108">Por exemplo, o exemplo a seguir recupera o número total de faixas no CD e o local inicial de cada faixa. Em seguida, ele usa a função [MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox) para relatar os locais iniciais das faixas.</span><span class="sxs-lookup"><span data-stu-id="4ddfb-108">For example, the following example retrieves the total number of tracks on the compact disc and the starting location of each track. Then, it uses the [MessageBox](/windows/win32/api/winuser/nf-winuser-messagebox) function to report the starting locations of the tracks.</span></span>
