---
title: Macros MIDI
description: Macros MIDI
ms.assetid: 64b23c16-cf4e-4b62-bbfc-c81ea4131bfa
keywords:
- áudio multimídia, macros MIDI
- áudio, macros MIDI
- MIDI (interface digital de instrumento musical), macros
- MIDI (interface digital de instrumentos musicais), macros
- referência para MIDI, macros
- Referência de MIDI, macros
- Macros MIDI
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 359fbbbfff8e3d8003c72fb999c7a55105c608b6
ms.sourcegitcommit: ebd3ce6908ff865f1ef66f2fc96769be0aad82e1
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/19/2020
ms.locfileid: "103823743"
---
# <a name="midi-macros"></a>Macros MIDI

As macros a seguir são usadas com MIDI.

-   [**MEVT \_ EVENTPARM**](/windows/win32/api/mmeapi/nf-mmeapi-mevt_eventparm)
-   [**\_EVENTTYPE MEVT**](/windows/win32/api/mmeapi/nf-mmeapi-mevt_eventtype)

## <a name="related-topics"></a>Tópicos relacionados

<dl> <dt>

[Referência de MIDI](midi-reference.md)
</dt> </dl>

 

 