...
---
# <a name="getting-a-providers-metadata"></a><span data-ttu-id="8bc61-103">Obtendo metadados do provedor</span><span class="sxs-lookup"><span data-stu-id="8bc61-103">Getting a Provider's Metadata</span></span>

<span data-ttu-id="8bc61-104">Um provedor usa um manifesto de instrumentação para se identificar, definir os eventos que ele grava e outros componentes, como canais, tarefas e palavras-chave.</span><span class="sxs-lookup"><span data-stu-id="8bc61-104">A provider uses an instrumentation manifest to identify itself, define the events that it writes, and other components such as channels, tasks, and keywords.</span></span> <span data-ttu-id="8bc61-105">Para acessar essas informações em tempo de execução, chame a função [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) para obter um identificador para os metadados do editor.</span><span class="sxs-lookup"><span data-stu-id="8bc61-105">To access this information at run time, call the [**EvtOpenPublisherMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtopenpublishermetadata) function to get a handle to the publisher's metadata.</span></span>

<span data-ttu-id="8bc61-106">Os metadados são divididos entre os metadados do Publicador e os metadados do evento.</span><span class="sxs-lookup"><span data-stu-id="8bc61-106">The metadata is divided between the publisher metadata and the event metadata.</span></span> <span data-ttu-id="8bc61-107">Os metadados do Publicador contêm os metadados que identificam o Publicador e quaisquer metadados que ele define, como canais, níveis, tarefas, opcodes e palavras-chave.</span><span class="sxs-lookup"><span data-stu-id="8bc61-107">The publisher metadata contains the metadata that identifies the publisher and any metadata that it defines such as channels, levels, tasks, opcodes, and keywords.</span></span> <span data-ttu-id="8bc61-108">Para obter uma lista dos metadados do provedor que você pode recuperar, consulte a enumeração [**EVT de ID de propriedade de \_ metadados do Publicador \_ \_ \_**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) .</span><span class="sxs-lookup"><span data-stu-id="8bc61-108">For a list of the provider metadata that you can retrieve, see the [**EVT\_PUBLISHER\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_publisher_metadata_property_id) enumeration.</span></span> <span data-ttu-id="8bc61-109">Para obter os metadados, chame a função [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) .</span><span class="sxs-lookup"><span data-stu-id="8bc61-109">To get the metadata, call the [**EvtGetPublisherMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetpublishermetadataproperty) function.</span></span> <span data-ttu-id="8bc61-110">Essa função retorna os valores de propriedade para as propriedades que identificam o provedor e os identificadores para os componentes canais, níveis, tarefas, opcodes e palavras-chave.</span><span class="sxs-lookup"><span data-stu-id="8bc61-110">This function returns the property values for the properties that identify the provider and handles to the channels, levels, tasks, opcodes, and keywords components.</span></span> <span data-ttu-id="8bc61-111">Para acessar os metadados para esses componentes, chame a função [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) .</span><span class="sxs-lookup"><span data-stu-id="8bc61-111">To access the metadata for these components, call the [**EvtGetObjectArrayProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgetobjectarrayproperty) function.</span></span>

<span data-ttu-id="8bc61-112">Para obter os metadados do evento, chame a função [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) para obter uma lista dos eventos que o provedor define.</span><span class="sxs-lookup"><span data-stu-id="8bc61-112">To get the event metadata, call the [**EvtOpenEventMetadataEnum**](/windows/desktop/api/WinEvt/nf-winevt-evtopeneventmetadataenum) function to get a list of the events that the provider defines.</span></span> <span data-ttu-id="8bc61-113">Em seguida, chame a função [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) em um loop para enumerar os eventos.</span><span class="sxs-lookup"><span data-stu-id="8bc61-113">Then call the [**EvtNextEventMetadata**](/windows/desktop/api/WinEvt/nf-winevt-evtnexteventmetadata) function in a loop to enumerate the events.</span></span> <span data-ttu-id="8bc61-114">Para obter uma lista de metadados que você pode obter para um evento, consulte a enumeração de [**ID de propriedade de \_ metadados de evento \_ \_ \_ EVT**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) .</span><span class="sxs-lookup"><span data-stu-id="8bc61-114">For a list of metadata that you can get for an event, see the [**EVT\_EVENT\_METADATA\_PROPERTY\_ID**](/windows/desktop/api/WinEvt/ne-winevt-evt_event_metadata_property_id) enumeration.</span></span> <span data-ttu-id="8bc61-115">Para obter os metadados, chame a função [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) .</span><span class="sxs-lookup"><span data-stu-id="8bc61-115">To get the metadata, call the [**EvtGetEventMetadataProperty**](/windows/desktop/api/WinEvt/nf-winevt-evtgeteventmetadataproperty) function.</span></span> <span data-ttu-id="8bc61-116">Essas propriedades retornam um valor inteiro para cada elemento da definição de evento.</span><span class="sxs-lookup"><span data-stu-id="8bc61-116">These properties return an integer value for each element of the event definition.</span></span> <span data-ttu-id="8bc61-117">Para a ID e a versão do evento, a exibição do valor inteiro é bem, mas para os outros elementos, como canal e nível, a exibição do valor inteiro pode não ter nenhum significado para o usuário.</span><span class="sxs-lookup"><span data-stu-id="8bc61-117">For event ID and version, displaying the integer value is fine but for the other elements, such as channel and level, displaying the integer value may not have any meaning to the user.</span></span> <span data-ttu-id="8bc61-118">O valor inteiro desses elementos é o valor do atributo Value.</span><span class="sxs-lookup"><span data-stu-id="8bc61-118">The integer value for these elements is the value of the value attribute.</span></span> <span data-ttu-id="8bc61-119">Você pode usar o valor inteiro para mapear para os metadados do provedor e obter a cadeia de caracteres da mensagem ou o valor do nome do elemento e exibi-lo para o usuário em vez disso.</span><span class="sxs-lookup"><span data-stu-id="8bc61-119">You can use the integer value to map into the provider's metadata and get the message string or name value for the element and display it to the user instead.</span></span>

<span data-ttu-id="8bc61-120">Para obter um exemplo que mostra como enumerar os provedores registrados no computador, consulte [enumerando provedores registrados](#enumerating-registered-providers).</span><span class="sxs-lookup"><span data-stu-id="8bc61-120">For an example that shows how to enumerate the registered providers on the computer, see [Enumerating registered providers](#enumerating-registered-providers).</span></span>

<span data-ttu-id="8bc61-121">Para obter um exemplo que mostra como obter os metadados de um provedor registrado, consulte [obtendo os metadados de um provedor](#getting-a-providers-metadata).</span><span class="sxs-lookup"><span data-stu-id="8bc61-121">For an example that shows how to get the metadata for a registered provider, see [Getting the metadata for a provider](#getting-a-providers-metadata).</span></span>

## <a name="enumerating-registered-providers"></a><span data-ttu-id="8bc61-122">Enumerando provedores registrados</span><span class="sxs-lookup"><span data-stu-id="8bc61-122">Enumerating registered providers</span></span>

<span data-ttu-id="8bc61-123">O exemplo a seguir mostra como enumerar os provedores que estão registrados no computador.</span><span class="sxs-lookup"><span data-stu-id="8bc61-123">The following example shows how to enumerate the providers that are registered on the computer.</span></span>
...
## <a name="getting-the-metadata-for-a-provider"></a><span data-ttu-id="8bc61-124">Obtendo os metadados para um provedor</span><span class="sxs-lookup"><span data-stu-id="8bc61-124">Getting the metadata for a provider</span></span>

<span data-ttu-id="8bc61-125">O exemplo a seguir mostra como obter os metadados para o provedor e os eventos que ele define.</span><span class="sxs-lookup"><span data-stu-id="8bc61-125">The following example shows how to get the metadata for the provider and the events that it defines.</span></span>
