...
Para assinar eventos, chame a função [**EvtSubscribe**](/windows/desktop/api/WinEvt/nf-winevt-evtsubscribe) . Você pode assinar eventos de um ou mais canais de administrador ou operacionais. O canal pode existir no computador local ou em um computador remoto. Para especificar os eventos que você deseja assinar, você pode usar uma consulta XPath ou uma consulta XML de estrutura. Para obter detalhes sobre como escrever a consulta, consulte [consumindo eventos](consuming-events.md).

O log de eventos do Windows fornece dois modelos de assinatura de evento:

-   O modelo de push envia eventos de forma assíncrona a um retorno de chamada que você implementa.
-   O modelo de pull usa um identificador de evento que você cria para sinalizar quando há eventos disponíveis que correspondem aos seus critérios de consulta. Em seguida, você enumera os eventos no conjunto de resultados.

Você pode assinar eventos anteriores e futuros, somente eventos futuros ou eventos anteriores e futuros começando após o evento de indicador.

Para obter detalhes sobre como assinar eventos, consulte [assinaturas push](#push-subscriptions) ou [assinaturas pull](#pull-subscriptions).

Para obter detalhes sobre como renderizar os eventos, consulte [renderizando eventos](rendering-events.md).

Se você quiser assinar eventos de onde parou, crie um indicador antes de encerrar sua assinatura e use-o na próxima vez que assinar eventos. Para obter detalhes, consulte [eventos de indicador](bookmarking-events.md).

## <a name="push-subscriptions"></a>Assinaturas push

O exemplo a seguir mostra como assinar eventos, implementar o retorno de chamada de assinatura e renderizar os eventos.
...
## <a name="pull-subscriptions"></a>Assinaturas pull

O modelo de assinatura pull usa um objeto de evento (consulte a função [**CreateEventEx**](/windows/desktop/api/synchapi/nf-synchapi-createeventexa) ) para sinalizar ao aplicativo que há eventos no conjunto de resultados que correspondem aos critérios de consulta. Crie uma construção de loop que aguarde no objeto de evento até que o evento seja sinalizado. Em seguida, chame a função [**EvtNext**](/windows/desktop/api/WinEvt/nf-winevt-evtnext) em um loop para enumerar os eventos no conjunto de resultados. Quando a função **EvtNext** falha e define o último erro para \_ não mais nenhum \_ \_ Item, redefina o objeto de evento e aguarde o serviço sinalizar o objeto novamente quando houver eventos no conjunto de resultados.
