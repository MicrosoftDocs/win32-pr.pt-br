...
Você pode criar uma barra de status usando a função [**CreateStatusWindow**](/windows/desktop/api/Commctrl/nf-commctrl-createstatuswindowa) ou usando a função [**CreateWindowEx**](/windows/desktop/api/winuser/nf-winuser-createwindowexa) e especificando a classe da janela [**STATUSCLASSNAME**](common-control-window-classes.md) .

Depois de criar a barra de status, você pode dividi-la em partes, definir o texto para cada parte e controlar a aparência da janela usando mensagens da barra de status.
...
### <a name="create-a-status-bar"></a>Criar uma barra de status

O exemplo a seguir demonstra como criar uma barra de status que tem uma alça de dimensionamento e dividir a janela em quatro partes iguais com base na largura da área do cliente da janela pai.
