---
title: Sobre controles de calendário de mês
description: Um controle de calendário mensal implementa uma interface do usuário como calendário.
...
ms.locfileid: "119834506"
---
# <a name="about-month-calendar-controls"></a>Sobre controles de calendário de mês

Um controle de calendário mensal implementa uma interface do usuário como calendário. Isso fornece ao usuário um método muito intuitivo e reconhecível de inserir ou selecionar uma data. O controle também fornece ao aplicativo os meios para obter e definir as informações de data no controle usando tipos de dados existentes.

-   [Recursos de controle de calendário mensal](#month-calendar-control-features)
    -   [Selecionando um dia](#selecting-a-day)
    -   [Selecionando um mês não aadjacente](#selecting-a-nonadjacent-month)
    -   [Selecionando um ano diferente](#selecting-a-different-year)
-   [Localização](#localization)
-   [Horas no controle de calendário mensal](#times-in-the-month-calendar-control)
...
> [!Note]  
> A aparência e o comportamento do controle de calendário do mês diferem ligeiramente em versões diferentes da biblioteca em tempo de executar. Este tópico se concentra no controle como ele aparece no Windows Vista com a versão 6 do Comctl32.dll.
...
-   O "círculo hoje" (na verdade, um retângulo nesta versão) aparece ao redor do dia atual e ao lado da linha "Hoje" como uma indicação visual. Este é o estilo padrão.
-   Os números de semana são mostrados à esquerda de cada linha de dias. Esse estilo deve ser especificado.
...
> [!Note]
>
> Windows dá suporte a datas anteriores à 1601. Consulte [**FILETIME para**](/windows/desktop/api/minwinbase/ns-minwinbase-filetime) obter detalhes.
>
...
| PAGE UP (VK \_ PRIOR)     | Selecione o mesmo dia no mês anterior. (Se esse mês não tiver o dia, o dia mais próximo será selecionado; por exemplo, a seleção se move de 31 de março para 28 ou 29 de fevereiro.)                                                      |
| PAGE DOWN (VK \_ NEXT)    | Selecione o mesmo dia no próximo mês.                                                                                                                                                                                                   |
| HOME (VK \_ HOME)         | Selecione o primeiro dia do mês atual.                                                                                                                                                                                               |
| END (VK \_ END)           | Selecione o último dia do mês atual.                                                                                                                                                                                                |
| CTRL + HOME             | Role um mês para trás e selecione um dia na coluna mais à esquerda.                                                                                                                                                                       |
| CTRL + FIM              | Role um mês para frente e selecione um dia na coluna mais à direita.                                                                                                                                                                       |
| CTRL + PÁGINA PARA CIMA          | Selecione o mesmo dia em um mês anterior. O número de meses pelos quais a seleção se move é o número de meses exibidos no controle . Por exemplo, se dois meses são exibidos, a seleção se moveria de 6 de junho para 6 de maio.    |
| CTRL + PÁGINA PARA BAIXO        | Selecione o mesmo dia em um mês anterior. O número de meses pelos quais a seleção se move é o número de meses exibidos no controle . Por exemplo, se dois meses são exibidos, a seleção se moveria de 6 de junho para 6 de agosto. |
...
O controle de calendário do mês não exibe a hora. No entanto, [**a estrutura SYSTEMTIME**](/windows/desktop/api/minwinbase/ns-minwinbase-systemtime) usada para definir e recuperar a data selecionada ou a data de hoje contém campos de hora. Quando uma data é definida programaticamente, o controle copia os campos de tempo como são ou os valida primeiro e, em seguida, se eles são inválidos, armazena os horários padrão atuais. Veja a seguir uma lista das mensagens que configuram uma data e uma descrição de como os campos de hora são tratados.



|  Mensagem         |  Descrição            |
|---------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**MCM \_ SETCURSEL**](mcm-setcursel.md)     | O controle copia os campos de tempo como estão, sem validação ou modificação.                                                                                                                                        |
| [**MCM \_ SETRANGE**](mcm-setrange.md)       | Os campos de tempo das estruturas passadas são validados. Se eles são válidos, os campos de tempo são copiados sem modificação. Se eles são inválidos, o controle copia os campos de tempo dos dados de hoje.                  |
| [**MCM \_ SETSELRANGE**](mcm-setselrange.md) | Os campos de tempo das estruturas passadas são validados. Se eles são válidos, os campos de tempo são copiados sem modificação. Se eles são inválidos, o controle retém os campos de tempo dos intervalos de seleção atuais. |
