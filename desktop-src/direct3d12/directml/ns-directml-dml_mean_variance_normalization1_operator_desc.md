...
---
# <a name="dml_mean_variance_normalization1_operator_desc-structure-directmlh"></a>DML_MEAN_VARIANCE_NORMALIZATION1_OPERATOR_DESC estrutura (directml.h)
...
**DML_MEAN_VARIANCE_NORMALIZATION1_OPERATOR_DESC** é um superconjunto da funcionalidade [DML_MEAN_VARIANCE_NORMALIZATION_OPERATOR_DESC](/windows/win32/api/directml/ns-directml-dml_mean_variance_normalization_operator_desc). Aqui, definir a matriz **Eixos** como é o equivalente à definição de CrossChannel como FALSE no DML_MEAN_VARIANCE_NORMALIZATION_OPERATOR_DESC ; enquanto definir a matriz Eixos como é equivalente à definição de `{ 0, 2, 3 }`     `{ 1, 2, 3 }` *CrossChannel* como **TRUE.**
