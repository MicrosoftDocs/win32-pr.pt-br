...
- **DML_INTERPOLATION_MODE_LINEAR**. Usa o algoritmo *Quadrilinear* , que computa o elemento Output, fazendo a média ponderada dos 2 elementos de entrada vizinhos mais próximos por dimensão. Como todas as quatro dimensões podem ser reampladas, a média ponderada é calculada em um total de 16 elementos de entrada para cada elemento de saída.
...
O número de valores nas matrizes para os pontos *Scales*, *InputPixelOffsets* e *OutputPixelOffsets.* Esse valor deve corresponder à contagem de dimensões *de InputTensor* e *OutputTensor*, que deve ser 4.


`Scales`

Tipo: \_ Tamanho do campo \_ \_ (DimensionCount) **const [FLOAT](../../winprog/windows-data-types.md) \***

As escalas a aplicar ao reamplar a entrada, em que o dimensiona > 1 escala a imagem e escala < 1 para baixo da imagem para essa dimensão. Observe que as escalas não precisam ser exatamente `OutputSize / InputSize` . Se a entrada após o dimensionamento for maior que o limite de saída, nós a cortaremos para o tamanho da saída. Por outro lado, se a entrada após o dimensionamento for menor que o limite de saída, as bordas de saída serão fixadas.
...
Os deslocamentos a aplicar aos pixels de entrada antes de resampling. Quando esse valor é , o canto superior esquerdo do pixel é usado em vez de seu centro, o que geralmente não `0` dará o resultado esperado. Para resampler [a](/windows/win32/api/directml/ns-directml-dml_resample_operator_desc)imagem usando o centro dos pixels e obter o mesmo comportamento que DML_RESAMPLE_OPERATOR_DESC , esse valor deve ser `0.5` .
...
Os deslocamentos a aplicar aos pixels de saída após a resampling. Quando esse valor é , o canto superior esquerdo do pixel é usado em vez de seu centro, o que geralmente não `0` dará o resultado esperado. Para resampler [a](/windows/win32/api/directml/ns-directml-dml_resample_operator_desc)imagem usando o centro dos pixels e obter o mesmo comportamento que DML_RESAMPLE_OPERATOR_DESC , esse valor deve ser `-0.5` .
