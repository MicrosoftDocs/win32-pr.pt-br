...
Desenvolva aplicativos de área de trabalho mais seguros usando serviços e APIs do Windows. Essas APIs fornecem:
...
| [CFG (Proteção de Fluxo de Controle) ](./secbp/control-flow-guard.md) | A proteção de fluxo de controle (CFG) é um recurso de segurança de plataforma altamente otimizado que foi criado para combater vulnerabilidades de corrupção de memória. |
...
| [Host do protocolo de autenticação extensível](./eaphost/about-eap-host.md) | O EAPHost é um componente de rede do Microsoft Windows que fornece uma infraestrutura EAP (Extensible Authentication Protocol) para a autenticação de implementações de protocolo "suplicante", como 802.1 X e ponto a ponto (PPP). |
