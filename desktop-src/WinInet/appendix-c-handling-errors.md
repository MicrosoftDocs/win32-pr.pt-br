...
---
# <a name="handling-errors-windows-internet"></a><span data-ttu-id="a5ee5-103">Tratamento de erros (Internet do Windows)</span><span class="sxs-lookup"><span data-stu-id="a5ee5-103">Handling Errors (Windows Internet)</span></span>

<span data-ttu-id="a5ee5-104">A função [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) recupera o último código de erro para todas as funções do Wininet.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-104">The [**GetLastError**](/windows/desktop/api/errhandlingapi/nf-errhandlingapi-getlasterror) function retrieves the last error code for all of the WinINet functions.</span></span> <span data-ttu-id="a5ee5-105">Se o erro [**\_ \_ estendido da \_ Internet**](wininet-errors.md) for retornado, haverá uma cadeia de caracteres ou um buffer que contém uma mensagem de erro detalhada.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-105">If [**ERROR\_INTERNET\_EXTENDED\_ERROR**](wininet-errors.md) is returned, there is a string or buffer that contains a detailed error message.</span></span> <span data-ttu-id="a5ee5-106">Chame a função [**InternetGetLastResponseInfo**](/windows/desktop/api/Wininet/nf-wininet-internetgetlastresponseinfoa) para recuperar o texto de erro estendido.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-106">Call the [**InternetGetLastResponseInfo**](/windows/desktop/api/Wininet/nf-wininet-internetgetlastresponseinfoa) function to retrieve the extended error text.</span></span>

<span data-ttu-id="a5ee5-107">Para obter o texto de erro de um erro, chame a função [**FormatMessage**](/windows/desktop/api/winbase/nf-winbase-formatmessage) , passando um identificador **HMODULE** para Wininet.dll, que pode ser obtido usando a função [GetModuleHandle](/windows/desktop/api/libloaderapi/nf-libloaderapi-getmodulehandlea) .</span><span class="sxs-lookup"><span data-stu-id="a5ee5-107">To get the error text for an error, call the [**FormatMessage**](/windows/desktop/api/winbase/nf-winbase-formatmessage) function, passing it an **HMODULE** handle to Wininet.dll, which can be obtained using the [GetModuleHandle](/windows/desktop/api/libloaderapi/nf-libloaderapi-getmodulehandlea) function.</span></span>

<span data-ttu-id="a5ee5-108">Veja a seguir um exemplo de uma função de tratamento de erros.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-108">The following is an example of an error-handling function.</span></span>
...
> [!Note]  
> <span data-ttu-id="a5ee5-109">O WinINet não oferece suporte a implementações de servidor.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-109">WinINet does not support server implementations.</span></span> <span data-ttu-id="a5ee5-110">Além disso, ele não deve ser usado de um serviço.</span><span class="sxs-lookup"><span data-stu-id="a5ee5-110">In addition, it should not be used from a service.</span></span> <span data-ttu-id="a5ee5-111">Para implementações de servidor ou serviços, use [o Microsoft Windows http Services (WinHTTP)](/windows/desktop/WinHttp/winhttp-start-page).</span><span class="sxs-lookup"><span data-stu-id="a5ee5-111">For server implementations or services use [Microsoft Windows HTTP Services (WinHTTP)](/windows/desktop/WinHttp/winhttp-start-page).</span></span>
