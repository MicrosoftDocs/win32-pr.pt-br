...
- **instalação do Windows Update ou do autoupdate**. Windows a atualização (anteriormente conhecida como atualização automática) baixa e instala automaticamente Windows atualizações. Além disso, ele fornece uma maneira fácil para os usuários baixarem e instalarem atualizações manualmente. A restauração do sistema cria um ponto de restauração antes que uma atualização seja instalada, seja automaticamente ou manual.
- **Operação de restauração do sistema**. A restauração do sistema cria automaticamente um ponto de restauração como um backup antes de qualquer operação de restauração ser iniciada. por exemplo, suponha que um usuário restaura acidentalmente Windows para um ponto de restauração incorreto. para desfazer essa restauração, o usuário pode restaurar Windows para um ponto de restauração anterior ao primeiro ponto de restauração. depois que Windows tiver sido restaurado para seu estado inicial, o usuário poderá repetir o processo, desta vez selecionando o ponto de restauração correto.
...
## <a name="known-issue-you-cannot-restore-the-system-to-a-restore-point-after-you-install-a-windows-10-update"></a>problema conhecido: não é possível restaurar o sistema para um ponto de restauração depois de instalar uma atualização de Windows 10

Considere o cenário a seguir.

1. você instala Windows 10 em um computador limpo.
...
Nesse cenário, o sistema não é restaurado para o ponto de restauração "R1". Em vez disso, o computador passa por um erro de parada (0xC000021A). você reinicia o computador, mas o sistema não pode retornar para a área de trabalho do Windows.

### <a name="cause"></a>Causa

Esse é um problema conhecido no Windows 10.

durante o processo de restauração do sistema, Windows prepara temporariamente a restauração de arquivos que estão em uso. Em seguida, ele salva as informações no registro. Quando o computador é reiniciado, ele conclui a operação em etapas.

nessa situação, Windows restaura os arquivos de catálogo e prepara os arquivos de driver (.sys) a serem restaurados quando o computador for reiniciado. no entanto, quando o computador é reiniciado, Windows carrega os drivers existentes antes de restaurar as versões posteriores dos drivers. Como as versões do driver não correspondem às versões dos arquivos de catálogo restaurados, o processo de reinicialização é interrompido.

### <a name="workaround"></a>Solução alternativa

**Para se recuperar da reinicialização com falha e continuar o processo de restauração**

depois que a falha ocorrer, reinicie o computador até ele entrar no ambiente de recuperação de Windows (WinRE). Para fazer isso, talvez seja necessário usar uma opção de reinicialização de hardware, e talvez seja necessário reiniciar várias vezes.

no ambiente de recuperação de Windows:
...
1. Deixe o processo de inicialização continuar. quando Windows reinicia, o processo de restauração do sistema deve ser retomado e concluído.
...
1. no ambiente de recuperação Windows, selecione **solucionar problemas** de  >  **opções avançadas**  >  **restauração do sistema** e, em seguida, selecione **desfazer restauração do sistema**.

Essas etapas retornam o computador para o estado em que estava antes de você iniciar o processo de restauração.

**para restaurar Windows para um ponto de restauração usando WinRE**

para iniciar o assistente de restauração do sistema em um computador afetado, use WinRE em vez da caixa de diálogo **Configurações** . Para fazer isso, execute estas etapas:

1. selecione **iniciar**  >  **Configurações**  >  **atualizar &**  >  **recuperação** de segurança.
...
- [Windows Ambiente de recuperação (Windows RE)](/windows-hardware/manufacture/desktop/windows-recovery-environment--windows-re--technical-reference)
- [Inicie seu computador no modo de segurança no Windows 10](https://support.microsoft.com/help/12376) 