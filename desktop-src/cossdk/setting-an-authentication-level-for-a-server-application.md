---
description: Ao definir um nível de autenticação para um aplicativo, você determina qual grau de autenticação é executado quando os clientes chamam o aplicativo.
...
Ao definir um nível de autenticação para um aplicativo, você determina qual grau de autenticação é executado quando os clientes chamam o aplicativo. Níveis de autenticação mais altos fornecem maior segurança e integridade de dados, embora geralmente com alguma degradação de desempenho. Para obter mais informações, consulte [Autenticação de cliente](client-authentication.md).

**Para selecionar um nível de autenticação para um aplicativo de servidor**

1.  Clique com o botão direito do mouse no aplicativo COM+ para o qual você está definindo a autenticação e clique em **Propriedades**.

2.  Na caixa de diálogo propriedades do aplicativo, clique na **guia** Segurança.

3.  Na caixa **Nível de autenticação para chamadas,** selecione o nível apropriado. Os níveis são os a seguir, ordenados da segurança mais baixa para a mais alta:

    -   **None**. Nenhuma autenticação ocorre.
    -   **Conexão**. Autentica as credenciais somente quando a conexão é feita.
