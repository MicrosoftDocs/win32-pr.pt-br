---
description: Este programa de exemplo funciona com o programa de servidor usando SSPI com um servidor do Windows Sockets.
ms.assetid: 7ec15770-d3bd-4488-abe8-058115fec071
title: Usando SSPI com um cliente do Windows Sockets
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104506087"
---
# <a name="using-sspi-with-a-windows-sockets-client"></a><span data-ttu-id="7691a-103">Usando SSPI com um cliente do Windows Sockets</span><span class="sxs-lookup"><span data-stu-id="7691a-103">Using SSPI with a Windows Sockets Client</span></span>

<span data-ttu-id="7691a-104">Este programa de exemplo funciona com o programa de servidor [usando SSPI com um servidor do Windows Sockets](using-sspi-with-a-windows-sockets-server.md).</span><span class="sxs-lookup"><span data-stu-id="7691a-104">This sample program works with the server program [Using SSPI with a Windows Sockets Server](using-sspi-with-a-windows-sockets-server.md).</span></span> <span data-ttu-id="7691a-105">Os programas de exemplo de cliente e servidor são projetados para funcionar juntos.</span><span class="sxs-lookup"><span data-stu-id="7691a-105">The client and server sample programs are designed to work together.</span></span> <span data-ttu-id="7691a-106">Ambos os programas usam o arquivo de cabeçalho SspiExample. h, que pode ser encontrado no [arquivo de cabeçalho para exemplos de cliente e servidor SSPI](header-file-for-sspi-client-and-server.md).</span><span class="sxs-lookup"><span data-stu-id="7691a-106">Both programs use the header file SspiExample.h, which can be found in [Header File for SSPI Client and Server Samples](header-file-for-sspi-client-and-server.md).</span></span> <span data-ttu-id="7691a-107">Este programa inclui chamadas para funções em Secur32. lib e Ws2 \_ 32. lib, que devem ser incluídas entre as bibliotecas de links.</span><span class="sxs-lookup"><span data-stu-id="7691a-107">This program includes calls to functions in Secur32.lib and Ws2\_32.lib, which must be included among the link libraries.</span></span>

<span data-ttu-id="7691a-108">Este programa demonstra o seguinte:</span><span class="sxs-lookup"><span data-stu-id="7691a-108">This program demonstrates the following:</span></span>

-   <span data-ttu-id="7691a-109">Estabelecendo uma conexão do Windows Sockets com um servidor.</span><span class="sxs-lookup"><span data-stu-id="7691a-109">Establishing a Windows Sockets connection with a server.</span></span>
-   <span data-ttu-id="7691a-110">Inicializando uma sessão SSPI autenticada com o Negotiate SSP.</span><span class="sxs-lookup"><span data-stu-id="7691a-110">Initializing an authenticated SSPI session with the Negotiate SSP.</span></span>
-   <span data-ttu-id="7691a-111">Conectar-se a um servidor e estabelecer uma sessão de comunicação segura.</span><span class="sxs-lookup"><span data-stu-id="7691a-111">Connecting with a server and establishing a secure communication session.</span></span>
-   <span data-ttu-id="7691a-112">Receber e descriptografar uma mensagem do servidor na sessão segura.</span><span class="sxs-lookup"><span data-stu-id="7691a-112">Receiving and decrypt a message from the server within the secure session.</span></span>

<span data-ttu-id="7691a-113">Este programa de exemplo usa tratamento de erro limitado.</span><span class="sxs-lookup"><span data-stu-id="7691a-113">This sample program uses limited error handling.</span></span>
