...
A função **falha AcquireCredentialsHandle (NTLM)** adquire um identificador para credenciais preexistentes de uma [*entidade de segurança*](../secgloss/s-gly.md). Esse identificador é exigido pelas funções [**InitializeSecurityContext (NTLM)**](initializesecuritycontext--ntlm.md) e [**AcceptSecurityContext (NTLM)**](acceptsecuritycontext--ntlm.md) . Elas podem ser *credenciais* preexistentes, que são estabelecidas por meio de um logon do sistema que não está descrito aqui, ou o chamador pode fornecer credenciais alternativas.
...
Ao usar o pacote NTLM, os comprimentos máximos de caracteres para o nome de usuário, a senha e o domínio são 256, 256 e 15, respectivamente.
