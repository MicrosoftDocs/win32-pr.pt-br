...
-   A interface [**IDispatch**](/windows/win32/api/oaidl/nn-oaidl-idispatch) . Essa é a interface de automação para linguagens como Visual Basic Scripting Edition, que não usam Associação de tempo de compilação. Relacionado a esses são os dados de biblioteca de tipos que um provedor deve fornecer. Para obter mais informações, consulte [problemas de implementação para provedores ADSI](implementation-issues-for-adsi-providers.md).
