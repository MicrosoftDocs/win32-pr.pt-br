...
---
# <a name="using-iadsgetinfoex-for-range-retrieval"></a><span data-ttu-id="60261-106">Usando IADs:: GetInfoEx para recuperação de intervalo</span><span class="sxs-lookup"><span data-stu-id="60261-106">Using IADs::GetInfoEx for Range Retrieval</span></span>

<span data-ttu-id="60261-107">O método [**IADs. GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) pode ser usado para recuperar um intervalo de valores de atributo.</span><span class="sxs-lookup"><span data-stu-id="60261-107">The [**IADs.GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) method can be used to retrieve a range of attribute values.</span></span> <span data-ttu-id="60261-108">O intervalo de valores a serem recuperados é especificado na matriz de nomes de atributo passada para o método.</span><span class="sxs-lookup"><span data-stu-id="60261-108">The range of values to retrieve is specified in the attribute name array passed to the method.</span></span>

<span data-ttu-id="60261-109">Os especificadores de intervalo para uma consulta de propriedade exigem o seguinte formato:</span><span class="sxs-lookup"><span data-stu-id="60261-109">The range specifiers for a property query require the following form:</span></span>
...
<span data-ttu-id="60261-110">em que " &lt; nome da propriedade &gt; " é o **ldapDisplayName** do atributo, " &lt; intervalo baixo &gt; " é o índice de base zero do primeiro valor de atributo a ser recuperado e " &lt; intervalo alto &gt; " é o índice de base zero do último valor de atributo a ser recuperado.</span><span class="sxs-lookup"><span data-stu-id="60261-110">where "&lt;property name&gt;" is the **ldapDisplayName** of the attribute, "&lt;low range&gt;" is the zero-based index of the first attribute value to retrieve and "&lt;high range&gt;" is the zero-based index of the last attribute value to retrieve.</span></span> <span data-ttu-id="60261-111">Zero é usado para " &lt; intervalo baixo &gt; " para especificar a primeira entrada.</span><span class="sxs-lookup"><span data-stu-id="60261-111">Zero is used for "&lt;low range&gt;" to specify the first entry.</span></span> <span data-ttu-id="60261-112">O caractere curinga ( \* ) pode ser usado para " &lt; intervalo alto &gt; " para especificar todas as entradas restantes.</span><span class="sxs-lookup"><span data-stu-id="60261-112">The wildcard character (\*) can be used for "&lt;high range&gt;" to specify all remaining entries.</span></span>

<span data-ttu-id="60261-113">O exemplo de código a seguir contém uma função que mostra como usar a recuperação de intervalo com [**IADs:: GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) para enumerar os membros de um grupo.</span><span class="sxs-lookup"><span data-stu-id="60261-113">The following code example contains a function that shows how to use range retrieval with [**IADs::GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) to enumerate the members of a group.</span></span>
...
<span data-ttu-id="60261-114">O exemplo de código a seguir contém uma função que mostra como usar a recuperação de intervalo com [**IADs. GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) para enumerar os membros de um grupo.</span><span class="sxs-lookup"><span data-stu-id="60261-114">The following code example contains a function that shows how to use range retrieval with [**IADs.GetInfoEx**](/windows/desktop/api/Iads/nf-iads-iads-getinfoex) to enumerate the members of a group.</span></span>
