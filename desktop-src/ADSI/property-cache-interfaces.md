---
title: Interfaces de cache de propriedade
description: Este tópico lista as interfaces ADSI que funcionam com caches de propriedade.
ms.assetid: b3479719-b5bf-4f19-91f9-b05e60bde161
ms.tgt_platform: multiple
keywords:
- interfaces de cache de propriedade ADSI
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: b43c84b739d3af1188a85222146d08a03c27f3a6dbe98a17f5c83f82e240366b
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "119509876"
---
# <a name="property-cache-interfaces"></a>Interfaces de cache de propriedade

As interfaces de cache de propriedade são projetadas para trabalhar com propriedades no cache. Eles consistem nas seguintes interfaces:

-   [**IADsPropertyEntry**](/windows/desktop/api/Iads/nn-iads-iadspropertyentry)
-   [**IADsPropertyList**](/windows/desktop/api/Iads/nn-iads-iadspropertylist)
-   [**IADsPropertyValue**](/windows/desktop/api/Iads/nn-iads-iadspropertyvalue)
-   [**IADsPropertyValue2**](/windows/desktop/api/Iads/nn-iads-iadspropertyvalue2)

 

 




