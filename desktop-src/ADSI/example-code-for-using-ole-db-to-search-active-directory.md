---
title: Código de exemplo para usar OLE DB para pesquisar Active Directory
description: O exemplo de código a seguir mostra como Pesquisar Active Directory usando C++, COM e OLE DB.
ms.assetid: 71812a4b-5e27-4389-b949-3d96c87b222d
ms.tgt_platform: multiple
keywords:
- Código de exemplo para usar OLE DB para pesquisar Active Directory ADSI
- ADSI ADSI, código de exemplo C/C++, usando OLE DB para acessar Active Directory
...
---
# <a name="example-code-for-using-ole-db-to-search-active-directory"></a><span data-ttu-id="c27ff-106">Código de exemplo para usar OLE DB para pesquisar Active Directory</span><span class="sxs-lookup"><span data-stu-id="c27ff-106">Example Code for Using OLE DB to Search Active Directory</span></span>

<span data-ttu-id="c27ff-107">O exemplo de código a seguir mostra como Pesquisar Active Directory usando C++, COM e OLE DB.</span><span class="sxs-lookup"><span data-stu-id="c27ff-107">The following code example shows how to search Active Directory using C++, COM, and OLE DB.</span></span> <span data-ttu-id="c27ff-108">Este é um exemplo de uma função que usa o nome distinto do contêiner para pesquisar em e as credenciais a serem usadas para a pesquisa.</span><span class="sxs-lookup"><span data-stu-id="c27ff-108">This is an example of a function that takes the distinguished name of the container to search under and the credentials to use for the search.</span></span> <span data-ttu-id="c27ff-109">O exemplo executará uma pesquisa de subárvore no contêiner especificado para todos os objetos que têm um **objectClass** de "user".</span><span class="sxs-lookup"><span data-stu-id="c27ff-109">The example will perform a subtree search under the specified container for all objects that have an **objectClass** of "user".</span></span> <span data-ttu-id="c27ff-110">O exemplo imprimirá os atributos **Name** e **ADsPath** de cada usuário na janela do console.</span><span class="sxs-lookup"><span data-stu-id="c27ff-110">The example will print the **name** and **ADsPath** attributes of each user to the console window.</span></span>

<span data-ttu-id="c27ff-111">Há dois dialetos de consulta que podem ser usados com o provedor de OLE DB ADSI, LDAP e SQL.</span><span class="sxs-lookup"><span data-stu-id="c27ff-111">There are two query dialects that can be used with the ADSI OLE DB provider, LDAP and SQL.</span></span> <span data-ttu-id="c27ff-112">O dialeto é especificado no método [ICommandText:: SetCommandText](/previous-versions/windows/desktop/ms709757(v=vs.85)) com um dos seguintes GUIDs:</span><span class="sxs-lookup"><span data-stu-id="c27ff-112">The dialect is specified in the [ICommandText::SetCommandText](/previous-versions/windows/desktop/ms709757(v=vs.85)) method with one of the following GUIDs:</span></span>

-   <span data-ttu-id="c27ff-113">**Dbguid \_ SQL** para o DIALETO SQL.</span><span class="sxs-lookup"><span data-stu-id="c27ff-113">**DBGUID\_SQL** for the SQL dialect.</span></span>
-   <span data-ttu-id="c27ff-114">**Dbguid \_ LDAPDialect** para o DIALETO LDAP.</span><span class="sxs-lookup"><span data-stu-id="c27ff-114">**DBGUID\_LDAPDialect** for the LDAP dialect.</span></span>

<span data-ttu-id="c27ff-115">Este exemplo usa o dialeto LDAP.</span><span class="sxs-lookup"><span data-stu-id="c27ff-115">This example uses the LDAP dialect.</span></span>

<span data-ttu-id="c27ff-116">O método [ICommandText:: SetCommandText](/previous-versions/windows/desktop/ms709757(v=vs.85)) também pode aceitar o **GUID \_ padrão dbguid** para o dialeto.</span><span class="sxs-lookup"><span data-stu-id="c27ff-116">The [ICommandText::SetCommandText](/previous-versions/windows/desktop/ms709757(v=vs.85)) method can also accept the **DBGUID\_DEFAULT** GUID for the dialect.</span></span> <span data-ttu-id="c27ff-117">Nesse caso, a ADSI tentará usar o dialeto SQL primeiro; Se isso falhar, a ADSI tentará usar o dialeto LDAP.</span><span class="sxs-lookup"><span data-stu-id="c27ff-117">In this case, ADSI will attempt to use the SQL dialect first; if that fails, ADSI will attempt to use the LDAP dialect.</span></span> <span data-ttu-id="c27ff-118">Para obter mais informações, consulte dialeto e [dialeto de SQL](sql-dialect.md)do [LDAP](ldap-dialect.md) .</span><span class="sxs-lookup"><span data-stu-id="c27ff-118">For more information, see [LDAP dialect](ldap-dialect.md) and [SQL dialect](sql-dialect.md).</span></span>

<span data-ttu-id="c27ff-119">Para obter mais informações sobre OLE DB, consulte o guia do programador de OLE DB.</span><span class="sxs-lookup"><span data-stu-id="c27ff-119">For more information about OLE DB, see the OLE DB Programmer's Guide.</span></span>
