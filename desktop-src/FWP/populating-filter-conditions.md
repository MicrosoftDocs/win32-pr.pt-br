---
title: Populando condições de filtro
description: O código de exemplo a seguir demonstra como preencher condições de filtro usadas por um aplicativo de servidor para localizar filtros e eventos que o afetam.
...
ms.locfileid: "104085022"
---
# <a name="populating-filter-conditions"></a><span data-ttu-id="c9cae-103">Populando condições de filtro</span><span class="sxs-lookup"><span data-stu-id="c9cae-103">Populating Filter Conditions</span></span>

<span data-ttu-id="c9cae-104">O código de exemplo a seguir demonstra como preencher condições de filtro usadas por um aplicativo de servidor para localizar filtros e eventos que o afetam.</span><span class="sxs-lookup"><span data-stu-id="c9cae-104">The following sample code demonstrates how to populate filter conditions used by a server application to find filters and events that affect it.</span></span>

> [!Note]  
> <span data-ttu-id="c9cae-105">Essas condições são as mesmas que as suportadas pela API **IsPortAllowed** de nível inferior.</span><span class="sxs-lookup"><span data-stu-id="c9cae-105">These conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
