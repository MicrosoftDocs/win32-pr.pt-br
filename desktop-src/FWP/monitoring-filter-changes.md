---
title: Monitorando alterações de filtro
description: O código de exemplo a seguir demonstra como monitorar a adição e a exclusão de filtros que podem afetar um aplicativo de servidor.
...
---
# <a name="monitoring-filter-changes"></a><span data-ttu-id="d9496-103">Monitorando alterações de filtro</span><span class="sxs-lookup"><span data-stu-id="d9496-103">Monitoring Filter Changes</span></span>

<span data-ttu-id="d9496-104">O código de exemplo a seguir demonstra como monitorar a adição e a exclusão de filtros que podem afetar um aplicativo de servidor.</span><span class="sxs-lookup"><span data-stu-id="d9496-104">The following sample code demonstrates how to monitor the addition and deletion of filters that might affect a server application.</span></span>

> [!Note]  
> <span data-ttu-id="d9496-105">As condições de filtro são as mesmas que as suportadas pela API **IsPortAllowed** de nível inferior.</span><span class="sxs-lookup"><span data-stu-id="d9496-105">The filter conditions are the same as those supported by the downlevel **IsPortAllowed** API.</span></span>
