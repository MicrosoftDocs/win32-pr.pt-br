...
Os valores em *param definem* um plano de referência em coordenadas de olho. A matriz modelview aplicada a elas não pode ser a mesma em vigor quando os vértices de polígono são transformados. Essa função estabelece um campo de coordenadas de textura que pode produzir linhas de contorno dinâmicos em objetos móveis.
