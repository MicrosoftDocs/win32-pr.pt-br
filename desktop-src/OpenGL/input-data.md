...
-   **Vértices**. Os vértices descrevem a forma do objeto geométrico desejado. Para especificar vértices, use as funções [**\* glVertex**](glvertex-functions.md) em conjunto com [**glBegin**](glbegin.md) e [**glEnd**](glend.md) para criar um ponto, uma linha ou um polígono. Você também pode usar [**glRect**](glrect-functions.md) para descrever um retângulo inteiro ao mesmo tempo.
...
-   **Cor atual**. A cor de um vértice, junto com as condições de iluminação, determinam a cor final e definitiva. A cor é especificada [**com \* glColor**](glcolor-functions.md) se estiver no modo RGBA ou [**com \* glIndex**](glindex-functions.md) se estiver no modo de índice de cor.
...
> [!Note]  
> Quando **glVertex \*** é chamado, o vértice resultante herda o sinalizador de borda atual, as coordenadas normal, Color e Texture. Portanto, **glEdgeFlag \***, **glNormal \***, **glColor \*** e **\* glTexCoord** devem ser chamados antes de **glVertex \***, se eles forem afetar o vértice resultante.
