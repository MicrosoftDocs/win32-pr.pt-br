---
title: Referência de primitivas
description: Referência de primitivas
ms.assetid: a88d38f1-eeb9-484d-a987-aba545084330
keywords:
- Pipeline de processamento de OpenGL, referência de primitivas
- referência de primitivas OpenGL
- Referência de primitiva de OpenGL
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 77c81727995b44afec170c5d3dfe34fe42a4fdc8
ms.sourcegitcommit: 2d531328b6ed82d4ad971a45a5131b430c5866f7
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 09/16/2019
ms.locfileid: "104292758"
---
# <a name="primitives-reference"></a>Referência de primitivas

-   [**glBitmap**](glbitmap.md)
-   [**glClipPlane**](glclipplane.md)
-   [**glCullFace**](glcullface.md)
-   [**glDepthRange**](gldepthrange.md)
-   [**glDrawPixels**](gldrawpixels.md)
-   [**glFog\***](glfog.md)
-   [**glFrontFace**](glfrontface.md)
-   [**glFrustum**](glfrustum.md)
-   [**glLineStipple**](gllinestipple.md)
-   [**glLineWidth**](gllinewidth.md)
-   [**glOrtho**](glortho.md)
-   [**glPixelMap\***](glpixelmap.md)
-   [**glPixelStore\***](glpixelstore-functions.md)
-   [**glPixelTransfer\***](glpixeltransfer.md)
-   [**glPixelZoom**](glpixelzoom.md)
-   [**glPointSize**](glpointsize.md)
-   [**glPolygonMode**](glpolygonmode.md)
-   [**glPolygonStipple**](glpolygonstipple.md)
-   [**glRasterPos\***](glrasterpos-functions.md)
-   [**glTexEnv\***](gltexenv-functions.md)
-   [**glTexImage1D**](glteximage1d.md)
-   [**glTexImage2D**](glteximage2d.md)
-   [**glTexParameter\***](gltexparameter-functions.md)
-   [**glViewport**](glviewport.md)

 

 




