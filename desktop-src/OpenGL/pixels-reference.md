---
title: Referência de pixels
description: Referência de pixels
ms.assetid: 3a705c20-2ae8-429c-82e2-2e91c70508c9
keywords:
- Pipeline de processamento OpenGL, referência de pixels
- OpenGL, referência de pixels
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: fda1ff27815af225b6947a3628ea3cd62caf35aa
ms.sourcegitcommit: 2d531328b6ed82d4ad971a45a5131b430c5866f7
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 09/16/2019
ms.locfileid: "103637137"
---
# <a name="pixels-reference"></a>Referência de pixels

-   [**glAccum**](glaccum.md)
-   [**glClear**](glclear.md)
-   [**glClearAccum**](glclearaccum.md)
-   [**glClearColor**](glclearcolor.md)
-   [**glClearDepth**](glcleardepth.md)
-   [**glClearIndex**](glclearindex.md)
-   [**glClearStencil**](glclearstencil.md)
-   [**glColorMask**](glcolormask.md)
-   [**glCopyPixels**](glcopypixels.md)
-   [**glDepthMask**](gldepthmask.md)
-   [**glDrawBuffer**](gldrawbuffer.md)
-   [**glIndexMask**](glindexmask.md)
-   [**glReadBuffer**](glreadbuffer.md)
-   [**glReadPixels**](glreadpixels.md)
-   [**glStencilMask**](glstencilmask.md)

 

 




