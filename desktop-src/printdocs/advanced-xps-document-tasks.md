---
description: Os tópicos nesta seção demonstram como executar tarefas mais avançadas com as interfaces de documento XPS.
ms.assetid: 70a48c58-4443-4899-9af1-3ef41973f012
title: Tarefas avançadas de programação de documentos XPS
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 5ff32e76457f2b4587e11ccdd893659302f6484c90262d440f0b8bb46e7f5784
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "117868609"
---
# <a name="advanced-xps-document-programming-tasks"></a>Tarefas avançadas de programação de documentos XPS

Os tópicos nesta seção demonstram como executar tarefas mais avançadas com as interfaces de documento XPS.

## <a name="tasks"></a>Tarefas

<dl>

[Mesclar documentos XPS](merging-xps-documents.md)  
[Processamento de documentos XPS em um filtro XPSDrv](processing-xps-documents-in-an-xpsdrv-filter.md)  
</dl>

## <a name="related-topics"></a>Tópicos relacionados

<dl> <dt>

[Assinaturas digitais do XPS OM](using-the-xps-digital-signatures.md)
</dt> <dt>

[Especificação de Papel XML](https://www.ecma-international.org/activities/XML%20Paper%20Specification/XPS%20Standard%20WD%201.6.pdf)
</dt> </dl>

 

 



