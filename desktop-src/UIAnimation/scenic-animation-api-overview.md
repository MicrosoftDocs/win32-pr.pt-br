...
A WINDOWS de Animação do Windows é uma API baseada em COM de thread único que fornece os seguintes recursos para desenvolvedores:
...
Para ver a referência completa da API, consulte [Referência Windows animação.](windows-animation-reference.md) Para código de exemplo, consulte [Tarefas Windows animação](using-windows-animation.md) e [exemplos Windows animação.](windows-animation-samples.md)
...
O diagrama a seguir mostra as interações entre um aplicativo e os componentes de animação Windows quando o aplicativo está impulsionando atualizações de animação diretamente.
