...
O Windows Sockets 2 permite que um aplicativo use mais de um provedor de serviços, mas a função [**Select**](/windows/desktop/api/Winsock2/nf-winsock2-select) é limitada a um conjunto de soquetes associados a um único provedor de serviços. Isso não, de forma alguma, impedir que um aplicativo tenha vários soquetes abertos por meio de vários provedores.
...
-   Usar a função [**WSAAsyncSelect**](/windows/desktop/api/winsock/nf-winsock-wsaasyncselect) quando operações de não bloqueio são empregadas e o aplicativo já está usando uma bomba de mensagem do Windows.
