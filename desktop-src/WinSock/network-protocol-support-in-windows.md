...
Windows Os soquetes consideram cada conjunto de protocolos de rede como uma família de endereços exclusiva. Portanto, o protocolo IPv6 é considerado a família de endereços **\_ INET6 de AF** e o protocolo IPv4 é considerado a família de endereços **\_ inet** da série AF. Os protocolos IPv6 e IPv4 oferecem suporte ao uso de vários protocolos IP em camadas, como TCP, UDP e ICMP.

Windows Os soquetes foram projetados inicialmente para adicionar suporte para IPv4 a Windows. no entanto, a interface de programação de soquetes de Windows foi criada a partir do início com a capacidade de dar suporte a outros conjuntos de protocolo de rede. ao longo do tempo, as versões do Windows e os soquetes de Windows associados incluíram suporte nativo para outros conjuntos de protocolo de rede (IPX/SPX e AppleTalk, por exemplo). o suporte para outros protocolos de rede também estava disponível para versões do Windows como software de terceiros dos fornecedores.
...
a tabela a seguir identifica o suporte nativo Windows para conjuntos de protocolo de rede comuns. 
...
| Bluetooth (consulte as observações)<br/> | Com suporte<br/>     | Com suporte<br/>     | Com suporte<br/>     | Com suporte<br/>                 | Com suporte<br/>                 | Sem suporte<br/>             |
...
**NetBIOS:** O protocolo NetBIOS é normalmente usado pelos serviços de nomenclatura no Windows. O NetBIOS pode usar vários conjuntos de protocolo de rede, incluindo IP (NetBIOS sobre TCP/IP), IPX/SPX e NetBEUI. o Winsock dá suporte a NetBIOS sobre TCP/IP (normalmente chamada de NetBT) somente nas versões de 32 bits do Windows 7, Windows Server 2008 e Windows Vista. o Winsock dá suporte a netbios sobre TCP/IP e netbios usando IPX no Windows Server 2003 e Windows XP. o Winsock dá suporte a netbios sobre TCP/IP, netbios usando IPX e NetBIOS usando NetBEUI no Windows 2000.
...
**Bluetooth:** o suporte a Winsock para Bluetooth como um conjunto de protocolos de rede inclui os perfis de PAN (rede pessoal Bluetooth) e DUN (rede de conexão discada). Bluetooth suporte no Windows também inclui o uso do Bluetooth dispositivos de interface humana (HID) e outros perfis para conectar-se a teclados, dispositivos apontadores e outros dispositivos de entrada que não estão relacionados a protocolos de rede.

**DLC em Windows 2003 e Windows XP:** o protocolo dlc (controle de vínculo de dados) tem suporte no Windows Server 2003 e no Windows XP quando o driver dlc incluído com o Microsoft Host Integration Server 2006, Host Integration Server 2004 ou Host Integration Server 2000 está instalado.

**ATM no Windows 2003, Windows XP e Windows 2000:** o protocolo ATM (modo de transferência assíncrona) tem suporte no Windows Server 2003, Windows XP e Windows 2000 quando um adaptador de rede ATM é instalado. O protocolo para o IP clássico sobre ATM (às vezes abreviado como CLIP/ATM) é definido em [RFC 2225](https://tools.ietf.org/html/rfc2225) e documentos relacionados publicados pela IETF. Windows o servidor 2003, Windows XP e Windows 2000 fornecem uma implementação completa desse padrão.

**NetBEUI no Windows 2000:** o protocolo NetBEUI não tem suporte direto dos soquetes de Windows. mas o protocolo NetBIOS que pode usar vários protocolos de rede dá suporte ao uso do protocolo NetBEUI no Windows 2000.
