...
Esta seção contém um exemplo de código-fonte que demonstra como implementar as funções **GetXbyY** usando as novas funções RnR independentes de protocolo. Um desenvolvedor deve implementar essas funções como um ponto de partida. Para atender à especificação do Windows Sockets, muitas outras funções são necessárias.
