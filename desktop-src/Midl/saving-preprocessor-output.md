...
Para uma compilação de DCOM, vários arquivos de pré-processador são geralmente produzidos, mesmo que sejam processados pelo MIDL como um fluxo de entrada contíguo virtual. em um ambiente típico de programação de Windows, os arquivos temporários podem ser encontrados no diretório Temp como nomes de arquivos, como MID \* . tmp. Se estiver preservando a saída do pré-processador, é uma prática recomendada observar os arquivos recentes no diretório Temp para identificar quais arquivos estão associados ao pré-processador executado.
