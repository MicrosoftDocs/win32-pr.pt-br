...
O sistema define os tamanhos de conjunto de trabalho padrão. Você também pode modificar os tamanhos de conjunto de trabalho usando a função [**SetProcessWorkingSetSize**](/windows/desktop/api/memoryapi/nf-memoryapi-setprocessworkingsetsize) . Definir esses valores não é uma garantia de que a memória será reservada ou residente. Tenha cuidado ao solicitar um tamanho de conjunto de trabalho mínimo ou máximo, pois isso pode prejudicar o desempenho do sistema.
