---
description: O exemplo nesta seção ilustra como usar as funções de retângulo. Ele consiste no procedimento de janela principal de um aplicativo que permite que o usuário mova e dimensione um bitmap.
ms.assetid: e8861240-9345-43e6-820d-d237247d1bd7
title: Usando retângulos (GDI do Windows)
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103922699"
---
# <a name="using-rectangles-windows-gdi"></a><span data-ttu-id="ae2e5-104">Usando retângulos (GDI do Windows)</span><span class="sxs-lookup"><span data-stu-id="ae2e5-104">Using Rectangles (Windows GDI)</span></span>

<span data-ttu-id="ae2e5-105">O exemplo nesta seção ilustra como usar as funções de retângulo.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-105">The example in this section illustrates how to use the rectangle functions.</span></span> <span data-ttu-id="ae2e5-106">Ele consiste no procedimento de janela principal de um aplicativo que permite que o usuário mova e dimensione um bitmap.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-106">It consists of the main window procedure from an application that enables the user to move and size a bitmap.</span></span>

<span data-ttu-id="ae2e5-107">Quando o aplicativo é iniciado, ele desenha um bitmap de 32 pixels por 32 pixels no canto superior esquerdo da tela.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-107">When the application starts, it draws a 32-pixel by 32-pixel bitmap in the upper left corner of the screen.</span></span> <span data-ttu-id="ae2e5-108">O usuário pode mover o bitmap arrastando-o.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-108">The user can move the bitmap by dragging it.</span></span> <span data-ttu-id="ae2e5-109">Para dimensionar o bitmap, o usuário cria um retângulo de destino arrastando o mouse e, em seguida, arrasta o bitmap e "descarta" no retângulo de destino.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-109">To size the bitmap, the user creates a target rectangle by dragging the mouse, then drags the bitmap and "drops" it on the target rectangle.</span></span> <span data-ttu-id="ae2e5-110">O aplicativo responde copiando o bitmap no retângulo de destino.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-110">The application responds by copying the bitmap into the target rectangle.</span></span>

<span data-ttu-id="ae2e5-111">O procedimento de janela que permite que o usuário mova e dimensione o bitmap é fornecido no exemplo a seguir.</span><span class="sxs-lookup"><span data-stu-id="ae2e5-111">The window procedure that allows the user to move and size the bitmap is given in the following example.</span></span>
