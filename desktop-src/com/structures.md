---
title: Estruturas (COM)
ms.assetid: 07f65855-f529-4d57-81fe-150167a03901
description: 'Saiba mais sobre: estruturas (COM)'
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 638d1f4421a5fe70ec8a4ab1fd499f9a6f3c4f7d2883b5e7638edf4687bca8e0
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "119129648"
---
# <a name="structures-com"></a>Estruturas (COM)

As seguintes estruturas são fornecidas pelo COM:

-   [**\_acesso ACTRL**](/windows/win32/api/accctrl/ns-accctrl-actrl_accessa)
-   [**\_entrada de acesso ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_access_entrya)
-   [**\_lista de \_ entradas de acesso ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_access_entry_lista)
-   [**\_entrada da propriedade ACTRL \_**](/windows/desktop/api/AccCtrl/ns-accctrl-actrl_property_entrya)
-   [**ASSOCIAR as \_ optas**](/windows/win32/api/objidl/ns-objidl-bind_opts)
-   [**ASSOCIAR \_ OPTS2**](/windows/win32/api/objidl/ns-objidl-bind_opts2~r1)
-   [**ASSOCIAR \_ OPTS3**](/windows/win32/api/objidl/ns-objidl-bind_opts3~r1)
-   [**CALLFRAME \_ MARSHALCONTEXT**](/windows/win32/api/callobj/ns-callobj-callframe_marshalcontext)
-   [**CALLFRAMEINFO**](/windows/win32/api/callobj/ns-callobj-callframeinfo)
-   [**CALLFRAMEPARAMINFO**](/windows/win32/api/callobj/ns-callobj-callframeparaminfo)
-   [**CATEGORYINFO**](/windows/win32/api/comcat/ns-comcat-categoryinfo)
-   [**COAUTHIDENTITY**](/windows/desktop/api/wtypesbase/ns-wtypesbase-coauthidentity)
-   [**COAUTHINFO**](/windows/desktop/api/wtypesbase/ns-wtypesbase-coauthinfo)
-   [**CONNECTDATA**](/windows/win32/api/ocidl/ns-ocidl-connectdata)
-   [**COSERVERINFO**](/windows/win32/api/objidlbase/ns-objidlbase-coserverinfo)
-   [**CSPLATFORM**](/windows/win32/api/wtypes/ns-wtypes-csplatform)
-   [**INTERFACEINFO**](/windows/win32/api/objidl/ns-objidl-interfaceinfo)
-   [**MÚLTIPLOS \_ QI**](/windows/win32/api/objidlbase/ns-objidlbase-multi_qi)
-   [**ORPC \_ DBG \_ All**](orpc-dbg-all.md)
-   [**\_buffer de DBG ORPC \_**](orpc-dbg-all.md)
-   [**\_argumentos de inicialização ORPC \_**](orpc-init-args.md)
-   [**QUERYCONTEXT**](/windows/win32/api/wtypes/ns-wtypes-querycontext)
-   [**RPCOLEMESSAGE**](/windows/win32/api/objidlbase/ns-objidlbase-rpcolemessage)
-   [**\_informações de autenticação exclusiva \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_info)
-   [**\_lista de autenticação exclusiva \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_list)
-   [**\_serviço de autenticação exclusivo \_**](/windows/win32/api/objidlbase/ns-objidlbase-sole_authentication_service)
-   [**SOleTlsData**](/previous-versions/windows/desktop/legacy/ms690269(v=vs.85))

 

 
