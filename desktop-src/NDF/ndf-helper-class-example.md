...
description: Este exemplo ilustra como implementar funções de diagnóstico e reparo de NDF.
...
ms.locfileid: "119802106"
---
# <a name="ndf-helper-class-extension"></a>Extensão de classe auxiliar NDF

Este exemplo ilustra como implementar funções de diagnóstico e reparo de NDF. Neste exemplo, um arquivo de configuração crítico deve existir para que o sistema permaneça tivo. Da mesma forma, o problema é determinar se o arquivo existe. Caso não faça isso, o sistema não está alhático e o problema é diagnosticada pelo NDF. O reparo é para criar o arquivo ausente.
...
Se [**LowHealth retornar**](/windows/desktop/api/ndhelper/nf-ndhelper-inetdiaghelper-lowhealth) **DS \_ CONFIRM,** [**GetRepairInfo**](/windows/desktop/api/ndhelper/nf-ndhelper-inetdiaghelper-getrepairinfo) será implementado para determinar a ação de reparo apropriada. Neste exemplo, isso significa a criação do arquivo.
...
O usuário é apresentado com as opções de correção retornadas pelo [**GetRepairInfo**](/windows/desktop/api/ndhelper/nf-ndhelper-inetdiaghelper-getrepairinfo), a menos que haja apenas uma opção de reparo, caso em que ele é executado automaticamente. O [**método Repair**](/windows/desktop/api/ndhelper/nf-ndhelper-inetdiaghelper-repair) é chamado com a estrutura [**RepairInfo**](/windows/win32/api/ndattrib/ns-ndattrib-repairinfo) aplicável para que o arquivo de configuração possa ser restaurado.
