...
Este tópico descreve as constantes nomeadas que identificam as propriedades dos elementos de automação da interface do usuário da Microsoft.
...
<td style="text-align: left;">Identifica a propriedade <strong>AcceleratorKey</strong> , que é uma cadeia de caracteres que contém as combinações da tecla aceleradora (também chamada de tecla de atalho) para o elemento Automation. <br/> As combinações de teclas de atalho invocam uma ação. Por exemplo, CTRL + O é geralmente usado para invocar a caixa de diálogo comum abrir arquivo. Um elemento Automation que tem a propriedade <strong>AcceleratorKey</strong> pode implementar o padrão de controle <a href="uiauto-implementinginvoke.md">Invoke</a> para a ação que é equivalente ao comando de atalho.<br/> Tipo de variante: <strong>VT_BSTR</strong><br/> Valor padrão: cadeia de caracteres vazia<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>AnnotationObjects</strong> , que é uma lista de objetos de anotação em um documento, como comentário, cabeçalho, rodapé e assim por diante.<br/> Tipo de variante: <strong>VT_I4</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: matriz vazia<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>AnnotationTypes</strong> , que é uma lista dos tipos de anotações em um documento, como comentário, cabeçalho, rodapé e assim por diante.<br/> Tipo de variante: <strong>VT_I4</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: matriz vazia<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>AriaProperties</strong> , que é uma cadeia de caracteres formatada que contém as informações de Propriedade do Aria (aplicativo de Internet avançado) acessíveis para o elemento de automação. Para obter mais informações sobre como mapear Estados e propriedades do ARIA para propriedades e funções de automação da interface do usuário, consulte <a href="uiauto-ariaspecification.md">automação da interface do usuário para especificação de aplicativos de Internet avançados acessíveis pelo W3C</a><br/> <strong>AriaProperties</strong> é uma coleção de pares de nome/valor com delimitadores de <strong>=</strong> (Equals) e <strong>;</strong> (ponto e vírgula), por exemplo, &quot; marcado = true; Disabled = false &quot; . O <strong>\</strong> (barra invertida) é usado como um caractere de escape quando esses caracteres delimitadores ou <strong>\</strong> aparecem nos valores. Por outros motivos de segurança, a implementação do provedor dessa propriedade pode tomar medidas para validar as propriedades originais do ARIA; no entanto, isso não é necessário.<br/> Tipo de variante: <strong>VT_BSTR</strong><br/> Valor padrão: cadeia de caracteres vazia<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>AriaRole</strong> , que é uma cadeia de caracteres que contém as informações de função do Aria (aplicativo de Internet avançado) acessíveis para o elemento Automation. Para obter mais informações sobre como mapear funções do ARIA para tipos de controle de automação da interface do usuário, consulte <a href="uiauto-ariaspecification.md">automação da interface do usuário para especificação de aplicativos de Internet avançados acessíveis</a><br/>
<blockquote>
[!Note]<br />
Como opção, o agente do usuário também pode oferecer uma descrição localizada da função de ARIA do W3C na propriedade <strong>LocalizedControlType</strong> . Quando a cadeia de caracteres localizada não for especificada, o sistema fornecerá a cadeia de caracteres <strong>LocalizedControlType</strong> padrão para o elemento.
</blockquote>
...
<td style="text-align: left;">Identifica a propriedade <strong>AutomationId</strong> , que é uma cadeia de caracteres que contém o identificador de automação da interface do usuário (ID) do elemento de automação.<br/> Quando estiver disponível, o <strong>AutomationId</strong> de um elemento deve ser o mesmo em qualquer instância do aplicativo, independentemente do idioma local. O valor deve ser exclusivo entre os elementos irmãos, mas não necessariamente exclusivo em toda a área de trabalho. Por exemplo, várias instâncias de um aplicativo, ou várias exibições de pasta no Microsoft Windows Explorer, podem conter elementos com a mesma propriedade <strong>AutomationId</strong> , como &quot; SystemMenuBar &quot; .<br/> Embora o suporte para <strong>AutomationId</strong> seja sempre recomendado para um melhor suporte de teste automatizado, essa propriedade não é obrigatória. Onde há suporte, o <strong>AutomationId</strong> é útil para criar um script de automação de teste que é executado independentemente do idioma da interface do usuário. Os clientes não devem fazer suposições sobre os valores de <strong>AutomationId</strong> expostos por outros aplicativos. Não há garantia de que o <strong>AutomationId</strong> seja estável em diferentes versões ou compilações de um aplicativo.<br/> Tipo de variante: <strong>VT_BSTR</strong><br/> Valor padrão: cadeia de caracteres vazia<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>BoundingRectangle</strong> , que especifica as coordenadas do retângulo que inclui completamente o elemento Automation. O retângulo é expresso em coordenadas de tela físicas. Ele pode conter pontos que não podem ser clicados se a região de forma ou clicável do item da interface do usuário for irregular ou se o item estiver obscurecido por outros elementos da interface do usuário.<br/> Tipo de variante: <strong>VT_R8</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: [0, 0, 0,0]<br/>
<blockquote>
[!Note]<br />
Essa propriedade será <strong>nula</strong> se o item não estiver exibindo uma interface do usuário no momento.
</blockquote>
<br/> <br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>CenterPoint</strong> , que especifica as coordenadas de ponto X e Y do elemento Automation. O espaço de coordenadas é o que o provedor considera logicamente uma página.<br/> Tipo de variante: <strong>VT_R8</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: <strong>VT_EMPTY</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>ClassName</strong> , que é uma cadeia de caracteres que contém o nome da classe do elemento Automation, conforme atribuído pelo desenvolvedor do controle.<br/> O nome da classe depende da implementação do provedor de automação da interface do usuário e, portanto, nem sempre está em um formato padrão. No entanto, se o nome da classe for conhecido, ele poderá ser usado para verificar se um aplicativo está funcionando com o elemento de automação esperado.<br/> Tipo de variante: <strong>VT_BSTR</strong><br/> Valor padrão: cadeia de caracteres vazia<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>ClickablePoint</strong> , que é um ponto no elemento Automation que pode ser clicado. Um elemento não poderá ser clicado se for completamente ou parcialmente obscurecido por outra janela.<br/> Tipo de variante: <strong>VT_R8</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: <strong>VT_EMPTY</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>ControllerFor</strong> , que é uma matriz de elementos de automação que são manipulados pelo elemento Automation que dá suporte a essa propriedade.<br/> <strong>ControllerFor</strong> é usado quando um elemento de automação afeta um ou mais segmentos da interface do usuário do aplicativo ou da área de trabalho; caso contrário, é difícil associar o impacto da operação de controle aos elementos da interface do usuário.<br/> Esse identificador é comumente usado para <a href="/windows/uwp/design/accessibility/accessible-text-requirements">a sugestão de acessibilidade automática</a>.<br/> Tipo de variante para provedores: <strong>VT_UNKNOWN</strong>  |  <strong>VT_ARRAY</strong><br/> Tipo de variante para clientes: <strong>VT_UNKNOWN</strong> (<a href="/windows/desktop/api/UIAutomationClient/nn-uiautomationclient-iuiautomationelementarray"><strong>IUIAutomationElementArray</strong></a> )<br/> Valor padrão: matriz vazia<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>ControlType</strong> , que é uma classe que identifica o tipo do elemento Automation. <strong>ControlType</strong> define características dos elementos da interface do usuário, bem como primitivos de controle da interface do usuário, como botão ou caixa de seleção. <br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: <a href="uiauto-controltype-ids.md"> <strong>UIA_CustomControlTypeId</strong></a><br/>
<blockquote>
[!Note]<br />
Use o valor padrão somente se o elemento Automation representa um tipo de controle completamente novo.
</blockquote>
<br/> <br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>FillType</strong> , que especifica o padrão usado para preencher o elemento Automation, como nenhum, cor, gradiente, imagem, padrão e assim por diante.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>FlowsFrom</strong> , que é uma matriz de elementos de automação que sugere a ordem de leitura antes do elemento de automação atual. Com suporte a partir do Windows 8.<br/> A propriedade <strong>FlowsFrom</strong> especifica a ordem de leitura quando os elementos de automação não são expostos ou estruturados na mesma ordem de leitura, conforme percebido pelo usuário. Embora a propriedade <strong>FlowsFrom</strong> possa especificar vários elementos anteriores, ela normalmente contém apenas o elemento anterior na ordem de leitura.<br/> Tipo de variante para provedores: <strong>VT_UNKNOWN</strong>  |  <strong>VT_ARRAY</strong><br/> Tipo de variante para clientes: <strong>VT_UNKNOWN</strong> (<a href="/windows/desktop/api/UIAutomationClient/nn-uiautomationclient-iuiautomationelementarray"><strong>IUIAutomationElementArray</strong></a>)<br/> Valor padrão: matriz vazia<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a <strong>Propriedade</strong> flowto, que é uma matriz de elementos de automação que sugere a ordem de leitura após o elemento de automação atual.<br/> A <strong>Propriedade flowto especifica a ordem</strong> de leitura quando os elementos de automação não são expostos ou estruturados na mesma ordem de leitura, conforme percebido pelo usuário. Embora a <strong>Propriedade</strong> flowto possa especificar vários elementos bem-sucedidos, ela normalmente contém apenas o próximo elemento na ordem de leitura.<br/> Tipo de variante para provedores: <strong>VT_UNKNOWN</strong>  |  <strong>VT_ARRAY</strong><br/> Tipo de variante para clientes: <strong>VT_UNKNOWN</strong> (<a href="/windows/desktop/api/UIAutomationClient/nn-uiautomationclient-iuiautomationelementarray"><strong>IUIAutomationElementArray</strong></a>)<br/> Valor padrão: matriz vazia<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>HasKeyboardFocus</strong> , que é um valor booliano que indica se o elemento de automação tem o foco do teclado.<br/> Tipo de variante: <strong>VT_BOOL</strong><br/> Valor padrão: <strong>false</strong><br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>HeadingLevel</strong> , que indica o nível de título de um elemento de automação da interface do usuário.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: <strong>HeadingLevel_None</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>IsKeyboardFocusable</strong> , que é um valor booliano que indica se o elemento de automação pode aceitar o foco do teclado.<br/> Tipo de variante: <strong>VT_BOOL</strong><br/> Valor padrão: <strong>false</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>LiveSetting</strong> , que é suportada por um elemento Automation que representa uma região dinâmica. A propriedade <strong>LiveSetting</strong> indica o &quot; nível de educador &quot; que um cliente deve usar para notificar o usuário sobre as alterações na região dinâmica. Essa propriedade pode ser um dos valores da enumeração <a href="/windows/desktop/api/uiautomationcore/ne-uiautomationcore-livesetting"><strong>LiveSetting</strong></a> . Com suporte a partir do Windows 8.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>NativeWindowHandle</strong> , que é um inteiro que representa o identificador (<strong>HWND</strong>) da janela do elemento Automation, se existir; caso contrário, essa propriedade será 0.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>OptimizeForVisualContent</strong> , que é um valor booliano que indica se o provedor expõe apenas elementos visíveis. Um provedor pode usar essa propriedade para otimizar o desempenho ao trabalhar com partes muito grandes de conteúdo. Por exemplo, como as páginas do usuário por meio de um grande conteúdo, o provedor pode destruir elementos de conteúdo que não são mais visíveis. Quando um elemento de conteúdo é destruído, o provedor deve retornar o <strong>UIA_E_ELEMENTNOTAVAILABLE</strong> código de erro. Com suporte a partir do Windows 8.<br/> Tipo de variante: <strong>VT_BOOL</strong><br/> Valor padrão: <strong>false</strong><br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>Orientation</strong> , que indica a orientação do controle representado pelo elemento Automation. A propriedade é expressa como um valor do tipo enumerado <a href="/windows/desktop/api/UIAutomationCore/ne-uiautomationcore-orientationtype"><strong>OrientationType</strong></a> .<br/> A propriedade <strong>Orientation</strong> tem suporte pelos controles, como barras de rolagem e controles deslizantes, que podem ter uma orientação vertical ou horizontal. Caso contrário, ele pode ser sempre <a href="/windows/desktop/api/UIAutomationCore/ne-uiautomationcore-orientationtype"><strong>OrientationType_None</strong></a>, o que significa que o controle não tem orientação.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0 (<a href="/windows/desktop/api/UIAutomationCore/ne-uiautomationcore-orientationtype"><strong>OrientationType_None</strong></a>)<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>OutlineColor</strong> , que especifica a cor usada para o contorno do elemento Automation. Esse atributo é especificado como um <strong>COLORREF</strong>, um valor de 32 bits usado para especificar uma cor RGB ou RGBA.<br/> Tipo de variante: <strong>VT_I4</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>OutlineThickness</strong> , que especifica a largura do contorno do elemento Automation.<br/> Tipo de variante: <strong>VT_R8</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: <strong>VT_EMPTY</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>PositionInSet</strong> , que é um inteiro baseado em 1 associado a um elemento Automation. <strong>PositionInSet</strong> descreve o local ordinal do elemento dentro de um conjunto de elementos que são considerados irmãos.<br/> <strong>PositionInSet</strong> trabalha em coordenação com a propriedade <strong>sizeofset</strong> para descrever o local ordinal no conjunto.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>ProcessId</strong> , que é um inteiro que representa o identificador de processo (ID) do elemento de automação.<br/> O identificador do processo (ID) é atribuído pelo sistema operacional. Ele pode ser visto na coluna <strong>pid</strong> da guia <strong>processos</strong> no Gerenciador de tarefas.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>ProviderDescription</strong> , que é uma cadeia de caracteres formatada que contém as informações de origem do provedor de automação da interface do usuário para o elemento Automation, incluindo informações de proxy.<br/> Tipo de variante: <strong>VT_BSTR</strong><br/> Valor padrão: cadeia de caracteres vazia<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>runtimeId</strong> , que é uma matriz de inteiros que representa o identificador de um elemento de automação.<br/> O identificador é exclusivo na área de trabalho, mas é garantido que seja exclusivo somente na interface do usuário da área de trabalho na qual foi gerado. Os identificadores podem ser reutilizados ao longo do tempo.<br/> O formato de <strong>runtimeId</strong> pode ser alterado. O identificador retornado deve ser tratado como um valor opaco e usado somente para comparação; por exemplo, para determinar se um elemento de automação está no cache.<br/> Tipo de variante: <strong>VT_I4</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: <strong>VT_EMPTY</strong><br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>size</strong> , que especifica a largura e a altura do elemento Automation.<br/> Tipo de variante: <strong>VT_R8</strong>  |  <strong>VT_ARRAY</strong><br/> Valor padrão: <strong>VT_EMPTY</strong><br/></td>
</tr>
<tr class="even">
...
<td style="text-align: left;">Identifica a propriedade <strong>sizeofset</strong> , que é um inteiro baseado em 1 associado a um elemento Automation. <strong>Sizeofset</strong> descreve a contagem de elementos de automação em um grupo ou conjunto que são considerados irmãos.<br/> <strong>Sizeofset</strong> trabalha em coordenação com a propriedade <strong>PositionInSet</strong> para descrever a contagem de itens no conjunto.<br/> Tipo de variante: <strong>VT_I4</strong><br/> Valor padrão: 0<br/></td>
</tr>
<tr class="odd">
...
<td style="text-align: left;">Identifica a propriedade <strong>VisualEffects</strong> , que é um campo de bits que especifica efeitos no elemento de automação, como sombra, reflexo, brilho, bordas suaves ou bisel.<br/> VisualEffects:<br/>
<ul>
