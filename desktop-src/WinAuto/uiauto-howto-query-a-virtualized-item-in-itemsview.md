...
Exibição de Itens é um componente de interface do usuário que permite aos usuários exibir e interagir com arquivos e outros itens. No Windows 7, a Exibição de Itens substitui o controle de exibição de lista para apresentar itens na exibição padrão do Windows Explorer. A Exibição de Itens também é usada na Caixa de Diálogo de Item Comum, menu Iniciar resultados da pesquisa e outros Windows 7 interface do usuário que usam o controle Navegador do Explorer. Em comparação com o controle de exibição de lista, a Exibição de Itens oferece as seguintes vantagens para os usuários:
...
Também é possível mover itens para dentro e [](uiauto-implementingscroll.md) fora da exibição usando o padrão de controle De rolagem (ou usando as barras de rolagem). Itens e grupos são realizados conforme rolam para a exibição e são virtualizados à medida que rolam para fora da exibição.
