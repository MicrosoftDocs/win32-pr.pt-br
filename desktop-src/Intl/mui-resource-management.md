...
O MUI fornece dois utilitários de recursos que você pode usar para preparar arquivos de recursos para seu aplicativo. Um utilitário específico do MUI, chamado MUIRCT, permite que você crie um arquivo LN e arquivos de recursos específicos de idioma associados. no Windows Vista e posterior, o compilador Windows RC também foi modificado para compilar esses arquivos de acordo com a tecnologia de recursos do MUI. Para obter a sintaxe e os detalhes dessas ferramentas, consulte [utilitários de recursos](resource-utilities.md).
