...
> [!Note]  
> Novos aplicativos do Windows devem usar [Unicode](unicode.md) para evitar as inconsistências de páginas de código variadas e para facilitar a localização.
...
Muitas funções da API do Windows têm versões "A" (ANSI) e "W" (amplas, Unicode). A versão "A" manipula o texto com base nas páginas de código do Windows, enquanto a versão "W" manipula o texto Unicode. Consulte [tipos de dados do Windows para cadeias de caracteres](windows-data-types-for-strings.md) e [convenções para protótipos de função](conventions-for-function-prototypes.md).
...
Além das páginas de código do Windows e OEM, seus aplicativos podem usar páginas de código não nativas. Os exemplos são páginas de código EBCDIC e Macintosh.
...
Seus aplicativos também podem chamar a função [**GetACP**](/windows/desktop/api/Winnls/nf-winnls-getacp) . Essa função recupera o identificador da página de código do Windows (ANSI) atual.
