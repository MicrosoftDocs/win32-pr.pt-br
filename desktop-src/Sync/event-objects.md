...
O objeto de evento é útil no envio de um sinal para um thread que indica que ocorreu um evento específico. Por exemplo, na entrada e na saída sobrepondo, o sistema define um objeto de evento especificado para o estado sinalizado quando a operação sobreexplorada foi concluída. Um único thread pode especificar objetos de evento diferentes em várias operações sobrecarroadas [simultâneas](wait-functions.md) e, em seguida, usar uma das funções de espera de vários objetos para aguardar o estado de qualquer um dos objetos de evento a ser sinalizado.
