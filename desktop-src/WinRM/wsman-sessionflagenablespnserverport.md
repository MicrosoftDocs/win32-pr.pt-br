...
O método **WSMan. SessionFlagEnableSPNServerPort** retorna o valor do sinalizador de autenticação **WSManFlagEnableSPNServerPort** para uso no parâmetro *flags* do método [**WSMan. CreateSession**](wsman-createsession.md) . Esse método fornece uma sintaxe mais eficiente para usar a constante para que os scripts não sejam necessários para definir um valor constante. Para obter mais informações sobre como chamar esse método, consulte [constantes de sessão](session-constants.md).

**WSManFlagEnableSPNServerPort** é uma constante na enumeração **\_ \_ WSManSessionFlags** . Para obter mais informações, consulte [**outras constantes de sessão**](other-session-constants.md).
