---
description: Esta seção descreve como usar o Microsoft Media Foundation para executar a captura de áudio e vídeo.
ms.assetid: 3d8de05b-3cf3-4f9f-98b9-65873592152a
title: Captura de áudio/vídeo
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 48f91e9872c60a6bb57d5011c3b87a8f24134736c4fab317cededb9293d428e8
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "117880891"
---
# <a name="audiovideo-capture"></a>Captura de áudio/vídeo

Esta seção descreve como usar o Microsoft Media Foundation para executar a captura de áudio e vídeo.

-   [Captura de áudio/vídeo no Media Foundation](audio-video-capture-in-media-foundation.md)
-   Captura de vídeo
    -   [Enumerando dispositivos de captura de vídeo](enumerating-video-capture-devices.md)
    -   [Manipulando a perda de dispositivo de vídeo](handling-video-device-loss.md)
    -   [Como definir o formato de captura de vídeo](how-to-set-the-video-capture-format.md)
    -   [Como definir a taxa de quadros de captura de vídeo](how-to-set-the-video-capture-frame-rate.md)

## <a name="related-topics"></a>Tópicos relacionados

<dl> <dt>

[Guia Media Foundation programação do Media Foundation](media-foundation-programming-guide.md)
</dt> </dl>

 

 



