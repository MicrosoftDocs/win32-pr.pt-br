...
---
# <a name="seeking-fast-forward-and-reverse-play"></a><span data-ttu-id="04e67-103">Busca, avanço rápido e reprodução reversa</span><span class="sxs-lookup"><span data-stu-id="04e67-103">Seeking, Fast Forward, and Reverse Play</span></span>

<span data-ttu-id="04e67-104">Este tópico mostra o código de exemplo para gerenciar alterações de busca e taxa, ao usar a [sessão de mídia](media-session.md) para reprodução.</span><span class="sxs-lookup"><span data-stu-id="04e67-104">This topic shows example code to manage seeking and rate changes, when using the [Media Session](media-session.md) for playback.</span></span>

<span data-ttu-id="04e67-105">Ao usar a sessão de mídia para reprodução, um aplicativo pode buscar e alterar a taxa de reprodução da seguinte maneira:</span><span class="sxs-lookup"><span data-stu-id="04e67-105">When using the Media Session for playback, an application can seek and change the playback rate as follows:</span></span>

-   <span data-ttu-id="04e67-106">Para buscar, chame [**IMFMediaSession:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) e especifique a posição de busca.</span><span class="sxs-lookup"><span data-stu-id="04e67-106">To seek, call [**IMFMediaSession::Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) and specify the seek position.</span></span>
-   <span data-ttu-id="04e67-107">Para alterar a taxa de reprodução, use a interface [**IMFRateControl**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) , conforme descrito em [controle de taxa](rate-control.md).</span><span class="sxs-lookup"><span data-stu-id="04e67-107">To change the playback rate, use the [**IMFRateControl**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) interface as described in [Rate Control](rate-control.md).</span></span>
-   <span data-ttu-id="04e67-108">Para obter quadros de vídeo atualizados ao procurar, defina a taxa de reprodução como zero antes de chamar [**IMFMediaSession:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start).</span><span class="sxs-lookup"><span data-stu-id="04e67-108">To get updated video frames while seeking, set the playback rate to zero before calling [**IMFMediaSession::Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start).</span></span> <span data-ttu-id="04e67-109">Essa operação é chamada de *depuração*.</span><span class="sxs-lookup"><span data-stu-id="04e67-109">This operation is called *scrubbing*.</span></span> <span data-ttu-id="04e67-110">(Consulte [como executar a depuração](how-to-perform-scrubbing.md).)</span><span class="sxs-lookup"><span data-stu-id="04e67-110">(See [How to Perform Scrubbing](how-to-perform-scrubbing.md).)</span></span>

<span data-ttu-id="04e67-111">No entanto, para criar a melhor experiência do usuário, você deve levar em conta os seguintes comportamentos:</span><span class="sxs-lookup"><span data-stu-id="04e67-111">To create the best user experience, however, you must take the following behaviors into account:</span></span>

-   <span data-ttu-id="04e67-112">A busca é assíncrona e a sessão de mídia enfileira todas as solicitações de busca em uma fila FIFO.</span><span class="sxs-lookup"><span data-stu-id="04e67-112">Seeking is asynchronous, and the Media Session queues all seek requests on a FIFO queue.</span></span> <span data-ttu-id="04e67-113">Se você enviar várias solicitações de busca, a interface do usuário poderá ser executada antes do estado de reprodução real.</span><span class="sxs-lookup"><span data-stu-id="04e67-113">If you submit multiple seek requests, the UI might run ahead of the actual playback state.</span></span> <span data-ttu-id="04e67-114">Por exemplo, suponha que seu aplicativo implemente uma barra de busca.</span><span class="sxs-lookup"><span data-stu-id="04e67-114">For example, suppose your application implements a seek bar.</span></span> <span data-ttu-id="04e67-115">Se o usuário arrastar a barra de busca para frente e para trás, pode haver um atraso enquanto as buscas posteriores são executadas.</span><span class="sxs-lookup"><span data-stu-id="04e67-115">If the user drags the seek bar forward and backward, there might be a lag while the forward seeks are executed.</span></span> <span data-ttu-id="04e67-116">Enquanto uma busca está em andamento, você deve armazenar em cache as solicitações de busca do usuário.</span><span class="sxs-lookup"><span data-stu-id="04e67-116">While a seek is in progress, you should cache the user's seek requests.</span></span> <span data-ttu-id="04e67-117">Quando a operação de busca atual for concluída, envie a solicitação de busca mais recente do usuário e descarte as outras.</span><span class="sxs-lookup"><span data-stu-id="04e67-117">When the current seek operation completes, submit the user's most recent seek request and discard the others.</span></span>
-   <span data-ttu-id="04e67-118">Algumas transições de taxa não são permitidas em alguns Estados de transporte.</span><span class="sxs-lookup"><span data-stu-id="04e67-118">Some rate transitions are not allowed in some transport states.</span></span> <span data-ttu-id="04e67-119">Por exemplo, não é permitido alternar da reprodução de encaminhamento para reprodução reversa durante a reprodução.</span><span class="sxs-lookup"><span data-stu-id="04e67-119">For example, switching from forward playback to reverse playback is not allowed while playing.</span></span> <span data-ttu-id="04e67-120">As transições com suporte são descritas em [**IMFRateControl:: SetRate**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate).</span><span class="sxs-lookup"><span data-stu-id="04e67-120">The supported transitions are described in [**IMFRateControl::SetRate**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate).</span></span> <span data-ttu-id="04e67-121">As alterações de taxa também são assíncronas.</span><span class="sxs-lookup"><span data-stu-id="04e67-121">Rate changes are also asynchronous.</span></span>

<span data-ttu-id="04e67-122">A seguinte classe auxiliar pode ser usada para gerenciar solicitações de busca e classificar alterações.</span><span class="sxs-lookup"><span data-stu-id="04e67-122">The following helper class can be used to manage seek requests and rate changes.</span></span> <span data-ttu-id="04e67-123">Enquanto uma operação assíncrona está pendente, a classe armazena em cache qualquer solicitação de busca ou alteração de taxa.</span><span class="sxs-lookup"><span data-stu-id="04e67-123">While an asynchronous operation is pending, the class caches any seek or rate-change requests.</span></span> <span data-ttu-id="04e67-124">Quando a operação atual for concluída, a classe enviará a solicitação mais recente, se houver.</span><span class="sxs-lookup"><span data-stu-id="04e67-124">When the current operation completes, the class submits the most recent request, if any.</span></span> <span data-ttu-id="04e67-125">A classe também gerencia o estado de transporte para evitar alterações de taxa inválidas.</span><span class="sxs-lookup"><span data-stu-id="04e67-125">The class also manages the transport state to avoid invalid rate changes.</span></span>

<span data-ttu-id="04e67-126">Aqui está a declaração da classe PlayerSeeking.</span><span class="sxs-lookup"><span data-stu-id="04e67-126">Here is the declaration of the PlayerSeeking class.</span></span>
...
<span data-ttu-id="04e67-127">Aqui está a implementação da classe PlayerSeeking.</span><span class="sxs-lookup"><span data-stu-id="04e67-127">Here is the implementation of the PlayerSeeking class.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="04e67-128">Tópicos relacionados</span><span class="sxs-lookup"><span data-stu-id="04e67-128">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="04e67-129">Controle de taxa</span><span class="sxs-lookup"><span data-stu-id="04e67-129">Rate Control</span></span>](rate-control.md)
</dt> <dt>

[<span data-ttu-id="04e67-130">Reprodução de áudio/vídeo</span><span class="sxs-lookup"><span data-stu-id="04e67-130">Audio/Video Playback</span></span>](audio-video-playback.md)
</dt> </dl>
