...
Muitos dispositivos de áudio dão suporte a formatos de fluxo PCM e não PCM. No entanto, o mecanismo de áudio pode misturar somente fluxos PCM. Assim, somente os fluxos de modo exclusivo podem ter formatos não PCM. Além disso, somente os formatos não PCM com taxas de dados fixas têm suporte em modo exclusivo. Um exemplo de um formato não PCM de taxa fixa é um fluxo de áudio do Windows Media Audio Professional (WMA pro) de 48-kHz que passa por um link de S/PDIF (interface digital) Sony/Philips no formato digital sem ser decodificado. Para obter mais informações sobre como usar fluxos do WMA pro em S/PDIF, consulte a seguinte documentação:

-   A descrição da marca WAVE \_ \_ Wave WMA \_ -Format na documentação do Windows DDK.
-   O white paper intitulado "suporte de driver de áudio para o formato pro-over-S/PDIF do WMA" no site de [tecnologias do dispositivo de áudio para Windows](https://www.microsoft.com/whdc/device/audio/default.mspx) .
