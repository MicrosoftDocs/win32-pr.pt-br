...
- users AD , Enumerando usuários em servidores membros e Windows 2000 Professional
...
1.  A bind ao computador usando as seguintes regras:
    ...
    2.  Use o seguinte formato de cadeia de caracteres de associação usando o provedor WinNT, o nome do computador e um parâmetro extra para instruir a ADSI de que ele está sendo vinculado a um computador: "WinNT:// <computer name> , <computer> ". O parâmetro " &lt; nome do computador " é o nome do computador para cujos grupos &gt; acessar. Esse parâmetro instrui a ADSI de que ele está se vinculando a um computador e permite que o analisador do provedor WinNT ignore algumas consultas de resolução de ambiguidade para determinar a que tipo de objeto você está se vinculando.
    3.  A bind à [**interface IADsContainer.**](/windows/desktop/api/iads/nn-iads-iadscontainer)
2.  Adicione "user" à [**propriedade IADsContainer.Filter.**](/windows/desktop/ADSI/iadscontainer-property-methods) Isso faz com que a enumeração contenha apenas objetos que têm a classe de objeto "user".
3.  Enumerar os objetos. Para cada objeto de usuário, use os métodos de interface [**IADs**](/windows/desktop/api/iads/nn-iads-iads) ou [**IADsUser**](/windows/desktop/api/iads/nn-iads-iadsuser) para ler as propriedades do usuário.
