...
description: Windows O Servidor 2003 inclui \ 0034;restaurar objetos excluídos \ 0034; Recurso.
...
- objetos AD , restaurando objetos excluídos
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "119025114"
---
# <a name="restoring-deleted-objects"></a>Restaurando objetos excluídos

Windows O Servidor 2003 inclui o recurso "restaurar objetos excluídos".
...
-   Um objeto não pode ser restaurado quando o tempo de vida da chave de exclusão do objeto tiver expirado porque quando o tempo de vida da chave de exclusão tiver expirado, o objeto será excluído permanentemente.
-   Objetos que existem na raiz do contexto de nomen por exemplo, um domínio ou partição de aplicativo, não podem ser restaurados.
-   Objetos de esquema não podem ser restaurados. Objetos de esquema nunca devem ser excluídos.
-   É possível restaurar contêineres excluídos, mas a restauração dos objetos excluídos que estavam no contêiner antes da exclusão é difícil porque a estrutura de árvore no contêiner deve ser reconstruída manualmente.

## <a name="permissions-required-to-restore-a-deleted-object"></a>Permissões necessárias para restaurar um objeto excluído

Quando um objeto é excluído, o descritor de segurança do objeto é mantido. Embora o proprietário seja identificável do descritor de segurança, por motivos de segurança, somente os administradores de domínio têm permissão para restaurar objetos excluídos. Os administradores de domínio podem conceder a permissão para restaurar a exclusão de objetos para outros usuários e grupos concedendo ao usuário ou ao grupo o direito de acesso de controle "Reanimar Tombstone". O direito de acesso de controle "Reanimar Tombstone" é concedido na raiz do Contexto de Nomenminação. Somente os usuários que tinham permissão de acesso de leitura em um objeto e seus atributos têm permissão para ler o objeto e atributos acessíveis depois que o objeto é excluído.

> [!Note]  
> Conceder a um usuário essa permissão pode ser um risco de segurança porque pode permitir que o usuário restaure um objeto de conta que tenha acesso aos recursos aos que o usuário normalmente não teria acesso. Ao restaurar uma conta, o usuário essencialmente obtém o controle dessa conta porque o usuário deve definir a senha inicial na conta quando a conta é restaurada.

 

Para restaurar completamente um objeto excluído, o usuário deve:

-   Ter ou ser membro de um grupo que tenha o direito de acesso de controle "Reanimar Tombstone".

-   Ter acesso de gravação para cada atributo obrigatório que requer atualização.

-   Ter acesso de gravação ao RDN (Nome Diferenciado Relativo).

-   Ter acesso de gravação a cada atributo opcional que precisa ser atualizado.

-   Ter direitos de criação filho no contêiner de destino para a classe de objeto do objeto restaurado.

    > [!Note]  
    > O **atributo isDeleted** não é verificado durante a operação de restauração. Nem a permissão delete-child no contêiner "Objetos Excluídos" será verificada.

     

## <a name="restoring-a-deleted-object"></a>Restaurando um objeto excluído

Para restaurar um objeto excluído, o objeto deve estar localizado primeiro no contêiner Objetos Excluídos. Para obter mais informações sobre como recuperar objetos excluídos, consulte [Recuperando objetos excluídos.](retrieving-deleted-objects.md)

Quando o objeto tiver sido localizado, as operações a seguir deverão ser concluídas em uma única operação LDAP. Isso requer o uso da [**função ldap \_ modify ext \_ \_ s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_modify_ext_s) com o controle [**OID LDAP \_ SERVER SHOW \_ \_ DELETED. \_**](/previous-versions/windows/desktop/ldap/ldap-server-show-deleted-oid)

-   Remova o **valor do atributo isDeleted.** O **valor do atributo isDeleted** deve ser removido, não definido como **FALSE.**
-   Substitua o nome diferenciado do objeto para que ele seja movido para um contêiner diferente do contêiner Objetos Excluídos. Pode ser qualquer contêiner que normalmente possa conter o objeto . O nome diferenciado do contêiner anterior do objeto pode ser encontrado no atributo **lastKnownParent** do objeto excluído. O **atributo lastKnownParent** só será definido se o objeto tiver sido excluído em um controlador de domínio Windows Server 2003. Portanto, é possível que o conteúdo do **atributo lastKnownParent** seja impreciso.
-   Restaure os atributos obrigatórios para o objeto que foram limpos durante a exclusão.

> [!Note]  
> O **atributo objectCategory** também pode ser definido quando o objeto é restaurado, mas não é necessário. Se nenhum **valor objectCategory** for especificado, **o objectCategory** padrão para **objectClass** do objeto será usado.
