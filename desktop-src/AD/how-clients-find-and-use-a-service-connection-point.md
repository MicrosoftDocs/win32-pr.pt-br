...
---
# <a name="how-clients-find-and-use-a-service-connection-point"></a><span data-ttu-id="413b9-104">Como os clientes encontram e usam um ponto de conexão de serviço</span><span class="sxs-lookup"><span data-stu-id="413b9-104">How Clients Find and Use a Service Connection Point</span></span>

<span data-ttu-id="413b9-105">O exemplo de código a seguir mostra como um aplicativo cliente pesquisa o catálogo global para um ponto de conexão de serviço (SCP).</span><span class="sxs-lookup"><span data-stu-id="413b9-105">The following code example shows how a client application searches the Global Catalog for a Service Connection Point (SCP).</span></span> <span data-ttu-id="413b9-106">Neste exemplo de código, o aplicativo cliente tem uma cadeia de caracteres GUID embutida em código que identifica o serviço.</span><span class="sxs-lookup"><span data-stu-id="413b9-106">In this code example, the client application has a hard-coded GUID string that identifies the service.</span></span> <span data-ttu-id="413b9-107">O instalador de serviço armazenou a mesma cadeia de caracteres de GUID como um dos valores do atributo **Keywords de palavras-chave** SCPs.</span><span class="sxs-lookup"><span data-stu-id="413b9-107">The service installer stored the same GUID string as one of the values of the SCPs multi-value **keywords** attribute.</span></span>

<span data-ttu-id="413b9-108">Este exemplo consiste em duas rotinas.</span><span class="sxs-lookup"><span data-stu-id="413b9-108">This sample consists of two routines.</span></span> <span data-ttu-id="413b9-109">A rotina **GetGC** recupera um ponteiro [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) para um catálogo global (GC).</span><span class="sxs-lookup"><span data-stu-id="413b9-109">The **GetGC** routine retrieves an [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) pointer for a Global Catalog (GC).</span></span> <span data-ttu-id="413b9-110">A rotina **ScpLocate** usa os métodos **IDirectorySearch** para pesquisar o GC.</span><span class="sxs-lookup"><span data-stu-id="413b9-110">The **ScpLocate** routine uses the **IDirectorySearch** methods to search the GC.</span></span>

<span data-ttu-id="413b9-111">O GC contém uma réplica parcial de cada objeto na floresta, mas não contém todos os atributos de SCP que o cliente requer.</span><span class="sxs-lookup"><span data-stu-id="413b9-111">The GC contains a partial replica of every object in the forest, but does not contain all of the SCP attributes that the client requires.</span></span> <span data-ttu-id="413b9-112">Primeiro, o cliente deve pesquisar o GC para localizar o SCP e recuperar seu DN.</span><span class="sxs-lookup"><span data-stu-id="413b9-112">First, the client must search the GC to find the SCP and retrieve its DN.</span></span> <span data-ttu-id="413b9-113">Em seguida, o cliente usa o DN do SCP para associar a um ponteiro [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) no SCP.</span><span class="sxs-lookup"><span data-stu-id="413b9-113">Then the client uses the SCP's DN to bind to an [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) pointer on the SCP.</span></span> <span data-ttu-id="413b9-114">Em seguida, o cliente chama o método [**IDirectoryObject:: Getobjectattributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) para recuperar o restante dos atributos.</span><span class="sxs-lookup"><span data-stu-id="413b9-114">The client then calls the [**IDirectoryObject::GetObjectAttributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) method to retrieve the rest of the attributes.</span></span>
