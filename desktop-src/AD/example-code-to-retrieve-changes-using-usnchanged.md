...
O exemplo de código a seguir usa o atributo **uSNChanged** de um objeto em Active Directory Domain Services para recuperar alterações desde uma consulta anterior. O exemplo de código pode executar uma sincronização completa ou uma atualização incremental. Para uma sincronização completa, o aplicativo de exemplo se conecta ao rootDSE de um controlador de domínio e lê os seguintes parâmetros que ele armazena para ser usado na próxima sincronização incremental:

-   O nome DNS do controlador de domínio. As sincronizações incrementais devem ser executadas no mesmo DC que a sincronização anterior.
-   O GUID de **invocação** do DC. O exemplo de código usa esse valor para detectar que o DC foi restaurado de um backup; nesse caso, o exemplo deve executar uma sincronização completa.
-   O **HighestCommittedUsn**. Esse valor se torna o limite inferior para o filtro **uSNChanged** na próxima sincronização incremental.

O exemplo de código usa a interface [**IDirectorySearch**](/windows/desktop/api/iads/nn-iads-idirectorysearch) , especificando o nome distinto da base da pesquisa, um escopo de pesquisa e um filtro. Não há restrições na base de pesquisa ou no escopo. Além de especificar os objetos de interesse, o filtro também deve especificar uma comparação de **uSNChanged** , como "uSNChanged >= <lower bound USN> ". Para uma sincronização completa, " &lt; USN de limite inferior &gt; " é zero. Para uma sincronização incremental, é o 1 mais o valor de **HighestCommittedUsn** da pesquisa anterior.

Lembre-se de que este aplicativo de exemplo destina-se apenas a mostrar como usar o **uSNChanged** para recuperar as alterações do servidor de Active Directory. Ele imprime as alterações e, na verdade, não sincroniza os dados em um armazenamento secundário. Consequentemente, ele não mostra como lidar com problemas como objetos movidos ou condições "sem pai". Ele mostra como recuperar objetos excluídos, mas não mostra como um aplicativo usa o **objectGUID** dos objetos excluídos para determinar o objeto correspondente a ser excluído no armazenamento.

Além disso, o exemplo armazena em cache o nome do DC, o **invocaid** e o **HighestCommittedUsn** no registro. Em um aplicativo de sincronização real, você deve armazenar os parâmetros no mesmo armazenamento que são mantidos consistentes com o servidor de Active Directory. Isso garante que os parâmetros e os dados de objeto permaneçam sincronizados se o banco de dado tiver sido restaurado de um backup.
