...
---
# <a name="example-code-for-checking-for-rights-to-create-schema-objects"></a><span data-ttu-id="ce326-104">Código de exemplo para verificar se há direitos para criar objetos de esquema</span><span class="sxs-lookup"><span data-stu-id="ce326-104">Example Code for Checking for Rights to Create Schema Objects</span></span>

<span data-ttu-id="ce326-105">O exemplo de código C/C++ a seguir mostra uma função que verifica o atributo **allowedChildClassesEffective** no contêiner de esquema (o ponteiro IADs para o contêiner de esquema é passado como um parâmetro) para as classes **attributeSchema** e **classSchema** .</span><span class="sxs-lookup"><span data-stu-id="ce326-105">The following C/C++ code example shows a function that checks the **allowedChildClassesEffective** attribute on the schema container (IADs pointer to schema container is passed as a parameter) for the **attributeSchema** and **classSchema** classes.</span></span> <span data-ttu-id="ce326-106">Ele retornará **S \_ OK** se ambas as classes estiverem listadas em **allowedChildClassesEffective**.</span><span class="sxs-lookup"><span data-stu-id="ce326-106">It returns **S\_OK** if both classes are listed in **allowedChildClassesEffective**.</span></span> <span data-ttu-id="ce326-107">Se ambos não forem, retornará **S \_ false**.</span><span class="sxs-lookup"><span data-stu-id="ce326-107">If both are not, it returns **S\_FALSE**.</span></span>
