---
title: Exemplo de código para criar um direito de acesso de controle
description: Este tópico inclui um exemplo de código que cria um objeto controlAccessRight no contêiner Extended-Rights.
...
ms.locfileid: "103915949"
---
# <a name="example-code-for-creating-a-control-access-right"></a><span data-ttu-id="5449a-103">Exemplo de código para criar um direito de acesso de controle</span><span class="sxs-lookup"><span data-stu-id="5449a-103">Example Code for Creating a Control Access Right</span></span>

<span data-ttu-id="5449a-104">O exemplo a seguir Visual Basic cria um objeto **controlAccessRight** no contêiner Extended-Rights.</span><span class="sxs-lookup"><span data-stu-id="5449a-104">The following Visual Basic example creates a **controlAccessRight** object in the Extended-Rights container.</span></span>
...
<span data-ttu-id="5449a-105">O exemplo de código C++ a seguir é uma função que cria um objeto **controlAccessRight** no contêiner Extended-Rights.</span><span class="sxs-lookup"><span data-stu-id="5449a-105">The following C++ code example is a function that creates a **controlAccessRight** object in the Extended-Rights container.</span></span> <span data-ttu-id="5449a-106">Ao chamar essa função, use o formato a seguir para especificar a cadeia de caracteres GUID para o parâmetro *pszRightsGUID* .</span><span class="sxs-lookup"><span data-stu-id="5449a-106">When you call this function, use the following format to specify the GUID string for the *pszRightsGUID* parameter.</span></span>
...
<span data-ttu-id="5449a-107">A matriz **ADSVALUE** da propriedade **AppliesTo** usa o mesmo formato de GUID e define o membro **dwType** como a **cadeia \_ de \_ \_ caracteres de ignorar caso de ADSTYPE**.</span><span class="sxs-lookup"><span data-stu-id="5449a-107">The **ADSVALUE** array for the **appliesTo** property uses the same GUID format and sets the **dwType** member to **ADSTYPE\_CASE\_IGNORE\_STRING**.</span></span>
...
<span data-ttu-id="5449a-108">Essa função de exemplo **CreateExtendedRight** pode ser chamada com o exemplo de código a seguir.</span><span class="sxs-lookup"><span data-stu-id="5449a-108">This **CreateExtendedRight** sample function can be called with the following code example.</span></span>
