...
O chamador das funções de restauração deve ter o privilégio de acesso de **\_ \_ nome de restauração se** . Use a função [**DsSetAuthIdentity**](dssetauthidentity.md) para definir o contexto de segurança no qual as funções de backup e restauração do diretório são chamadas.
