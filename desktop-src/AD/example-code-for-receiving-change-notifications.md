...
---
# <a name="example-code-for-receiving-change-notifications"></a><span data-ttu-id="12df4-104">Exemplo de código para receber notificações de alteração</span><span class="sxs-lookup"><span data-stu-id="12df4-104">Example Code for Receiving Change Notifications</span></span>

<span data-ttu-id="12df4-105">O exemplo de código a seguir mostra como usar o controle de notificação de alteração LDAP para receber notificações de alterações em um objeto no Active Directory Domain Services.</span><span class="sxs-lookup"><span data-stu-id="12df4-105">The following code example shows how to use the LDAP change notification control to receive notifications of changes to an object in Active Directory Domain Services.</span></span> <span data-ttu-id="12df4-106">O exemplo registra para notificações, lê o estado inicial do objeto e, em seguida, usa um loop para aguardar e processar alterações no objeto.</span><span class="sxs-lookup"><span data-stu-id="12df4-106">The example registers for notifications, reads the initial state of the object, and then uses a loop to wait for and process changes to the object.</span></span>

<span data-ttu-id="12df4-107">Primeiro, o exemplo de código chama a função [**\_ \_ ext de pesquisa LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) , que é uma operação de pesquisa assíncrona que retorna após o registro de uma solicitação de notificação.</span><span class="sxs-lookup"><span data-stu-id="12df4-107">First, the code example calls the [**ldap\_search\_ext**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_ext) function, which is an asynchronous search operation that returns after registering a notification request.</span></span> <span data-ttu-id="12df4-108">Em segundo lugar, depois de configurar a solicitação de notificação, o exemplo chama a função de [**\_ pesquisa LDAP \_ s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) , que é uma operação de pesquisa síncrona que lê o estado atual do objeto.</span><span class="sxs-lookup"><span data-stu-id="12df4-108">Second, after setting up the notification request, the example calls the [**ldap\_search\_s**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_search_s) function, which is a synchronous search operation that reads the current state of the object.</span></span> <span data-ttu-id="12df4-109">Em terceiro lugar, o exemplo usa um loop que chama \_ o resultado do LDAP para aguardar os resultados da operação de pesquisa assíncrona.</span><span class="sxs-lookup"><span data-stu-id="12df4-109">Third, the example, uses a loop that calls ldap\_result to wait for results of the asynchronous search operation.</span></span> <span data-ttu-id="12df4-110">Quando a função de [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) retorna, o exemplo processa os resultados da pesquisa e repete o loop.</span><span class="sxs-lookup"><span data-stu-id="12df4-110">When the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function returns, the example processes the search results and repeats the loop.</span></span>

<span data-ttu-id="12df4-111">Lembre-se de que, se o exemplo ler o estado atual e, em seguida, configurar a solicitação de notificação, haveria um período de tempo durante o qual as alterações poderiam ocorrer antes da solicitação de notificação ser registrada.</span><span class="sxs-lookup"><span data-stu-id="12df4-111">Be aware that if the example read the current state and then set up the notification request, there would be a period of time during which changes could occur before the notification request was registered.</span></span> <span data-ttu-id="12df4-112">Ao ler o objeto depois de configurar a solicitação de notificação, a janela funciona em ordem inversa – você pode receber notificações de alterações que ocorreram antes de ler o estado inicial.</span><span class="sxs-lookup"><span data-stu-id="12df4-112">By reading the object after setting up the notification request, the window works in reverse–you could receive notifications of changes that occurred before reading the initial state.</span></span> <span data-ttu-id="12df4-113">Para lidar com isso, o exemplo armazena em cache o valor do atributo [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) do objeto quando lê o estado inicial do objeto.</span><span class="sxs-lookup"><span data-stu-id="12df4-113">To handle this, the example caches the object [**uSNChanged**](/windows/desktop/ADSchema/a-usnchanged) attribute value when it reads the object's initial state.</span></span> <span data-ttu-id="12df4-114">Em seguida, quando o [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) retorna com uma notificação de alteração, o exemplo compara o valor do atributo **uSNChanged** em cache com o valor relatado pelo **\_ resultado LDAP**.</span><span class="sxs-lookup"><span data-stu-id="12df4-114">Then, when [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) returns with a change notification, the example compares the cached **uSNChanged** attribute value with the value reported by **ldap\_result**.</span></span> <span data-ttu-id="12df4-115">Se o novo valor de atributo **uSNChanged** for menor ou igual ao valor armazenado em cache, o exemplo descartará os resultados porque eles indicam uma alteração que ocorreu antes da operação de leitura inicial.</span><span class="sxs-lookup"><span data-stu-id="12df4-115">If the new **uSNChanged** attribute value is less than or equal to the cached value, the example discards the results because they indicate a change that occurred prior to the initial read operation.</span></span>

<span data-ttu-id="12df4-116">Este exemplo executa uma pesquisa de nível base que monitora um único objeto.</span><span class="sxs-lookup"><span data-stu-id="12df4-116">This example performs a base level search that monitors a single object.</span></span> <span data-ttu-id="12df4-117">Você pode especificar o escopo de **\_ onscope de escopo \_ LDAP** para monitorar todos os objetos filho do objeto especificado.</span><span class="sxs-lookup"><span data-stu-id="12df4-117">You can specify the **LDAP\_SCOPE\_ONELEVEL** scope to monitor all child objects of the specified object.</span></span> <span data-ttu-id="12df4-118">Você também pode modificar o código para registrar até cinco solicitações de notificação e, em seguida, usar a função de [**\_ resultado LDAP**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) para aguardar notificações de qualquer uma das solicitações.</span><span class="sxs-lookup"><span data-stu-id="12df4-118">You can also modify the code to register up to five notification requests and then use the [**ldap\_result**](/previous-versions/windows/desktop/api/winldap/nf-winldap-ldap_result) function to wait for notifications from any of the requests.</span></span> <span data-ttu-id="12df4-119">Lembre-se de que as solicitações de notificação de alteração afetam o desempenho do servidor.</span><span class="sxs-lookup"><span data-stu-id="12df4-119">Remember that change notification requests impact the performance of the server.</span></span> <span data-ttu-id="12df4-120">Para obter mais informações sobre como melhorar o desempenho, consulte [alterar notificações em Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span><span class="sxs-lookup"><span data-stu-id="12df4-120">For more information about how to improve performance, see [Change Notifications in Active Directory Domain Services](change-notifications-in-active-directory-domain-services.md).</span></span>
