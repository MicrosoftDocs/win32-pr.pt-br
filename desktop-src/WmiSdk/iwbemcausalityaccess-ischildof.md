---
description: O método IsChildOf determina se uma solicitação é um filho de uma pId (solicitação especificada).
...
O **método IsChildOf** determina se uma solicitação é um filho de uma pId (solicitação especificada). Uma solicitação pai pode ter várias solicitações filho. Cada solicitação é identificada por um GUID (Identificador Global Exclusivo) e pode ter uma solicitação pai ou pode ser uma solicitação principal. Um GUID é um número exclusivo de 128 bits.
