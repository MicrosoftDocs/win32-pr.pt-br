...
O **método \_ AssociatorsAsync** de [**SWbemObject**](swbemobject.md) obtém objetos (classes ou instâncias) associados ao objeto atual. Esses objetos são chamados de pontos de extremidade. Esse método executa a mesma função que os ASSOCIAdores da consulta WQL executam.
...
Obrigatórios. Coletor de objeto que recebe os objetos de forma assíncrona como um retorno de chamada.
...
Para obter mais informações sobre os ASSOCIAdores de consultas WQL associadas, instâncias de origem e pontos de extremidade, consulte [ASSOCIATORS of Statement](associators-of-statement.md).
