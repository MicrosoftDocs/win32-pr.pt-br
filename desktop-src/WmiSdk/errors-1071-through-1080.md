---
description: Descreve os erros do provedor SNMP WMI 1071 a 1080.
ms.assetid: c9161c96-6839-4b32-b1bd-b40af18ab314
ms.tgt_platform: multiple
title: Erros 1071 a 1080
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: e4c92cfd1c79b5bedd070229adcbc4c663d209903428073fc976cd575022e9ea
ms.sourcegitcommit: e6600f550f79bddfe58bd4696ac50dd52cb03d7e
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "119679616"
---
# <a name="errors-1071-through-1080"></a>Erros 1071 a 1080

Descreve os erros do provedor SNMP WMI 1071 a 1080.

[Aviso 1071](#warning-1071)

[Aviso 1072](#warning-1072)

[Erro fatal 1074](#fatal-error-1074)

[Erro fatal 1075](#fatal-error-1075)

[Erro fatal 1077](#fatal-error-1077)

[Erro fatal 1079](#fatal-error-1079)

[Aviso 1080](#warning-1080)

## <a name="warning-1071"></a>Aviso 1071

<dl> <dt>

<span id="_1071__Warning_____fileName__line____Symbol__identifier__not_present_in_imported_module__identifier__"></span><span id="_1071__warning_____filename__line____symbol__identifier__not_present_in_imported_module__identifier__"></span><span id="_1071__WARNING_____FILENAME__LINE____SYMBOL__IDENTIFIER__NOT_PRESENT_IN_IMPORTED_MODULE__IDENTIFIER__"></span>**<1071, aviso>: " <fileName> linha de<\#>: símbolo <identifier> não presente no módulo importado <identifier> "**
</dt> <dd>

Aviso semântico de módulo sobre referência cruzada, específico para nem SNMPv1 nem SNMPv2C. Um símbolo importado de um módulo não é resolvido para nada nesse módulo. Embora esse módulo seja especificado na entrada, esse aviso é exibido.

</dd> </dl>

## <a name="warning-1072"></a>Aviso 1072

<dl> <dt>

<span id="_1072__Warning_____fileName___line___OBJECT-TYPE__name__is_a_descendant_of_OBJECT-TYPE__name__of_module__name__"></span><span id="_1072__warning_____filename___line___object-type__name__is_a_descendant_of_object-type__name__of_module__name__"></span><span id="_1072__WARNING_____FILENAME___LINE___OBJECT-TYPE__NAME__IS_A_DESCENDANT_OF_OBJECT-TYPE__NAME__OF_MODULE__NAME__"></span><**1072, aviso>: " <fileName> : <linha \#> objeto-Type <name> é um descendente do tipo Object-Type <name> do módulo <name> "**
</dt> <dd>

Aviso semântico do módulo de invocação de macro do **tipo de objeto** . Um objeto escalar não pode ter nenhum objeto descendente.

Esse aviso pode ocorrer em SNMPv1 ou SNMPv2C.

</dd> </dl>

## <a name="fatal-error-1074"></a>Erro fatal 1074

<dl> <dt>

<span id="_1074__Fatal_____fileName___line___SEQUENCE__Conceptual_row__OBJECT-TYPE__name__does_not_have_a_SEQUENCE_OF__Conceptual_table__OBJECT-TYPE_as_its_parent_"></span><span id="_1074__fatal_____filename___line___sequence__conceptual_row__object-type__name__does_not_have_a_sequence_of__conceptual_table__object-type_as_its_parent_"></span><span id="_1074__FATAL_____FILENAME___LINE___SEQUENCE__CONCEPTUAL_ROW__OBJECT-TYPE__NAME__DOES_NOT_HAVE_A_SEQUENCE_OF__CONCEPTUAL_TABLE__OBJECT-TYPE_AS_ITS_PARENT_"></span>**<1074, fatal>: " <fileName> : \# tipo de <de linha> Sequence (linha conceitual)-Type <name> não tem uma sequência de tipo de objeto (tabela conceitual) como seu pai"**
</dt> <dd>

Erro semântico do módulo de invocação de macro do **tipo de objeto** . Cada objeto de linha deve ter um objeto Table como seu pai.

Esse erro pode ocorrer em SNMPv1 ou SNMPv2C.

</dd> </dl>

## <a name="fatal-error-1075"></a>Erro fatal 1075

<dl> <dt>

<span id="_1075__Fatal____fileName___line____OBJECT-TYPE__name__cannot_be_a_child_of_table_OBJECT-TYPE__name__of_module__name__unless_it_is_in_the_SEQUENCE_of_the_latter_"></span><span id="_1075__fatal____filename___line____object-type__name__cannot_be_a_child_of_table_object-type__name__of_module__name__unless_it_is_in_the_sequence_of_the_latter_"></span><span id="_1075__FATAL____FILENAME___LINE____OBJECT-TYPE__NAME__CANNOT_BE_A_CHILD_OF_TABLE_OBJECT-TYPE__NAME__OF_MODULE__NAME__UNLESS_IT_IS_IN_THE_SEQUENCE_OF_THE_LATTER_"></span>**<1075, fatal>: " <fileName> : <linha \#>: o tipo de objeto <name> não pode ser um filho do tipo <name> de objeto de tabela do módulo <name> , a menos que esteja na sequência do último"**
</dt> <dd>

Erro semântico do módulo de invocação de macro do **tipo de objeto** . Cada objeto de sequência deve ter exatamente os mesmos objetos que são mencionados na definição de tipo de sequência como seus filhos. Da mesma forma, cada sequência de objeto deve ter exatamente um filho.

Esse erro pode ocorrer em SNMPv1 ou SNMPv2C.

</dd> </dl>

## <a name="fatal-error-1077"></a>Erro fatal 1077

<dl> <dt>

<span id="_1077__Fatal_____fileName___line____The_syntax_of_table_OBJECT-TYPE__name__is_not_the_SEQUENCE_OF_the_syntax_of_the_child_OBJECT-TYPE__name__of_module__name__"></span><span id="_1077__fatal_____filename___line____the_syntax_of_table_object-type__name__is_not_the_sequence_of_the_syntax_of_the_child_object-type__name__of_module__name__"></span><span id="_1077__FATAL_____FILENAME___LINE____THE_SYNTAX_OF_TABLE_OBJECT-TYPE__NAME__IS_NOT_THE_SEQUENCE_OF_THE_SYNTAX_OF_THE_CHILD_OBJECT-TYPE__NAME__OF_MODULE__NAME__"></span>**<1077, fatal>: " <fileName> : <linha \#>: a sintaxe da tabela Object-Type <name> não é a sequência da sintaxe do tipo de objeto filho <name> do módulo <name> "**
</dt> <dd>

Erro semântico do módulo de invocação de macro do **tipo de objeto** . A cláusula Syntax de um objeto Table deve ser a "SEQUENCE OF" da sintaxe do objeto Row.

Esse erro pode ocorrer em SNMPv1 ou SNMPv2C.

</dd> </dl>

## <a name="fatal-error-1079"></a>Erro fatal 1079

<dl> <dt>

<span id="_1079__Fatal___fileName___line____OBJECT-TYPE__name__is_an_extra_child_of_OBJECT-TYPE__name__of_module__name__"></span><span id="_1079__fatal___filename___line____object-type__name__is_an_extra_child_of_object-type__name__of_module__name__"></span><span id="_1079__FATAL___FILENAME___LINE____OBJECT-TYPE__NAME__IS_AN_EXTRA_CHILD_OF_OBJECT-TYPE__NAME__OF_MODULE__NAME__"></span>**<1079,> fatal: "fileName>: <line \#>: Object-Type <name> é um filho extra do tipo Object-Type <name> do módulo <name> "**
</dt> <dd>

Erro semântico do módulo de invocação de macro do **tipo de objeto** . Cada sequência de objeto deve ter exatamente um filho.

Esse erro pode ocorrer em SNMPv1 ou SNMPv2C.

</dd> </dl>

## <a name="warning-1080"></a>Aviso 1080

<dl> <dt>

<span id="_1080__Warning_____fileName__line____MIB_Module__moduleName___from_which_symbol__symbolName__is_imported__is_not_present_in_input_"></span><span id="_1080__warning_____filename__line____mib_module__modulename___from_which_symbol__symbolname__is_imported__is_not_present_in_input_"></span><span id="_1080__WARNING_____FILENAME__LINE____MIB_MODULE__MODULENAME___FROM_WHICH_SYMBOL__SYMBOLNAME__IS_IMPORTED__IS_NOT_PRESENT_IN_INPUT_"></span>**<1080, aviso>: " <fileName><linha \#>: módulo MIB <moduleName> , de onde o símbolo <symbolName> é importado, não está presente na entrada"**
</dt> <dd>

Aviso semântico de módulo sobre referência cruzada, específico para nem SNMPv1 nem SNMPv2C. Se um dos módulos na seção Imports do módulo principal ou de um módulo subsidiária não existir, mas os símbolos desse módulo não forem referenciados, esse aviso será exibido.

</dd> </dl>

 

 



