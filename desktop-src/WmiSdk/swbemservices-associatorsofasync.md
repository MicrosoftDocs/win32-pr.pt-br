---
description: Método SWbemServices.AssociatorsOfAsync – retorna uma coleção de objetos (classes ou instâncias) chamados de pontos de extremidade associados a um objeto especificado.
...
ms.locfileid: "118991786"
---
# <a name="swbemservicesassociatorsofasync-method"></a>Método SWbemServices.AssociatorsOfAsync

O **método AssociatorsOfAsync** do objeto [**SWbemServices**](swbemservices.md) retorna uma coleção de objetos (classes ou instâncias) chamados de pontos de extremidade associados a um objeto especificado. A chamada para **AssociatorsOfAsync** retorna imediatamente e os resultados e o status são retornados ao chamador por meio de eventos entregues ao sink especificado em *objWbemSink.* Para manipular cada objeto retornado, crie *um objWbemSink.* [**Manipulador de eventos OnObjectReady.**](swbemsink-onobjectready.md)

Depois que todos os objetos chegam, o processamento é feito no *objWbemSink.* [**Evento OnCompleted.**](swbemsink-oncompleted.md) Esse método executa a mesma função que a consulta ASSOCIATORS OF WQL executa. Para obter mais informações sobre como criar um sink, consulte [Recebendo um evento WMI](receiving-a-wmi-event.md).
...
Obrigatórios. O sink do objeto que recebe os objetos de forma assíncrona. Crie um [**objeto SWbemSink**](swbemsink.md) para receber os objetos.
...
Cadeia de caracteres que contém um nome de classe. Se especificado, esse parâmetro opcional indica que os pontos de extremidade retornados devem pertencer ou ser derivados da classe especificada neste parâmetro.
...
Faz com que o WMI retorne dados de alteração de classe junto com a definição de classe base. Para obter mais informações sobre qualificadores corrigidos, consulte [Localizando informações de classe WMI](localizing-wmi-class-information.md).
