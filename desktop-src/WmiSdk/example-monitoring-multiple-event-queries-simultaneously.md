...
---
# <a name="example-monitoring-multiple-event-queries-simultaneously"></a><span data-ttu-id="60744-103">Exemplo: monitorando várias consultas de evento simultaneamente</span><span class="sxs-lookup"><span data-stu-id="60744-103">Example: Monitoring Multiple Event Queries Simultaneously</span></span>

<span data-ttu-id="60744-104">O exemplo de script a seguir mostraSWbemServices.Exea notificação de evento assíncrona, usando o método [**cNotificationQueryAsync**](swbemservices-execnotificationqueryasync.md) e o objeto [**SWbemSink**](swbemsink.md) .</span><span class="sxs-lookup"><span data-stu-id="60744-104">The following script example shows asynchronous event notification, making use of the [**SWbemServices.ExecNotificationQueryAsync**](swbemservices-execnotificationqueryasync.md) method and [**SWbemSink**](swbemsink.md) object.</span></span>

> [!Note]  
> <span data-ttu-id="60744-105">Como o retorno de chamada para o coletor pode não ser retornado no mesmo nível de autenticação que o cliente requer, é recomendável que você use semisynchronous em vez de comunicação assíncrona.</span><span class="sxs-lookup"><span data-stu-id="60744-105">Because the callback to the sink might not be returned at the same authentication level as the client requires, it is recommended that you use semisynchronous instead of asynchronous communication.</span></span> <span data-ttu-id="60744-106">Para obter mais informações, consulte [chamando um método](calling-a-method.md).</span><span class="sxs-lookup"><span data-stu-id="60744-106">For more information, see [Calling a Method](calling-a-method.md).</span></span>
