...
ms.locfileid: "119955206"
---
# <a name="swbemlocatorconnectserver-method"></a>Método SWbemLocator.ConnectServer

O **método ConnectServer** do objeto [**SWbemLocator**](swbemlocator.md) se conecta ao namespace no computador especificado no *parâmetro strServer.* O computador de destino pode ser local ou remoto, mas deve ter o WMI instalado. Para exemplos e uma comparação com o tipo de moniker de conexão, consulte [Criando um script WMI](creating-a-wmi-script.md).
...
Nome do computador ao qual você está se conectando. Se o computador remoto estiver em um domínio diferente da conta de usuário na qual você faz logoff, use o nome do computador totalmente qualificado. Se você não fornecer esse parâmetro, a chamada assume como padrão o computador local.

Exemplo: `server1.network.fabrikam`

Você também pode usar um endereço IP nesse parâmetro. Se o endereço IP estiver no formato IPv6, o computador de destino deverá estar executando o IPv6. Um endereço em IPv4 é parecido com `123.123.123.123`

Um endereço IP no formato IPv6 é parecido com `2010:836B:4179::836B:4179`
...
Cadeia de caracteres que especifica o namespace no qual você faz logon. Por exemplo, para fazer logon no namespace padrão \\ raiz, use o padrão \\ raiz. Se você não especificar esse parâmetro, ele assume como padrão o namespace configurado como o namespace padrão para script. Para obter mais informações, consulte [Criando um aplicativo WMI ou script](creating-a-wmi-application-or-script.md).
...
Nome de usuário a ser usado para se conectar. A cadeia de caracteres pode estar na forma de um nome de usuário ou um Nome de Usuário de \\ Domínio. Deixe esse parâmetro em branco para usar o contexto de segurança atual. O *parâmetro strUser* só deve ser usado com conexões com servidores WMI remotos. Se você tentar especificar *strUser para* uma conexão WMI local, a tentativa de conexão falhará. Se a autenticação Kerberos estiver em uso, o nome de usuário e a senha especificados em *strUser* e *strPassword* não poderão ser interceptados em uma rede. Você pode usar o formato UPN para especificar *o strUser*.
...
Cadeia de caracteres que especifica o código de localização. Se você quiser usar a localidade atual, deixe-a em branco. Se não estiver em branco, esse parâmetro deverá ser uma cadeia de caracteres que indica a localidade desejada em que as informações devem ser recuperadas. Para identificadores de localidade da Microsoft, o formato da cadeia de caracteres é "MS xxxx", em que xxxx é uma cadeia de caracteres no formato hexadecimal que indica \_ o LCID. Por exemplo, inglês americano seria exibido como "MS \_ 409".
...
Se o *parâmetro strAuthority* começar com a cadeia de caracteres "Kerberos:", a autenticação Kerberos será usada e esse parâmetro deverá conter um nome principal Kerberos. O nome principal kerberos é especificado como Kerberos:*domínio*, como em que é o servidor ao qual `Kerberos:fabrikam` você está tentando se `fabrikam` conectar.
...
Para usar a autenticação NTLM (NT Lan Manager), você deve especificá-la como NTLMDomain:*domínio*, como em que é o `NTLMDomain:fabrikam` nome do `fabrikam` domínio.
...
> [!Note]  
> Se o domínio for especificado em *strUser*, que é o local preferencial, ele não deverá ser especificado aqui. Especificar o domínio em ambos os parâmetros resulta em um erro de parâmetro inválido.
...
Um valor de 0 para esse parâmetro faz com que a chamada para **ConnectServer** retorne somente depois que a conexão com o servidor for estabelecida. Isso pode fazer com que o programa pare de responder indefinidamente se a conexão não puder ser estabelecida.
...
A **chamada ConnectServer** é garantida para retornar em 2 minutos ou menos. Use esse sinalizador para impedir que seu programa ceasing para responder indefinidamente se a conexão não puder ser estabelecida.
...
Se for bem-sucedido, o WMI retornará um objeto [**SWbemServices**](swbemservices.md) que está vinculado ao namespace especificado em *strNamespace* no computador especificado em *strServer*.
...
O nome de usuário atual ou especificado e a senha não eram válidos ou autorizados a fazer a conexão.
...
O **método ConnectServer** geralmente é usado ao se conectar a uma conta com um nome de usuário e credenciais de senha diferentes em um computador remoto porque você não pode especificar uma senha diferente em uma cadeia de caracteres [moniker.](constructing-a-moniker-string.md) Para obter mais informações, [consulte Conectando-se ao WMI em um computador remoto.](connecting-to-wmi-on-a-remote-computer.md)

O uso de um endereço IPv4 para se conectar a um servidor remoto pode resultar em um comportamento inesperado. A causa provável são entradas DNS desa stale em seu ambiente. Nessas circunstâncias, a entrada PTR stale para o computador será usada, com resultados imprevisíveis. Para evitar esse comportamento, você pode anexar um ponto (".") ao endereço IP antes de chamar ConnectServer. Isso faz com que a análise inversa de DNS falhe, mas pode permitir que a **chamada ConnectServer** tenha êxito no computador correto.

## <a name="examples"></a>Exemplos

O exemplo de código VBScript a seguir descreve como se conectar a um computador remoto para obter [**dados win32 \_ IP4RouteTable.**](/previous-versions/windows/desktop/wmiiprouteprov/win32-ip4routetable) O nome de domínio especificado em *strDomain* é usado *em strAuthority*.
