---
description: Esta seção contém a referência para o gerenciamento do Hyper-V.
ms.assetid: 87588AE3-54DF-4415-BA24-1D6E8B46E758
title: Referência de gerenciamento do Hyper-V
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 79dddd22c5f9545781c3e3c71606961ca79762ea86eab50f4cd5a5e9c38783d4
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "119427506"
---
# <a name="hyper-v-management-reference"></a>Referência de gerenciamento do Hyper-V

Esta seção contém a referência para o gerenciamento do Hyper-V.

## <a name="in-this-section"></a>Nesta seção



| Tópico                                                                               | Descrição                                                                               |
|-------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------|
| [Códigos de controle de gerenciamento do Hyper-V](hyper-v-management-control-codes.md)<br/> | Esta seção identifica os códigos de controle que são usados no gerenciamento do Hyper-V.<br/> |
| [Estruturas de gerenciamento do Hyper-V](hyper-v-management-structures.md)<br/>       | Esta seção identifica as estruturas que são usadas no gerenciamento do Hyper-V.<br/>    |



 

 

 




