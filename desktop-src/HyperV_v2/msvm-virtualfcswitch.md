...
Especifica a saúde atual do elemento. Esse atributo expressa a saúde desse elemento, mas não necessariamente a de seus subcomponentes.

Quando ocorrer um erro crítico, verifique o log de eventos para obter detalhes. A **propriedade EnabledState** também pode conter mais informações. Por exemplo, quando o espaço em disco é críticomente baixo, **HealthState** é definido como 25, a máquina virtual pausa e **EnabledState** é definido como 32768 (pausado).
...
| Valor                                                                                                                                                                                                                                                            | Significado                                                                                                             |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------|
| <span id="OK"></span><span id="ok"></span><dl> <dt>**OK**</dt> <dt>5</dt> </dl>                                                                               | O elemento é totalmente funcional e está operando dentro de parâmetros operacionais normais e sem erro.<br/> |
| <span id="Major_Failure"></span><span id="major_failure"></span><span id="MAJOR_FAILURE"></span><dl> <dt>**Falha principal**</dt> <dt>20</dt> </dl>             | O elemento sofreu uma falha grave.<br/>                                                                |
| <span id="Critical_failure"></span><span id="critical_failure"></span><span id="CRITICAL_FAILURE"></span><dl> <dt>**Falha crítica**</dt> <dt>25</dt> </dl> | O elemento é nãofuncional e a recuperação pode não ser possível.<br/>                                         |
...
Essa propriedade é herdada de [**CIM \_ ComputerSystem**](/windows/desktop/CIMWin32Prov/cim-computersystem)e é sempre definida como **Null.**
...
A data e a hora em que a configuração da máquina virtual foi criada para uma máquina virtual, ou **Null,** para um sistema operacional de gerenciamento. Essa propriedade é herdada de [**CIM \_ ManagedSystemElement.**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)
...
O rótulo pelo qual o objeto é conhecido. Essa propriedade é herdada do [**sistema CIM. \_**](/windows/desktop/CIMWin32Prov/cim-system)
...
Uma matriz que contém os status atuais do objeto . Essa propriedade é herdada de [**CIM \_ ManagedSystemElement.**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)
...
Uma cadeia de caracteres que descreve como ou por que o sistema é dedicado quando a matriz **Dedicada** inclui o valor 2 (Outro). Essa propriedade é herdada de [**CIM \_ ComputerSystem**](/windows/desktop/CIMWin32Prov/cim-computersystem)e é sempre definida como **Null.**
...
Essa propriedade é herdada [**de \_ COMPUTERSystem cim**](/windows/desktop/CIMWin32Prov/cim-computersystem), mas não é usada.
