...
| [**Definido**](msvm-emulatedethernetport-reset.md)                           | Redefine o dispositivo emulado.<br/>   |
...
Uma descrição do objeto . Essa propriedade é herdada de [**\_ ManagedElement do CIM**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)e sempre é definida como "Porta Ethernet Emulada da Microsoft".
...
Um endereço ou outras informações de identificação usadas para nomear exclusivamente o dispositivo lógico. Essa propriedade é herdada de [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)e sempre é definida como "Microsoft: dados \\ *específicos do dispositivo GUID".*
...
Um nome de exibição para o objeto . Essa propriedade é herdada de [**\_ ManagedElement do CIM**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)e é sempre definida como "Adaptador de Rede Herdado".
...
Os recursos habilitados na lista de todos os com suporte, que são definidos na matriz **Recursos.** Essa propriedade é herdada de [**\_ EthernetPort CIM,**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport)mas não é usada.
...
Os estados habilitados e desabilitados de um elemento. Essa propriedade é herdada [**de CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))e é sempre definida como 5 ("Não Aplicável").
...
Indica se o erro relatado em **LastErrorCode** agora está limpo. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
Uma cadeia de caracteres que fornece mais informações sobre o erro registrado **em LastErrorCode** e informações sobre as ações corretivas que podem ser tomadas. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
Indica se a porta está operando no modo duplex completo. Essa propriedade é herdada [**de Cim \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)e sempre é definida como **True.**
...
A saúde atual do elemento. Esse atributo expressa a saúde desse elemento, mas não necessariamente a de seus subcomponentes. Essa propriedade é herdada de [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)e é sempre definida como 5 ("OK").
...
Os tipos de links. Quando definido como 1 ("Outro"), a propriedade relacionada **OtherLinkTechnology** contém uma descrição de cadeia de caracteres do tipo de link. Essa propriedade é herdada de [**Cim \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport) e é sempre definida como 2 ("Ethernet").
...
O tamanho máximo do campo INFO (não MAC) que será recebido ou transmitido. Essa propriedade é herdada [**de \_ ETHERNETPort CIM**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport)e sempre é definida como 1500.
...
Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
A largura de banda máxima da porta, em bits por segundo. Essa propriedade é herdada [**de CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))e é sempre definida como 100000000.
...
O rótulo pelo qual o objeto é conhecido. Quando subclasse, essa propriedade pode ser substituído para ser uma propriedade de chave. Essa propriedade é herdada de [**\_ ManagedSystemElement do CIM**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)e sempre é definida como "Porta Ethernet".
...
Os status atuais do elemento. Essa propriedade é herdada de [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)e sempre é definida como 2 ("OK").
...
Uma matriz de cadeias de caracteres de forma livre que fornece explicações mais detalhadas para qualquer uma das funcionalidades habilitadas especificadas como "Outros". Essa propriedade é herdada de [**\_ EthernetPort CIM**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport) e não é usada.
...
Todos os dados, além das informações de ID do dispositivo, que podem ser usados para identificar um dispositivo lógico. Por exemplo, você pode usar essa propriedade para manter o nome de exibição do sistema operacional para o dispositivo. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
Um valor de cadeia de caracteres **que descreve LinkTechnology** quando ele é definido como 1 ("Outro"). Essa propriedade é herdada de [**\_ NetworkPort cim,**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)mas não é usada.
...
O tipo de módulo, quando **PortType** é definido como 1 ("Outro"). Essa propriedade é herdada de [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85)) e é sempre definida como "Ethernet Virtual".
...
O endereço de rede que está em código em uma porta. Esse endereço pode ser alterado usando uma atualização de firmware ou uma configuração de software. Quando essa alteração for feita, o campo deverá ser atualizado ao mesmo tempo. Isso deverá ser deixado em branco se não houver nenhum endereço em código para o adaptador de rede. Essa propriedade é herdada de [**CIM \_ NetworkPort.**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)
...
As portas de rede geralmente são numeradas em relação a um módulo lógico ou a um elemento de rede. Essa propriedade é herdada de [**CIM \_ NetworkPort.**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)



| Valor                                                                        | Significado                             |
|------------------------------------------------------------------------------|-------------------------------------|
| <dl> <dt>0</dt> </dl> | A NIC não é emulada.<br/> |
...
O modo específico que está atualmente habilitado para a porta. Quando definido como 1 ("Outro"), a propriedade relacionada **OtherPortType** contém uma descrição de cadeia de caracteres do tipo de porta. Essa propriedade é herdada de [**\_ ETHERNETPort CIM**](/previous-versions/windows/desktop/iscsitarg/cim-ethernetport)e sempre é definida como 1 ("Outra").
...
Os recursos de gerenciamento de energia do dispositivo. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
Indica se o dispositivo pode ser gerenciado por energia. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
O número de horas consecutivas que esse dispositivo foi ligado, desde seu último ciclo de energia. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
A largura de banda solicitada da porta, em bits por segundo. A largura de banda real é relatada em LogicalPort.Speed. Essa propriedade é herdada [**de CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85))e é sempre definida como 100000000.
...
O último estado solicitado ou desejado para o serviço de gerenciamento. Quando **EnabledState** é definido como 5 ("Não aplicável"), essa propriedade não tem nenhum significado. Essa propriedade é herdada [**de CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))e sempre é definida como 12 ("Não Aplicável").
...
A largura de banda atual da porta, em bits por segundo. Para portas que variam em largura de banda ou para aquelas em que nenhuma estimativa precisa pode ser feita, essa propriedade deve conter a largura de banda nominal. Essa propriedade é herdada [**de \_ NetworkPort cim**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)e sempre é definida como 1000000000.
...
Cadeias de caracteres que descrevem os **vários valores de matriz OperationalStatus.** As entradas nessa matriz são correlacionadas com aquelas no mesmo índice de matriz em **OperationalStatus**. Essa propriedade é herdada [**de \_ ManagedSystemElement do CIM**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)e sempre é definida como "OK".
...
O estado do dispositivo lógico. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
A MTU (unidade de transmissão máxima) que pode ser suportada, em bytes. Essa propriedade é herdada [**de Cim \_ NetworkPort**](/previous-versions/windows/desktop/iscsitarg/cim-networkport)e é sempre definida como 1500.
...
A ID da máquina virtual no **formato GUID.** Essa propriedade é herdada de [**CIM \_ LogicalDevice.**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)
...
A data ou a hora em **que EnabledState** do elemento foi alterado pela última vez. Se o estado do elemento não tiver sido alterado e essa propriedade for populada, ela deverá ser definida como um valor de intervalo 0. Se uma alteração de estado tiver sido solicitada, mas rejeitada ou ainda não processada, a propriedade não deverá ser atualizada. Essa propriedade é herdada de [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) e não é usada.
...
O número total de horas que esse dispositivo foi ligado. Essa propriedade é herdada [**de CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) e não é usada.
...
Em algumas circunstâncias, uma porta lógica pode ser identificável como uma porta de front-end ou back-end. Se não houver nenhuma restrição sobre o uso da porta, o valor deverá ser definido como 4 ("Não Restrito"). Essa propriedade é herdada de [**CIM \_ LogicalPort**](/previous-versions//cc136869(v=vs.85)) e é sempre definida como 4 ("Não Restrito").

</dd> </dl>

## <a name="remarks"></a>Comentários

O acesso à **classe Msvm \_ EmulatedEthernetPort** pode ser restrito pela Filtragem de UAC. Para obter mais informações, consulte [Controle de conta de usuário e WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
