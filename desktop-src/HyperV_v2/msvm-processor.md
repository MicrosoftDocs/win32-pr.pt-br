...
---
# <a name="msvm_processor-class"></a>\_Classe de processador Msvm

Representa o processador virtual em uma máquina virtual.
...
A classe de **\_ processador Msvm** tem estes tipos de membros:
...
A classe de **\_ processador Msvm** tem esses métodos.
...
| [**RequestStateChange**](msvm-processor-requeststatechange.md) | Solicita uma alteração de estado.<br/>      |
| [**Redefinir**](msvm-processor-reset.md)                           | Redefine o dispositivo virtual.<br/>    |
...
A classe de **\_ processador Msvm** tem essas propriedades.
...
A largura do endereço do processador, em bits. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e o valor é 32 ou 64, dependendo das características da máquina virtual.
...
O status atual do processador. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
A velocidade máxima do processador físico, levando em conta a capacidade para o processador virtual. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
A largura dos dados do processador, em bits. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e o valor é 32 ou 64, dependendo das características da máquina virtual.
...
Um endereço ou outras informações de identificação para nomear exclusivamente o dispositivo lógico. Essa propriedade é herdada [**do \_ LogicalDevice CIM**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)e é sempre definida como "Microsoft:*GUID* \\ *specific data específico*".
...
A velocidade de relógio do barramento externo do processador físico subjacente. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
A família de processadores do processador físico subjacente. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Uma matriz de cadeias de caracteres de forma livre que fornece explicações e detalhes por trás das entradas na matriz [**OtherIdentifyingInfo**](msvm-keyboard.md) . Essa propriedade é herdada [**do \_ LogicalDevice CIM**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)e é sempre definida como **NULL**.
...
A porcentagem atual da capacidade total de processamento consumida pela máquina virtual. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
O histórico registrado de percentual da potência de processamento total consumida pela máquina virtual. Esta é uma matriz que contém exemplos.
...
A velocidade máxima do relógio, em megahertz, do processador virtual. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
Uma descrição do tipo de família de processador. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e sempre é definida como **NULL**.
...
Indica se o gerenciamento de energia tem suporte no dispositivo. Essa propriedade é herdada [**do \_ LogicalDevice CIM**](/windows/desktop/CIMWin32Prov/cim-logicaldevice), mas não é usada.
...
Uma cadeia de caracteres que descreve a função do processador. Por exemplo, "processador central" ou "processador matemático". Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
O nível de revisão do processador na família de processadores do processador físico subjacente. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).
...
A data ou a hora em que o estado da máquina virtual foi alterado. Se o estado do elemento não tiver sido alterado e essa propriedade for populada, ela deverá ser definida como um valor de intervalo de 0. Se uma alteração de estado foi solicitada, mas rejeitada ou ainda não processada, a propriedade não deve ser atualizada. Essa propriedade é herdada do [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)).
...
Um identificador global exclusivo para o processador. Esse identificador só pode ser exclusivo dentro de uma família de processadores. Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor)e sempre é definida como **NULL**.
...
As informações de soquete da CPU, que incluem dados sobre como atualizar o processador (se houver suporte para atualizações). Essa propriedade é herdada [**do \_ processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor).

</dd> </dl>

## <a name="remarks"></a>Comentários

O acesso à classe de **\_ processador Msvm** pode ser restringido pela filtragem UAC. Para obter mais informações, consulte [controle de conta de usuário e WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
...
[**\_Processador CIM**](/windows/desktop/CIMWin32Prov/cim-processor)
</dt> <dt>
