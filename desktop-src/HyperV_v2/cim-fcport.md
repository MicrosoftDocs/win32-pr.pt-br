---
description: Representa os recursos e o gerenciamento de um dispositivo de porta fc (fibre channel).
...
Representa os recursos e o gerenciamento de um dispositivo de porta fc (fibre channel).
...
As configurações de COS (classe ativa de serviço) para o fibre channel.
...
Os protocolos FC-4 em execução no fibre channel.
...
PDU de LAN encapsulada de **BBL FDDI** (81)
...
**BBL 802.3 PDU de LAN encapsulada** (82)
...
O modo habilitado para a porta. Os modos de porta são definidos pelos padrões ANSI X3. Se a porta estiver registrada, esse será o tipo de porta negociado; caso contrário, o tipo de porta configurado será relatado.
...
Node Port support FC arbitrarted loop
...
A porta pode negociar para se tornar uma porta de nó (N) ou uma porta de nó com suporte a NL (loop arbitrário) do FC
...
Porta de Expansão conectando elementos de malha (por exemplo, comutadores FC)
...
Malha (elemento) Porta que suporta loop com suporte a FC
...
As configurações de COS (classe de serviço) com suporte pelo fibre channel.
