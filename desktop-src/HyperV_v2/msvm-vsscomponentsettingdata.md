---
description: Representa o estado configurado do serviço Serviço de Cópias de Sombra de Volume (VSS).
...
ms.locfileid: "118146209"
---
# <a name="msvm_vsscomponentsettingdata-class"></a>Classe Msvm \_ VssComponentSettingData

Representa o estado configurado do serviço Serviço de Cópias de Sombra de Volume (VSS).
...
Os estados habilitados e desabilitados de um elemento. Essa propriedade é herdada [**de CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85)) e tem um valor padrão de 2 (Habilitado).
...
Expõe uma atribuição específica para hospedar ou recursos subjacentes. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata) e é sempre definida como **Null.**
...
Indica como esse recurso é mapeados para recursos subjacentes. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
A ID do pool de recursos do qual o recurso é alocado. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
A quantidade de recursos garantida para estar disponível para essa alocação. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
Uma cadeia de caracteres que descreve um subtipo específico de implementação para esse recurso. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata) e é sempre definida como **Null.**
...
O tipo de recurso que essa configuração de alocação representa. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata) e é sempre definida como 1 (Outro).
...
Especifica a unidade de medida para essa alocação de recursos. O valor dessa propriedade é um valor legal do qualificador unidades programáticas, conforme definido no Anexo C.1 de DSP0004 V2.5 ou posterior. Essa propriedade é herdada de [**Cim \_ ResourceAllocationSettingData**](/previous-versions/windows/desktop/clushyperv/cim-resourceallocationsettingdata).
...
O acesso à **classe Msvm \_ VssComponentSettingData** pode ser restrito pela Filtragem de UAC. Para obter mais informações, consulte [Controle de conta de usuário e WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
