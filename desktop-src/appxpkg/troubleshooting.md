...
description: Use essas sugestões para solucionar problemas que você tem ao empacotar, implantar ou consultar um pacote de aplicativos.
...
Use essas sugestões para solucionar problemas que você tem ao empacotar, implantar ou consultar um pacote Windows aplicativo (.msix/.appx) como desenvolvedor.

> [!NOTE]
> Este artigo destina-se a desenvolvedores. Se você não for um desenvolvedor e estiver procurando ajuda com um erro Windows instalação do aplicativo, consulte [suporte Windows .](https://support.microsoft.com/hub/4338813/windows-help?os=windows-10)
...
2. Na página à esquerda, expanda **Visualizador de Eventos (Local)** Logs de Serviços e Aplicativos  >    >  **da Microsoft**  >  **Windows**.
3. Verifique se há logs disponíveis nestas categorias:

    * **AppxPackagingOM**  >  **Microsoft-Windows-AppxPackaging/Operational**
    * **AppXDeployment-Server**  >  **Microsoft-Windows-AppXDeploymentServer/Operational**

Comece observando os logs em **AppXDeployment-Server.** Se o erro foi causado por **0x80073CF0** ou **ERROR_INSTALL_OPEN_PACKAGE_FAILED**, detalhes adicionais podem estar presentes nos logs **appxpackagingOM.**
...
Esta tabela lista alguns dos códigos de erro mais comuns. Se você precisar de mais ajuda com um desses erros ou se estiver encontrando um código de erro que não está nesta lista, confira opções [de ajuda adicionais](#get-additional-help).
...
<td>0x80070002</td>
<td>Arquivo ou caminho não encontrado. Isso pode ocorrer durante uma validação typelib COM requer que o caminho para o diretório realmente exista dentro do pacote MSIX.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_BAD_FORMAT</strong></td>
<td>0x8007000B</td>
<td>O pacote não está formatado corretamente e precisa ser reescrevado ou assinado.<br/> Você poderá receber esse erro se houver uma incompatibilidade entre o nome da pessoa do certificado de assinatura e o AppxManifest.xml do editor.<br/> Consulte <a href="how-to-sign-a-package-using-signtool.md">Como assinar um pacote de aplicativos usando o SignTool.</a><br/></td>
</tr>
<tr class="even">
<td><strong>E_INVALIDARG</strong></td>
<td>0x80070057</td>
<td>Um ou mais argumentos não são válidos. Se você verificar o log de eventos do AppXDeployment-Server e vir o seguinte evento: "Durante a instalação do pacote, o sistema não conseguiu registrar a extensão windows.repositoryExtension devido ao seguinte erro: O parâmetro está incorreto". <br/> Você poderá receber esse erro se os elementos de manifesto DisplayName ou Description contêm caracteres não permitidos pelo Windows firewall; ou seja, |  e todos , devido a Windows falha ao criar o perfil AppContainer para o pacote. Remova esses caracteres do manifesto e tente instalar o pacote. <br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_INSTALL_OPEN_</strong><br/> <strong>PACKAGE_FAILED</strong><br/></td>
<td>0x80073CF0</td>
<td>O pacote não pôde ser aberto.<br/> Possíveis causas:<br/>
<ul>
<li>O pacote não está assinado.</li>
<li>O nome do publicador não é igual ao assunto do certificado de assinatura.</li>
<li>O file:// está ausente ou o pacote não pôde ser encontrado no local especificado.</li>
</ul>
Para obter mais informações, verifique o log <strong>de eventos AppxPackagingOM.</strong><br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_INSTALL_PACKAGE_</strong><br/> <strong>NOT_FOUND</strong><br/></td>
<td>0x80073CF1</td>
<td>O pacote não pôde ser encontrado.<br/> Você pode receber esse erro ao remover um pacote que não está instalado para o usuário atual.<br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_INSTALL_INVALID_</strong><br/> <strong>Pacote</strong><br/></td>
<td>0x80073CF2</td>
...
<li>O pacote não dá suporte à arquitetura correta do processador.</li>
</ul>
Para obter mais informações, verifique o log <strong>de eventos AppXDeployment-Server.</strong><br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_INSTALL_OUT_</strong><br/> <strong>OF_DISK_SPACE</strong><br/></td>
<td>0x80073CF4</td>
<td>Não há espaço em disco suficiente no computador. Liberar algum espaço e tentar novamente.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_INSTALL_NETWORK_</strong><br/> <strong>Falha</strong><br/></td>
<td>0x80073CF5</td>
...
<td>0x80073CF6</td>
<td>O pacote não pode ser registrado.<br/> Para obter mais informações, verifique o log <strong>de eventos AppXDeployment-Server.</strong><br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_INSTALL_</strong><br/> <strong>DEREGISTRATION_EFAILURE</strong><br/></td>
<td>0x80073CF7</td>
<td>O pacote não pode ter o registro não feito.<br/> Você pode receber esse erro ao remover um pacote.<br/> Para obter mais informações, verifique o log <strong>de eventos AppXDeployment-Server.</strong><br/></td>
</tr>
<tr class="even">
...
<td>0x80073CF9</td>
<td>Falha na instalação do pacote. Entre em contato com o fornecedor do software.<br/> Para obter mais informações, verifique o log <strong>de eventos AppXDeployment-Server.</strong><br/></td>
</tr>
<tr class="even">
...
<ul>
<li>o aplicativo é implantado usando F5 no Visual Studio em um computador com uma licença de desenvolvedor de Windows.</li>
<li>o pacote é assinado com uma assinatura da Microsoft e implantado como parte de Windows ou da Microsoft Store.</li>
<li>o pacote é assinado com uma assinatura confiável e instalado em um computador com uma licença de desenvolvedor, um computador ingressado no domínio com a política AllowAllTrustedApps habilitada ou um computador com uma licença de sideload Windows com a política AllowAllTrustedApps habilitada.</li>
</ul></td>
</tr>
<tr class="even">
...
<td>0x80073D04</td>
<td>A assinatura não é válida. Para se registrar no modo de desenvolvedor, AppxSignature. P7X e AppxBlockMap.xml devem ser válidos ou não devem estar presentes.<br/> se você for um desenvolvedor usando F5 com Visual Studio, verifique se o diretório do projeto criado não contém arquivos de mapa de assinatura ou de bloco de versões anteriores do pacote.<br/></td>
</tr>
<tr class="odd">
...
<td>0x80073D0A</td>
<td>não foi possível instalar o pacote porque o serviço de Firewall Windows não está em execução. habilite o serviço de Firewall Windows e tente novamente.<br/></td>
</tr>
<tr class="odd">
...
<tr class="odd">
<td><strong>ERROR_INSTALL_VOLUME_</strong><br/> <strong>ESTÁ</strong><br/></td>
<td>0x80073D0D</td>
...
<td>0x80073D23</td>
<td>A operação de implantação foi bloqueada porque a implantação de perfil especial não é permitida (perfis especiais são perfis de usuário em que as alterações são descartadas depois que o usuário sai). Tente fazer logor em uma conta que não seja um perfil especial. Você pode tentar fazer logo out e fazer logo back na conta atual ou tentar fazer logom em uma conta diferente.<br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_DEPLOYMENT_FAILED_</strong><br/> <strong>CONFLICTING_MUTABLE_PACKAGE_</strong><br/> <strong>Diretório</strong><br/></td>
<td>0x80073D24</td>
<td>A operação de implantação falhou devido ao diretório de pacote mutável de um <a href="/uwp/schemas/appxpackage/uapmanifestschema/element-desktop6-mutablepackagedirectory">pacote conflitante.</a> Para instalar esse pacote, remova o pacote existente com o diretório de pacote mutável conflitante.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_SINGLETON_RESOURCE_</strong><br/> <strong>INSTALLED_IN_ACTIVE_USER</strong><br/></td>
<td>0x80073D25</td>
<td>Falha na instalação do pacote porque um recurso singleton foi especificado e outro usuário com esse pacote instalado está conectado. Certifique-se de que todos os usuários ativos com o pacote instalado sejam desconectados e repetir a instalação.<br/></td>
</tr>
<tr class="even">
<td>ERROR_DIFFERENT_VERSION_<strong></strong><br/> <strong>OF_PACKAGED_SERVICE_INSTALLED</strong><br/></td>
<td>0x80073D26</td>
<td>Falha na instalação do pacote porque uma versão diferente do serviço está instalada. Tente instalar uma versão mais recente do pacote.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_SERVICE_EXISTS_</strong><br/> <strong>AS_NON_PACKAGED_SERVICE</strong><br/></td>
<td>0x80073D27</td>
<td>Falha na instalação do pacote porque existe uma versão do serviço fora de um pacote .msix/.appx. Entre em contato com seu fornecedor de software.<br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_PACKAGED_SERVICE_</strong><br/> <strong>REQUIRES_ADMIN_PRIVILEGES</strong><br/></td>
<td>0x80073D28</td>
<td>Falha na instalação do pacote porque são necessários privilégios de administrador. Entre em contato com um administrador para instalar este pacote.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_REDIRECTION_TO_</strong><br/> <strong>DEFAULT_ACCOUNT_NOT_ALLOWED</strong><br/></td>
<td>0x80073D29</td>
<td>A implantação do pacote falhou porque a operação teria redirecionado para a conta padrão, quando o chamador disse para não fazer isso.<br/></td>
</tr>
<tr class="even">
<td><strong>ERROR_PACKAGE_LACKS_</strong><br/> <strong>CAPABILITY_TO_DEPLOY_ON_HOST</strong><br/></td>
<td>0x80073D2A</td>
<td>A implantação do pacote falhou porque o pacote requer uma funcionalidade para direcionar esse host de modo nativo.<br/></td>
</tr>
<tr class="odd">
<td><strong>ERROR_UNSIGNED_PACKAGE_</strong><br/> <strong>INVALID_CONTENT</strong><br/></td>
<td>0x80073D2B</td>
<td>Falha na implantação do pacote porque seu conteúdo não é válido para um pacote não assinado.<br/></td>
</tr>
<tr class="even">
...
<td>0x80073D2D</td>
<td>A implantação do pacote falhou porque seu editor não está no namespace assinado.<br/></td>
</tr>
<tr class="even">
...
<td>0x80073D2F</td>
<td>Uma dependência de runtime de host que resolve um pacote com conteúdo de confiança total exige que o pacote principal tenha a funcionalidade <strong>runFullTrust.</strong><br/></td>
</tr>
<tr class="odd">
...
<td>0x80080203</td>
<td>O pacote não é válido porque não tem um manifesto ou mapa de blocos ou um arquivo de integridade de código está presente, mas um arquivo de assinatura está ausente.<br/> Verifique se o pacote não está faltando um ou mais desses arquivos necessários:<br/>
<ul>
...
<td>0x80080204</td>
<td>O arquivo AppxManifest.xml pacote não é válido.<br/></td>
</tr>
<tr class="even">
<td><strong>APPX_E_INVALID_BLOCKMAP</strong></td>
<td>0x80080205</td>
<td>O arquivo AppxBlockMap.xml pacote não é válido.<br/></td>
</tr>
<tr class="odd">
...
<td>0x80080207</td>
<td>O valor de hash computado do bloco não corresponderá ao valor tem armazenado no mapa de blocos.<br/></td>
</tr>
<tr class="odd">
<td><strong>APPX_E_REQUESTED_</strong><br/> <strong>RANGE_TOO_LARGE</strong><br/></td>
<td>0x80080208</td>
<td>O intervalo de byte solicitado é de mais de 4 GB quando convertido em um intervalo de byte de blocos.<br/></td>
</tr>
<tr class="even">
...
<td>0x800B0109</td>
<td>Uma cadeia de certificados processada, mas encerrada em um certificado raiz que não é confiável para o provedor de confiança.<br/> Consulte <a href="/previous-versions/br230260(v=vs.110)">Assinando um pacote</a>.<br/></td>
</tr>
<tr class="even">
<td><strong>CERT_E_CHAINING</strong></td>
<td>0x800B010A</td>
<td>Uma cadeia de certificados não pôde ser criada para uma autoridade de certificação raiz confiável.<br/> Consulte <a href="/previous-versions/br230260(v=vs.110)">Assinando um pacote</a>.<br/></td>
</tr>
<tr class="odd">
<td><strong>APPX_E_INVALID_</strong> <br/> <strong>SIP_CLIENT_DATA</strong> <br/></td>
<td>0x80080209</td>
<td>A <a href="/windows/win32/api/mssip/ns-mssip-sip_subjectinfo"><strong>SIP_SUBJECTINFO</strong></a>usada para assinar o pacote não continha os dados necessários<br/></td>
</tr>
<tr class="even">
<td><strong>APPX_E_INVALID_</strong> <br/> <strong>KEY_INFO</strong> <br/></td>
<td>0x8008020A</td>
<td>A <a href="/windows/win32/api/appxpackaging/ns-appxpackaging-appx_key_info"><strong>APPX_KEY_INFO</strong></a> usada para criptografar ou descriptografar o pacote contém dados inválidos.<br/></td>
</tr>
<tr class="odd">
...
<td>0x8008020C</td>
<td>O <a href="/windows/msix/app-installer/app-installer-root">arquivo .appinstaller</a> para o pacote é inválido.<br/></td>
</tr>
<tr class="odd">
<td><strong>APPX_E_DELTA_BASELINE_</strong><br/> <strong>VERSION_MISMATCH</strong><br/></td>
<td>0x8008020D</td>
<td>A versão do pacote de linha de base no pacote delta não corresponderá à versão no pacote de linha de base a ser atualizada.<br/></td>
</tr>
<tr class="even">
...
<td>0x80080210</td>
<td>O pacote anexado delta não é permitido para a operação atual.<br/></td>
</tr>
<tr class="odd">
...
<td>0x80080214</td>
<td>O estado de compactação do arquivo na linha de base e no pacote atualizado não é igual.<br/></td>
</tr>
<tr class="odd">
...
## <a name="applications-dont-start-and-their-names-are-dimmed"></a>Os aplicativos não são iniciar e seus nomes estão esmaecidas

Em um Windows 10, você não pode iniciar alguns aplicativos e os nomes dos aplicativos aparecem esmaecidas.

![Alguns nomes de aplicativo aparecem esmaecidas no menu Iniciar](./images/app-names-dimmed.png)

Ao tentar abrir um aplicativo selecionando o nome esmaecida, você pode receber uma das seguintes mensagens de erro:

> Há um problema com \<*application name*> . Entre em contato com o administrador do sistema sobre como reparar ou reinstalá-lo  
> Erro: este aplicativo não pode abrir

Além disso, as seguintes entradas de evento são registradas no log "Microsoft-Windows-TWinUI/Operational" em **Applications and Services\Microsoft\Windows\Apps:**

> Nome do log: Microsoft-Windows-TWinUI/Operational  
> Fonte: Microsoft-Windows-Immersive-Shell  
> Data: <*data*>  
> ID do evento: 5960  
...
> Palavras-chave:  
> Descrição:  
> Ativação do aplicativo Microsoft.BingNews_8wekyb3d8bbwe! AppexNews para o Windows. O contrato de lançamento foi bloqueado com 0x80073CFC erro porque seu pacote está no estado: Modificado.  
...
1. Inicie o Editor do Registro e, em seguida, **localize aHKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModel\StateChange\PackageList** subkey.
2. Para fazer o back-up dos dados da sub-chave, clique com o botão direito do mouse em **PackageList,** selecione **Exportar** e salve os dados como um arquivo do Registro.
3. Para cada um dos aplicativos listados nas entradas de log da ID de Evento 5960, siga estas etapas:  
   ...
   2. De definir o valor **de PackageStatus** como zero (**0**).
   > [!NOTE]  
   > Se não houver entradas para o aplicativo em **PackageList**, o problema terá alguma outra causa. No caso do evento de exemplo neste artigo, a sub-chave completa é **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModel\StateChange\PackageList\Microsoft.BingNews_8wekyb3d8bbwe!AppexNews\PackageStatus**
4. Reinicie o computador.

## <a name="get-additional-help"></a>Obter ajuda técnica

se precisar de ajuda adicional para resolver um problema que você está experimentando ao empacotar, implantar ou consultar um pacote de aplicativo Windows (. msix/. appx) como desenvolvedor, consulte esses recursos adicionais de suporte do desenvolvedor.
...
- o site de [suporte do Windows developer](https://developer.microsoft.com/windows/support) explica outras opções de suporte.
