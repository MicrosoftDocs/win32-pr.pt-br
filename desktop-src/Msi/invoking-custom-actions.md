...
-   Chame a ação especificada diretamente com a [**função MsiDoAction.**](/windows/desktop/api/Msiquery/nf-msiquery-msidoactiona)
...
Quando o instalador obtém um nome de ação da função [**MsiDoAction**](/windows/desktop/api/Msiquery/nf-msiquery-msidoactiona) ou de uma tabela de sequência, ele primeiro procura uma ação padrão desse nome. Se não for possível encontrar a ação padrão, o instalador consultará a tabela [CustomAction](customaction-table.md) para verificar se a ação especificada é uma ação personalizada. Se a ação especificada não for uma ação personalizada, o instalador consultará a tabela [Diálogo](dialog-table.md) para uma caixa de diálogo.
