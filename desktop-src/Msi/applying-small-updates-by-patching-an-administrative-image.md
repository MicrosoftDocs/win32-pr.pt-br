...
3.  A partir da linha de comando, use:
    ...
    **msiexec/a** *\[ caminho para o arquivo \] . msi da imagem administrativa* **/p** *patch. msp*

4.  Isso atualiza os arquivos do aplicativo e o arquivo. msi da imagem administrativa. Para obter uma lista das opções que podem ser usadas com Msiexec.exe, consulte [Opções de linha de comando](command-line-options.md). Windows Installer determina automaticamente se a imagem administrativa está usando nomes de arquivo curtos e define a propriedade [**SHORTFILENAMES**](shortfilenames.md) .
