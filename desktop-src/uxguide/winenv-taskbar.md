...
Os controles na barra de tarefas são chamados de botões da barra de tarefas. Quando um programa cria uma janela primária (ou uma janela secundária com determinadas características), o Windows adiciona um botão da barra de tarefas para essa janela e o Remove quando essa janela é fechada.

Os programas criados para o Windows 7 podem aproveitar esses novos recursos de botão da barra de tarefas:

-   As listas de atalhos fornecem acesso rápido a destinos usados com frequência (como arquivos, pastas e links) e comandos por meio de um menu de contexto acessível no botão da barra de tarefas do programa e no item de menu Iniciar, mesmo que o programa não esteja em execução no momento.
...
Os programas criados para o Windows 7 podem aproveitar esses recursos de botão da barra de tarefas. Faça as seguintes perguntas importantes para determinar se elas devem ser usadas ou não:
...
-   **Os usuários geralmente precisam acessar arquivos, pastas, links ou outros recursos usados recentemente ou frequentemente?** Nesse caso, considere fornecer uma lista de atalhos para acessar esses recursos úteis.
    ...
    Neste exemplo, o Windows Internet Explorer usa uma lista de atalhos para apresentar páginas visitadas com frequência.
...
-   **Seu programa contém guias, janelas de projeto, janelas filhas MDI ou janelas secundárias que os usuários geralmente desejariam mudar diretamente?** Nesse caso, dar a essas janelas miniaturas de botão da barra de tarefas pode ser apropriado.

## <a name="design-concepts"></a>Conceitos de design

### <a name="using-jump-lists-and-thumbnail-toolbars-effectively"></a>Usando listas de atalhos e barras de ferramentas de miniatura com eficiência

Listas de atalhos e barras de ferramentas de miniatura ajudam os usuários a acessar recursos e executar comandos com mais eficiência. No entanto, ao projetar como seu programa dá suporte a esses recursos, não tire a maior eficiência para o concedido. Se os usuários não puderem prever com precisão qual recurso tem o comando de que precisam ou se precisarem verificar vários locais, eventualmente os usuários ficarão frustrados e vão parar de usar esses recursos.

Listas de atalhos e barras de ferramentas de miniatura funcionam em conjunto com mais eficiência quando são:

-   **Claramente diferenciada.** Os usuários sabem quando procurar um destino ou comando em uma lista de atalhos e quando procurar em uma barra de ferramentas de miniatura. Há uma clara finalidade para cada uma, para que os usuários raramente confundam o conteúdo dos dois. Em geral, as listas de atalhos são usadas para iniciar novas tarefas, enquanto as barras de ferramentas de miniaturas são usadas para interagir com as tarefas em execução durante o uso de outros programas.
-   **Ajudar.** Os destinos e comandos oferecidos são aqueles que os usuários precisam. Se os usuários provavelmente não precisarem de algo, ele não será incluído. Não use o número máximo de itens se eles não forem necessários.
-   **Visíveis.** Os destinos e comandos oferecidos são aqueles que os usuários esperam encontrar. Os usuários raramente precisam examinar mais de um lugar.
-   **Bem organizado.** Os usuários podem encontrar o que estão procurando rapidamente. Eles usam rótulos descritivos, mas concisos, e ícones adequados para auxiliar o reconhecimento.

Certifique-se de fazer a pesquisa de usuário para certificar-se de que você tem tudo certo. Se, em última análise, você achar que não é possível criar listas de atalhos e barras de ferramentas de miniatura que alcancem essas metas, considere fornecer apenas uma delas. É melhor ter uma maneira previsível de fornecer comandos do que dois tipos confusos.

## <a name="guidelines"></a>Diretrizes

### <a name="taskbar-buttons"></a>Botões da barra de tarefas

-   **Faça com que os seguintes tipos de janela apareçam na barra de tarefas (para o Windows 7, usando uma miniatura de botão da barra de tarefas):**
    ...
    -   Assistentes
-   **Para o Windows 7, use miniaturas de botão da barra de tarefas para agrupar os seguintes tipos de janela com o botão da barra de tarefas da janela principal do qual ele foi iniciado.** Cada programa (especificamente, cada programa percebido como um programa separado) deve ter um único botão da barra de tarefas.

    -   Janelas secundárias
    -   Guias do espaço de trabalho
    -   Janelas de projeto
    -   Janelas filho MDI

    **Correto:**

    ![captura de tela do Windows Explorer e barra de progresso ](images/winenv-taskbar-image11.png)

    Neste exemplo, uma janela secundária é agrupada com o botão da barra de tarefas da sua janela principal.

    **Incorreto:**

    ![captura de tela do Windows Explorer e do painel de controle ](images/winenv-taskbar-image12.png)

    Neste exemplo, o painel de controle está agrupado incorretamente com o Windows Explorer. Os usuários os percebem como programas separados.

    **Incorreto:**

    ![captura de tela de programa, barra de progresso e barra de tarefas ](images/winenv-taskbar-image13.png)

    Neste exemplo, o backup do Windows usa incorretamente dois botões da barra de tarefas para um único programa.

-   **A restauração de uma janela primária também deve restaurar todas as suas janelas secundárias,** mesmo que essas janelas secundárias tenham seus próprios botões de barra de tarefas. Ao restaurar, coloque as janelas secundárias na parte superior da janela principal.
-   **Para o Windows 7, os programas que normalmente têm a presença da área de trabalho podem exibir temporariamente um botão da barra de tarefas para mostrar o status.** Faça isso somente se seu programa for normalmente exibido na área de trabalho e os usuários freqüentemente interagem com ele. Um programa que normalmente é executado sem a presença da área de trabalho deve usar seu ícone da área de notificação, mesmo que talvez nem sempre esteja visível.

    **Incorreto:**

    ![captura de tela do botão da barra de tarefas do Windows Sync Center ](images/winenv-taskbar-image14.png)

    Neste exemplo, a central de sincronização do Windows usa incorretamente um botão de barra de tarefas temporário para exibir o status. Em vez disso, ele deve usar seu ícone da área de notificação.

### <a name="icons"></a>Ícones

-   **Crie o ícone do programa para ter uma aparência ótima na barra de tarefas.** Verifique se ele é significativo e reflete sua função e sua marca. Torne-o distinto, torne-o especial e verifique se ele é bem processado em todos os tamanhos de ícone. Gaste o tempo necessário para obtê-lo certo. Siga as [diretrizes de ícone do estilo Aero](vis-icons.md).
-   **Se seu programa usar ícones de sobreposição, crie o ícone base do seu programa para lidar bem com as sobreposições.** Os ícones de sobreposição são exibidos no canto inferior direito, portanto, crie o ícone para que a área possa ser obscurecida.

    ![captura de tela de ícones e com sobreposição inferior direita ](images/winenv-taskbar-image15.png)

    Neste exemplo, o ícone do botão da barra de tarefas do programa não tem informações importantes na área inferior direita.

-   **Não use sobreposições no ícone base do programa,** independentemente de o programa usar ícones de sobreposição ou não. Usar uma sobreposição no ícone base será confuso, pois os usuários terão que descobrir que ele não está comunicando o status.

    **Incorreto:**

    ![captura de tela do ícone de base com sobreposição ](images/winenv-taskbar-image16.png)

    Neste exemplo, o ícone base do programa parece que ele está mostrando o status.
...
-   **Use ícones de sobreposição para indicar apenas o status útil e relevante.** Considere a exibição de um ícone de sobreposição para ser uma possível interrupção do trabalho do usuário, portanto, a alteração do status deve ser importante o suficiente para mérito em uma interrupção potencial.

    **Incorreto:**

    ![captura de tela de três ícones de sobreposição ](images/winenv-taskbar-image17.png)

    Nesses exemplos, o ícone de sobreposição não é importante o suficiente para mérito de uma possível interrupção.

-   **Use ícones de sobreposição para status temporário.** Os ícones de sobreposição perdem seu valor se forem exibidos constantemente, portanto, o status normal do programa não deve mostrar um ícone. Remova o ícone de sobreposição quando o ícone:

    -   **É para um problema:** Remova o ícone depois que o problema tiver sido resolvido.
    -   **Alerta que algo é novo:** Remova o ícone depois que o usuário ativou o programa.

    **Exceção:** O programa poderá exibir constantemente um ícone de sobreposição se os usuários sempre precisarem saber seu status.
...
-   **Não exiba um ícone para indicar que um problema foi resolvido.** Em vez disso, basta remover qualquer ícone anterior que indique um problema. Suponha que os usuários normalmente esperam que o seu programa seja executado sem problemas.
-   **Exibir ícones de sobreposição ou ícones da área de notificação, mas nunca ambos.** Seu programa pode dar suporte a ambos os mecanismos para compatibilidade com versões anteriores, mas se o seu programa exibir o status usando ícones de sobreposição, ele também não deverá usar ícones de área de notificação para status.

    **Incorreto:**

    ![captura de tela da barra de tarefas com o ícone exibido duas vezes ](images/winenv-taskbar-image19.png)

    Neste exemplo, o novo ícone de email é exibido de forma redundante.

-   **Não atualize o botão da barra de tarefas para chamar a atenção para uma alteração de status.** Fazer isso seria muito confuso. Permita que os usuários descubram os ícones de sobreposição por conta própria.
-   **Prefira ícones de sobreposição padrão para indicar alterações no status ou no status.** Use estes ícones de sobreposição padrão: 
...
-   **Use os nomes de grupo padrão quando apropriado.** Os nomes de grupo padrão são familiares e mais fáceis para os usuários entenderem.
    ...
    Os comandos recebem o nome do grupo de tarefas, que é atribuído pelo Windows e, portanto, não podem ser alterados.
...
-   **Quando adequado, use uma lista de destino "automática".** Os destinos automáticos são gerenciados pelo Windows, mas o programa controla os destinos específicos que são passados.
    ...
    -   O uso frequente, se recente, resultaria em muitos destinos inúteis. As listas frequentes são mais estáveis e a melhor opção quando os usuários acessam vários destinos diferentes, mas não são prováveis de retornar aos raramente usados.
        ...
        **Incorreto:**
        ...
        Usar o recente no Windows Internet Explorer resultaria em muitos destinos inúteis.
...
-   **Quando necessário, use uma lista de destino personalizada.** Seu programa tem controle total sobre o conteúdo e a ordem de classificação de uma lista de destino personalizada e, portanto, pode basear a lista em qualquer fator.
    ...
    -   Caso contrário, use um tipo diferente de destino para atender melhor às metas do usuário. Geralmente, essas listas ajudam os usuários a executar tarefas que não fizeram antes, como ler novas mensagens, assistir a novos vídeos ou verificar sua próxima reunião.
        ...
        Neste exemplo, o Windows Media Center lista os programas recentemente registrados mostra que o usuário ainda não viu.
...
-   **Desabilitar comandos que não se aplicam ao contexto atual ou que resultaria em um erro diretamente.** Não oculte esses comandos porque isso torna a apresentação da barra de ferramentas instável.
-   **Não ignore a miniatura quando os usuários clicarem em um comando se eles provavelmente revisarem os resultados ou clicarem imediatamente em outro comando.** Remova a miniatura de comandos que indicam que o usuário foi concluído por enquanto, como com comandos que exibem outras janelas.

    ![captura de tela da miniatura do Media Player com o comando ](images/winenv-taskbar-image44.png)

    Neste exemplo, clicar em avançar no Windows Media Player continua a exibir a miniatura porque os usuários podem querer fornecer outros comandos.

    ![captura de tela de miniatura com ícone de chat ](images/winenv-taskbar-image45.png)

    Neste exemplo, clicar em chat no Windows Live Messenger ignora a miniatura porque os usuários têm mais probabilidade de enviar uma mensagem.

**Apresentação**

-   **Verifique se os ícones da barra de ferramentas de miniatura estão de acordo com as diretrizes de ícone do estilo Aero.** Para cada comando, forneça ícones de cor completa de 16x16, 20x20 e 24x24 pixels de alta qualidade. As versões maiores são usadas em modos de exibição de alto dpi.
-   **Verifique se os ícones estão claramente visíveis em relação à cor do plano de fundo da barra de ferramentas nos Estados normal e em foco.** Sempre avalie ícones no contexto e nos modos de alto contraste.
-   **Escolha os designs de ícone de comando que comunicam claramente seu efeito.** Ícones de comando bem projetados são auto-explicativos para ajudar os usuários a localizar e entender os comandos com eficiência.
-   **Escolha os ícones que são reconhecíveis e distinguíveis.** Verifique se os ícones têm formas e cores distintivos. Isso ajuda os usuários a localizar os comandos rapidamente, mesmo que não se lembrem do símbolo de ícone. Após o uso inicial, os usuários não devem contar com dicas de ferramenta para distinguir entre os comandos.
-   **Forneça uma dica de ferramenta para rotular cada comando.** Uma boa dica de ferramenta rotula o controle sem rótulo que está sendo apontado. Para obter diretrizes e exemplos, consulte [ToolTips and Infotips](ctrl-tooltips-and-infotips.md).

### <a name="progress-bars"></a>Barras de progresso

-   **Siga as diretrizes gerais da barra de progresso,** incluindo não reiniciando ou fazendo backup do progresso, e usando uma barra de progresso vermelha para indicar um problema.
-   **Evite usar barras de progresso indeterminadas.** As barras de progresso indeterminadas mostram a atividade, não o progresso. Reserve barras de progresso indeterminadas para as raras situações em que os usuários não têm a atividade concedida.

Para obter mais diretrizes, consulte [barras de progresso](progress-bars.md).

## <a name="text"></a>Texto

### <a name="window-titles"></a>Títulos de janela

Ao escolher os títulos de janela, considere a aparência do título na barra de tarefas:

-   Otimize os títulos para exibição na barra de tarefas colocando concisamente as informações de distinção primeiro.
-   Para as caixas de diálogo de progresso sem janela restrita, resuma primeiro o progresso. Exemplo: "66% concluído".
-   Evite títulos de janela que tenham truncamentos inconvenientes.

    **Incorreto:**

    ![captura de tela de título que corta o nome do programa ](images/winenv-taskbar-image48.png)

    Neste exemplo, o título da janela truncado tem resultados incompletos.

### <a name="jump-list-commands"></a>Comandos da lista de atalhos

-   **Inicie comandos com um verbo.**
...
-   Consulte os itens na barra de tarefas especificamente por seu rótulo ou, geralmente, como botões da barra de tarefas.
-   Quando possível, formate os rótulos da barra de tarefas usando texto em negrito. Caso contrário, coloque o rótulo entre aspas somente se necessário para evitar confusão.
-   Consulte os ícones de sobreposição como ícones de botão da barra de tarefas. Não se refira a eles como notificações, mesmo que sua finalidade seja notificar os usuários. No entanto, você pode dizer que esses ícones notificam os usuários sobre eventos específicos.

Exemplo: o ícone do botão New mail da barra de tarefas notifica você de que uma nova mensagem de email chegou.
