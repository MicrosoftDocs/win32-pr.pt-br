...
1.  **Trabalhe para usuários padrão.** Crie o recurso para todos os usuários limitando seu escopo ao usuário atual. Ao limitar as configurações ao usuário atual (em oposição ao sistema todo), você elimina a necessidade de uma interface de usuário de elevação inteiramente e permite que os usuários concluam a tarefa.
    ...
    **Correto:**
    ...
    neste exemplo, o recurso de fuso horário foi reprojetado no Windows 7 e Windows Vista para funcionar para todos os usuários.
...
4.  **Trabalhe somente para administradores.** Use essa abordagem somente para programas e recursos do administrador! Se um recurso for destinado apenas a administradores (e não tiver nenhum caminho de navegação ou informações de somente leitura úteis para usuários padrão), você poderá solicitar credenciais de administrador no ponto de entrada antes de mostrar qualquer interface do usuário. Use essa abordagem para assistentes longos e [fluxos de página](glossary.md) quando todos os caminhos exigirem privilégios administrativos.
    ...
    Se o programa inteiro for somente para administradores, marque-o para solicitar credenciais de administrador para iniciar. Windows exibe esses ícones de programa com a sobreposição do UAC shield.
...
-   **se o programa der suporte a várias versões do Windows, exiba a blindagem do UAC se pelo menos uma versão exigir elevação.** como Windows xp nunca requer elevação, considere remover as blindagens do UAC para Windows XP se você puder fazer isso consistentemente e sem prejudicar o desempenho.
...
-   **Exiba a interface do usuário do protetor de UAC e elevação com base na tabela a seguir:**
    ...
    | Etapa da tarefa<br/>  | Alguns branches exigem elevação.<br/>                                                                                                                      | ![captura de tela do link de comando com o uac shield ](images/winenv-uac-image21.png)<br/> Blindagem UAC em links de comando que exigem elevação.<br/>                                                              | Exibir a interface do usuário de elevação quando os links de comando com a blindagem UAC são clicados.<br/>                      |

    

     

### <a name="elevation-ui"></a>Interface do usuário de elevação

-   **Se o usuário fornece uma conta que não é válida (nome ou senha) ou não tem privilégios de administrador, basta repetir a interface do usuário de credencial.** Não exibir uma mensagem de erro.
-   **Se o usuário cancelar a interface do usuário de credencial, retorne o usuário para a interface do usuário original.** Não exibir uma mensagem de erro.
-   Se o Controle de Conta de Usuário tiver sido desligado e um usuário Padrão tentar executar uma tarefa que requer elevação, forneça uma mensagem de erro informando "Esta tarefa requer privilégios de administrador. Para executar essa tarefa, você deve fazer logoff usando uma conta de administrador."

![captura de tela da tarefa requer mensagem de privilégios ](images/winenv-uac-image22.png)

Neste exemplo, o Controle de Conta de Usuário foi desligado, portanto, uma mensagem de erro explica que o usuário deve usar uma conta de administrador.

### <a name="wizards"></a>Assistentes

-   **Não elevar várias vezes.** Depois que um assistente for elevado, ele deverá permanecer elevado.
-   Se a tarefa for executada no assistente, coloque uma blindagem UAC no botão "Próximo" da página De confirmação (que deve receber um [rótulo mais específico).](win-wizards.md) Quando o usuário confirma:
    -   Se a próxima página for uma página Progresso, avance para essa página e exibir modicamente a interface do usuário de elevação. Após a elevação bem-sucedida, execute a tarefa.
    -   Se a próxima página for uma página Conclusão, avance para essa página (mas substitua temporariamente seu conteúdo por "Aguardando permissão...") e exibir modicamente a interface do usuário de elevação. Após a elevação bem-sucedida, execute a tarefa e, em seguida, exibir o conteúdo da página Conclusão.
    -   Se o usuário cancelar a interface do usuário de elevação, retorne à página Confirmação. Isso permite que o usuário tente novamente.
-   Se a tarefa for executada após a conclusão do assistente, coloque uma blindagem UAC no botão "Concluir" da página commit (que deve receber um [rótulo mais específico).](win-wizards.md) Quando o usuário confirma:
    -   Permaneça na página Confirmação e exibe modicamente a interface do usuário de elevação. Após a elevação bem-sucedida, feche o assistente.
    -   Se o usuário cancelar a interface do usuário de elevação, retorne à página Confirmação. Isso permite que o usuário tente novamente.
-   Para assistentes longos destinados somente a administradores, você pode solicitar credenciais de administrador no ponto de entrada antes de mostrar qualquer interface do usuário.

## <a name="text"></a>Texto

-   **Não use reellipses apenas porque um comando requer elevação.** A necessidade de elevar é indicada com o blindagem UAC.
...
-   Consulte administradores de computador integrados como administradores integrados.

Na documentação do usuário:

-   Consulte o ato de dar consentimento para executar uma tarefa administrativa como dar permissão.
...
-   No contexto do UAC, consulte administradores como Administradores protegidos quando não elevados e Administradores elevados após elevação.
-   Consulte a caixa de diálogo usada para inserir senhas como a interface do usuário de credencial. Consulte a caixa de diálogo usada para dar consentimento como a interface do usuário de consentimento. Consulte ambos geralmente como Interface do Usuário de Elevação.
