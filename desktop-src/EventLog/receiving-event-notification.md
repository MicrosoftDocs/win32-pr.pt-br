---
description: Recebendo notificação de eventos
ms.assetid: 3200d666-d927-4198-b1f6-1636971f5f07
title: Recebendo notificação de eventos
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103661918"
---
# <a name="receiving-event-notification"></a><span data-ttu-id="164c0-103">Recebendo notificação de eventos</span><span class="sxs-lookup"><span data-stu-id="164c0-103">Receiving Event Notification</span></span>

<span data-ttu-id="164c0-104">O exemplo a seguir mostra como usar a função [**NotifyChangeEventLog**](/windows/desktop/api/Winbase/nf-winbase-notifychangeeventlog) para receber notificações quando um evento é registrado.</span><span class="sxs-lookup"><span data-stu-id="164c0-104">The following example shows how to use the [**NotifyChangeEventLog**](/windows/desktop/api/Winbase/nf-winbase-notifychangeeventlog) function to receive notification when an event is logged.</span></span> <span data-ttu-id="164c0-105">Este exemplo filtra os eventos gravados pelo exemplo em [relatando um evento](reporting-an-event.md).</span><span class="sxs-lookup"><span data-stu-id="164c0-105">This example filters for the events written by the example in [Reporting an Event](reporting-an-event.md).</span></span>
