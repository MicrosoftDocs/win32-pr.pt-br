...
Para relatar eventos, você deve primeiro definir os eventos em um arquivo de texto de mensagem. Para obter detalhes sobre como gravar um arquivo de texto de mensagem, consulte [arquivos de texto da mensagem](message-text-files.md). O seguinte mostra o arquivo de texto de mensagem usado neste exemplo.
...
Para compilar o arquivo de texto da mensagem, use o seguinte comando:

**MC-U provider.mc**

Para compilar os recursos gerados pelo compilador de mensagem, use o seguinte comando:
...
O seguinte mostra o arquivo de cabeçalho que o compilador gerou para o arquivo de texto de mensagem acima. Inclua o arquivo de cabeçalho em seu projeto.
...
O exemplo a seguir mostra como usar a função [**ReportEvent**](/windows/desktop/api/Winbase/nf-winbase-reporteventa) para gravar os eventos definidos no arquivo de texto da mensagem acima.
...
Antes de executar este exemplo, registre o provedor no registro. Para obter detalhes sobre as configurações do registro, consulte [origens do evento](event-sources.md). Adicione "myeventprovider" como uma chave do registro na seguinte chave:

**HKEY \_ local \_ Machine \\ sistema \\ CurrentControlSet \\ Services \\ EventLog \\ Application**
...
| EventMessageFile     | REG \_ sz    | *caminho* \\ doprovider.dll |
| ParameterMessageFile | REG \_ sz    | *caminho* \\ doprovider.dll |
