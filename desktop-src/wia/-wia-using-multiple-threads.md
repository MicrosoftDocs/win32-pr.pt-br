---
description: ao usar uma interface de aquisição de imagem de Windows (WIA) em mais de um thread em um único processo, um aplicativo deve fornecer marshaling para essa interface.
...
ao usar uma interface de aquisição de imagem de Windows (WIA) em mais de um thread em um único processo, um aplicativo deve fornecer marshaling para essa interface. Se um thread criar um ponteiro de interface, você não poderá usar esse ponteiro em um thread diferente sem Marshalling.
