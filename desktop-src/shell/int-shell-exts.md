---
description: Grande parte da implementação de um objeto de manipulador de extensão do Shell é ditada por seu tipo. No entanto, há alguns elementos comuns. Este tópico aborda os aspectos da implementação que são compartilhados por todos os manipuladores de extensão do Shell.
...
ms.locfileid: "119661216"
---
# <a name="initializing-shell-extension-handlers"></a>Inicializando manipuladores de extensão do Shell

Grande parte da implementação de um objeto de manipulador de extensão do Shell é ditada por seu tipo. No entanto, há alguns elementos comuns. Este tópico aborda os aspectos da implementação que são compartilhados por todos os manipuladores de extensão do Shell.

Todos os manipuladores de extensão do Shell são objetos COM (Component Object Model em processo). Eles devem ser atribuídos a um GUID e registrados conforme descrito em [Registrando manipuladores](handlers.md)de extensão de shell. Eles são implementados como DLLs e devem exportar as seguintes funções padrão:
...
-   Manipuladores do "arrastar e soltar"
...
A interface [**IPersistFile**](/windows/win32/api/objidl/nn-objidl-ipersistfile) foi projetada para permitir que um objeto seja carregado ou salvo em um arquivo de disco. Ele tem seis métodos além de [**IUnknown**](/windows/win32/api/unknwn/nn-unknwn-iunknown), cinco de seus próprios e o [**método GetClassID**](/windows/win32/api/objidl/nf-objidl-ipersist-getclassid) que herda de [**IPersist.**](/windows/win32/api/objidl/nn-objidl-ipersist) Com extensões do Shell, **o IPersist** é usado apenas para inicializar um objeto de manipulador de extensão do Shell. Como normalmente não há necessidade de ler ou gravar no disco, somente os métodos **GetClassID** e [**Load**](/windows/win32/api/objidl/nf-objidl-ipersistfile-load) exigem uma implementação nontoken.

O Shell chama [**GetClassID**](/windows/win32/api/objidl/nf-objidl-ipersist-getclassid) primeiro e a função retorna o CLSID (identificador de classe) do objeto do manipulador de extensão. Em seguida, o Shell [**chama Carregar**](/windows/win32/api/objidl/nf-objidl-ipersistfile-load) e passa dois valores. O primeiro, *pszFile,* é uma cadeia de caracteres Unicode com o nome do arquivo ou pasta em que o Shell está prestes a operar. O segundo é *dwMode,* que indica o modo de acesso ao arquivo. Como normalmente não há necessidade de acessar arquivos, *dwMode* normalmente é zero. O método armazena esses valores conforme necessário para referência posterior.

O fragmento de código a seguir ilustra como um manipulador de extensão shell típico implementa os [**métodos GetClassID**](/windows/win32/api/objidl/nf-objidl-ipersist-getclassid) e [**Load.**](/windows/win32/api/objidl/nf-objidl-ipersistfile-load) Ele foi projetado para lidar com ANSI ou Unicode. ClSID SampleExtHandler é o GUID do objeto do manipulador de extensão e CSampleShellExtension é o nome da classe usada para implementar a \_ interface. As **\_ variáveis m szFileName** e **m \_ dwMode** são variáveis privadas usadas para armazenar o nome do arquivo e os sinalizadores de acesso.
...
-   *pidlFolder* contém o ponteiro de uma pasta para uma PIDL (lista de identificadores de item). Esse é um PIDL absoluto. Para extensões de folha de propriedades, esse valor é **NULL.** Para extensões de menu de atalho, é o PIDL da pasta que contém o item cujo menu de atalho está sendo exibido. Para manipuladores de arrastar e soltar não padrão, é o PIDL da pasta de destino.
-   *pDataObject* contém um ponteiro para a interface [**IDataObject**](/windows/win32/api/objidl/nn-objidl-idataobject) de um objeto de dados. O objeto de dados contém um ou mais nomes de arquivo no [formato \_ CF HDROP.](dragdrop.md)
-   *hRegKey* contém uma chave do Registro para o tipo de pasta ou objeto de arquivo.

O [**método IShellExtInit::Initialize**](/windows/desktop/api/shobjidl_core/nf-shobjidl_core-ishellextinit-initialize) armazena o nome do arquivo, o ponteiro [**IDataObject**](/windows/win32/api/objidl/nn-objidl-idataobject) e a chave do Registro, conforme necessário para uso posterior. O fragmento de código a seguir ilustra uma implementação **de IShellExtInit::Initialize**. Para simplificar, este exemplo pressupõe que o objeto de dados contém apenas um único arquivo. Em geral, o objeto de dados pode conter vários arquivos, cada um deles precisará ser extraído.
...
Há duas maneiras de personalizar infotips. Uma maneira é implementar um objeto que dá suporte a [**IQueryInfo**](/windows/win32/api/shlobj_core/nn-shlobj_core-iqueryinfo) e, em seguida, registrar o objeto na sub-chave adequada no Registro (veja abaixo). Como alternativa, você pode especificar uma cadeia de caracteres fixa ou uma lista de determinadas propriedades de arquivo a serem exibidas.

Para exibir uma cadeia de caracteres fixa para uma extensão de namespace, crie uma sub-chave chamada **InfoTip** abaixo da chave CLSID da extensão de namespace. De definir os dados dessa sub-chave como a cadeia de caracteres que você deseja exibir.
...
Para exibir uma cadeia de caracteres fixa para um tipo de arquivo, crie uma sub-chave chamada **InfoTip** abaixo da **chave ProgID** do tipo de arquivo para o qual você deseja fornecer infotips. De definir os dados dessa sub-chave como a cadeia de caracteres que você deseja exibir.
...
Se você quiser que o Shell mostre determinadas propriedades de arquivo na infotip para um tipo de arquivo específico, crie uma sub-chave chamada **InfoTip** abaixo da **chave ProgID** desse tipo de arquivo. Defina os dados dessa sub-chave como uma lista de nomes de propriedade canônica de ponto e vírgula ou {fmtid}, pares pid em que *propname* é um nome de propriedade canônica e *{fmtid},pid* é um par [**FMTID/PID**](./objects.md).
...
|------------------|-------------------------------|-------------------------------------------------------------------------------------------|
| Autor           | Autor do documento        | [**AUTOR DO \_ PIDSI**](../stg/the-summary-information-property-set.md)                             |
| Título            | Título do documento         | [**TÍTULO DO \_ PIDSI**](../stg/the-summary-information-property-set.md)                              |
| Assunto          | Resumo do assunto               | [**ASSUNTO DO \_ PIDSI**](../stg/the-summary-information-property-set.md)                            |
| Comentário          | Comentários do documento             | [**PIDSI \_ Propriedades comment**](../stg/the-summary-information-property-set.md) ou folder/drive |
...
| OriginalLocation | Local do arquivo original     | Pasta folder e Lixeira pasta                                                   |
| DateDeleted      | O arquivo de data foi excluído         | Lixeira pasta                                                                        |
...
| Acessado         | Data do último acesso            | Exibição de detalhes da pasta padrão                                                              |
| InFolder         | Diretório que contém o arquivo | Resultados da pesquisa de documentos                                                                   |
| Rank             | Qualidade da combinação de pesquisa       | Resultados da pesquisa de documentos                                                                   |
...
| Empresa          | Nome da empresa                  | [**PIDDSI \_ COMPANY**](../stg/the-documentsummaryinformation-and-userdefined-property-sets.md)   |
| Categoria         | Categoria de documento             | [**CATEGORIA PIDDSI \_**](../stg/the-documentsummaryinformation-and-userdefined-property-sets.md)  |
| Direitos autorais        | Direitos autorais de mídia               | [**DIREITOS AUTORAIS DO PIDMSI \_**](../stg/the-documentsummaryinformation-and-userdefined-property-sets.md) |
| HTMLInfoTipFile  | Arquivo InfoTip HTML             | Desktop.ini arquivo para pasta                                                               |
