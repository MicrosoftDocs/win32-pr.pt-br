...
para criar um plug-in Windows Media Player DSP que processa áudio, você precisará modificar o código de exemplo na função chamada **DoProcessOutput**. **DoProcessOutput** é chamado cada vez que Windows Media Player chama com êxito **IMediaObject::P rocessoutput**. É a função que executa as tarefas de processamento de sinal digital que produzem o resultado audível que o plug-in do DSP pretende produzir.
...
as seções a seguir fornecem detalhes sobre como modificar o código gerado pelo assistente de Plug-in Windows Media Player para criar seu próprio Plug-in de DSP de áudio:
