...
O exemplo de código CrawlScopeCommandLine demonstra como definir opções de linha de comando para operações de indexação do Gerenciador de escopo de rastreamento (CSM).
...
1. Abra o Windows Explorer e navegue até o diretório do projeto **CrawlScopeCommandLine** .
2. Clique duas vezes no ícone do arquivo csmcmd. sln para abrir o projeto no Visual Studio.
...
2. No prompt de comando, digite `csmcmd.exe` ou, no Windows Explorer, clique duas vezes no ícone para csmcmd.exe.
