...
ms.locfileid: "119439956"
---
# <a name="setipusezerobroadcast-method-of-the-win32_networkadapterconfiguration-class"></a>Método SetIPUseZeroBroadcast da classe Win32 \_ NetworkAdapterConfiguration

O método estático da [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **SetIPUseZeroBroadcast** é usado para definir o uso de difusão zero de IP.
...
*IPUseZeroBroadcast* \[ no\]
</dt> <dd>

Se **for true**, a difusão de IP zero será usada. O padrão é **false**.
...
Se o parâmetro *IPUseZeroBroadcast* estiver definido como **TRUE,** o IP usará difusões zero (0.0.0.0) em vez de difusões one (255.255.255.255.255). A maioria dos sistemas usa difusões avulas, mas os sistemas derivados de implementações BSD usam difusões zero. Sistemas que usam difusões diferentes não interoperarão na mesma rede.

## <a name="examples"></a>Exemplos

O exemplo modificar [Zero-Broadcast usar](https://Gallery.TechNet.Microsoft.Com/3d1ec74a-bf96-41cf-bb90-f98cd6494fb3) para todos os adaptadores de rede VBScript configura um computador para usar difusões zero (0.0.0.0) em vez de difusões avulas (255.255.255.255.255).
