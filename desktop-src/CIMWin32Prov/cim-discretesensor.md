...
ms.locfileid: "119578746"
---
# <a name="cim_discretesensor-class"></a>\_Classe CIM DiscreteSensor

A classe **CIM \_ DiscreteSensor** tem um conjunto de valores de cadeia de caracteres legais que ele pode relatar. Os valores são enumerados na propriedade **PossibleValues** do sensor. Um sensor discreto sempre tem uma leitura atual que corresponde a um dos valores enumerados.

Devido à adição das propriedades **CurrentState** e **PossibleStates** ao [**\_ sensor CIM**](cim-sensor.md), a subclasse de sensor discreto não é mais necessária; no entanto, ela é mantida para compatibilidade com versões anteriores. As informações nas propriedades **CurrentReading** e **PossibleValues** normalmente têm os mesmos valores e semânticas das propriedades **CurrentState** e **PossibleStates** , que são herdadas do **\_ sensor CIM**.
...
A classe **CIM \_ DiscreteSensor** tem estes tipos de membros:
...
A classe **CIM \_ DiscreteSensor** tem esses métodos.
...
A classe **CIM \_ DiscreteSensor** tem essas propriedades.
...
As cadeias de caracteres na propriedade **PossibleValues** são consideradas aceitáveis (ou seja, não são erros).
...
Enumera as saídas de cadeia de caracteres do sensor discreto.
...
Essa propriedade não indica que os recursos de gerenciamento de energia estão habilitados no momento ou, se habilitado, quais recursos têm suporte. Para obter mais informações, consulte a matriz **PowerManagementCapabilities** .
...
A **classe \_ DiscreteSensor CIM** é derivada do [**\_ Sensor CIM.**](cim-sensor.md)
