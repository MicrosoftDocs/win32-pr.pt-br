...
---
# <a name="win32_mappedlogicaldisk-class"></a>\_Classe Win32 MappedLogicalDisk

A [classe WMI](/windows/desktop/WmiSdk/retrieving-a-class) **\_ MappedLogicalDisk do Win32** representa os dispositivos de armazenamento em rede que são mapeados como discos lógicos no sistema de computador.
...
A classe **Win32 \_ MappedLogicalDisk** tem estes tipos de membros:
...
A classe **Win32 \_ MappedLogicalDisk** tem esses métodos.



| Método            | Descrição                                                                                                                                                                                |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Redefinir**         | Não implementado. Para implementar esse método, consulte o método [**Reset**](reset-method-in-class-cim-controller.md) no [**\_ LogicalDisk CIM**](cim-logicaldisk.md).<br/>                 |
| **SetPowerState** | Não implementado. Para implementar esse método, consulte o método [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) no [**\_ LogicalDisk CIM**](cim-logicaldisk.md).<br/> |
...
A classe **Win32 \_ MappedLogicalDisk** tem essas propriedades.
...
Estado de acesso do dispositivo.
...
Tamanho, em bytes, dos blocos que formam esta extensão de armazenamento. Se esse conceito não for válido para o tipo de dispositivo, o valor será 1.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("win32api \| File System Functions GetVolumeInformation o Vol. de sistema de arquivos \| [](/windows/desktop/api/fileapi/nf-fileapi-getvolumeinformationa) \| \_ \_ é \_ compactado")
</dt> </dl>
...
Tipos de verificação de erros usados pelo hardware.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) (" \| funções do sistema de arquivos win32api [**GetVolumeInformation**](/windows/desktop/api/fileapi/nf-fileapi-getvolumeinformationa))
</dt> </dl>
...
Espaço disponível no disco lógico.

Essa propriedade é herdada [**do \_ LogicalDisk CIM**](cim-logicaldisk.md).
...
Contém o comprimento máximo de um componente de nome de arquivo suportado pela unidade do Windows.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("win32api \| WNetGetConnection funções de rede do Windows \| ")
</dt> </dl>

Nome do caminho de rede para o dispositivo lógico.
...
Se **for true**, o gerenciamento de cotas não estará habilitado para este volume.
...
Se **for true**, o gerenciamento de cota foi usado, mas foi desabilitado. Incompleto refere-se às informações deixadas no sistema de arquivos após o gerenciamento de cota ter sido desabilitado.
...
Se for **true**, o sistema de arquivos estará configurando para gerenciamento de cotas.
...
ID da sessão do usuário. O usuário pode estar conectado usando um logon local ou uma sessão de terminal.
...
Se **for true**, o sistema de arquivos no qual essa unidade de rede está mapeada dá suporte a cotas de disco.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("win32api \| File System Functions \| [**GetVolumeInformation**](/windows/desktop/api/fileapi/nf-fileapi-getvolumeinformationa) \| FS \_ \_ Compact File Compression")
</dt> </dl>

Se for **true**, a partição de disco lógico dará suporte à compactação baseada em arquivo, como é o caso com NTFS. Essa propriedade é **false** quando a propriedade **compactada** é **true**.
...
Nome do volume do disco lógico. Esse valor de propriedade pode ter um máximo de 32 caracteres.
...
Número de série de volume do disco lógico. Esse valor de propriedade pode ter um máximo de 11 caracteres.
...
As instâncias retornadas para essa classe são as seguintes, suponhamos que o usuário A está enumerando as instâncias:

-   O provedor procura uma sessão de logon do usuário A nesse computador:

    -   Se houver uma (e apenas uma) tal sessão de logon, o provedor retornará as unidades mapeadas dessa sessão.
    -   Se houver mais de uma sessão para o usuário A no computador, nenhuma instância de unidade mapeada será retornada (porque o provedor não tem uma maneira razoável de decidir qual sessão usar).

-   Se não houver sessões do usuário A em execução, e houver um usuário conectado localmente B:

    -   Se houver uma única sessão para o usuário B, o provedor representará um e retornará as unidades mapeadas do usuário B. Esse caso dá suporte ao cenário de assistência técnica que deseja ver as instâncias de um usuário conectado localmente. No entanto, se as instâncias são retornadas depende das configurações de política de segurança local nas ferramentas administrativas do painel de controle. Se a política a seguir estiver definida como "Object Creator", nenhuma instância de unidade mapeada será retornada, mesmo que um seja um membro do grupo Administradores:

        "Objeto do sistema: proprietário padrão para objetos criados por membros do grupo de administradores".

    -   Novamente, se houver mais de uma sessão do usuário B em execução no computador, o provedor não terá como decidir qual delas usar. Nesse caso, nenhuma instância de unidade mapeada é retornada.

Para obter mais informações sobre como usar o **Win32 \_ MappedLogicalDisk**, consulte [como posso determinar quais unidades estão mapeadas para compartilhamentos de rede?](https://blogs.technet.com/b/heyscriptingguy/archive/2005/10/27/how-can-i-determine-which-drives-are-mapped-to-network-shares.aspx)

## <a name="examples"></a>Exemplos

O trecho do PowerShell a seguir mostra unidades mapeadas.
