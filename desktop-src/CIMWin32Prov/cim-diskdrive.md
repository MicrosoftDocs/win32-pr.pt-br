...
ms.locfileid: "119080750"
---
# <a name="cim_diskdrive-class-cimwin32-wmi-providers"></a>Classe CIM_DiskDrive (provedores WMI CIMWin32)

A classe **CIM \_ DiskDrive** representa uma unidade de disco físico, como visto pelo sistema operacional. Os recursos da unidade de disco correspondem às características lógica e de gerenciamento da unidade e, em alguns casos, podem não refletir as características físicas do dispositivo. Uma interface para uma unidade física é um membro dessa classe. No entanto, um objeto baseado em outro dispositivo lógico não é membro dessa classe.
...
A classe **CIM \_ DiskDrive** tem estes tipos de membros:
...
A classe **CIM \_ DiskDrive** tem esses métodos.



| Método                                                                | Descrição                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Definido**](reset-method-in-class-cim-cdromdrive.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
...
A classe **CIM \_ DiskDrive** tem essas propriedades.
...
É conhecido que o dispositivo esteja em um modo de economia de energia, mas seu status exato nesse modo é desconhecido.
...
O dispositivo não está funcionando, mas pode ser levado a energia completa ' rapidamente '.
...
Recursos do dispositivo de acesso à mídia. Essa propriedade é herdada do [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).
...
Matriz de cadeias de caracteres de forma livre que fornecem explicações detalhadas para acessar os recursos do dispositivo indicados na matriz **Recursos.** Essa propriedade é herdada [**de CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).
...
Cadeia de caracteres de forma livre que descreve o tipo de detecção e correção de erros com suporte pelo dispositivo.
...
Tamanho máximo, em quilobytes, de mídia com suporte do dispositivo. Kilobytes são interpretados como o número de bytes multiplicado por 1000 (não o número de bytes multiplicado por 1024).
...
Há suporte para o método [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) .
...
O método [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) pode ser invocado com o parâmetro *PowerState* definido como 5 ("ciclo de energia").
...
O método [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) pode ser invocado com o parâmetro *PowerState* definido como 5 ("ciclo de energia") e o parâmetro de *hora* definido como uma data e hora e um intervalo específicos para o Power-on.
...
A propriedade **CreationClassName** do sistema de scoping.
...
Definindo o scoping da **propriedade Name do** sistema.
...
A **classe CIM \_ DiskDrive** é derivada de [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).

O WMI não implementa essa classe. Consulte [Classes Win32 para](win32-provider.md) classes derivadas de CIM **\_ DiskDrive**.
