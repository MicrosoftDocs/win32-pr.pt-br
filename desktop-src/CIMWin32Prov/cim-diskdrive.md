...
Distingue um dispositivo que pode acessar ambos os lados da mídia de dois lados de um dispositivo que lê apenas um único lado e requer que a mídia seja transferida.
...
Indica que a mídia não precisa ser desejetada explicitamente do dispositivo antes de ser acessada por um elemento de seletor.
...
Matriz de cadeias de caracteres de forma livre que fornecem explicações detalhadas para acessar recursos do dispositivo indicados na matriz **Capabilities** . Essa propriedade é herdada do [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).
...
Se **for true**, o erro relatado na propriedade **LastErrorCode** será limpo.
...
A classe **CIM \_ DiskDrive** é derivada de [**\_ MediaAccessDevice CIM**](cim-mediaaccessdevice.md).

O WMI não implementa essa classe. Consulte [classes Win32](win32-provider.md) para classes derivadas do **CIM \_ DiskDrive**.
