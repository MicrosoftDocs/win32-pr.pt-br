...
ms.locfileid: "119021024"
---
# <a name="cim_networkadapter-class"></a>\_Classe CIM adaptador

A classe **CIM \_ adaptador** é uma classe abstrata que define os conceitos gerais de hardware de rede (por exemplo, endereço permanente ou velocidade de operação). As informações são transmitidas usando a associação [**CIM \_ DeviceSAPImplementation**](cim-devicesapimplementation.md) .

Os adaptadores de rede e os pontos de extremidade relacionados representam o potencial de conectividade entre os pares. O potencial de conectividade é significativamente diferente do mestre ou subordinado e controlador ou de relações controladas pelo [**\_ controlador CIM**](cim-controller.md). Ocasionalmente, no entanto, um único dispositivo atua como um adaptador de rede e um controlador, por exemplo, quando um adaptador de Fiber Channel opera como um controlador SCSI do sistema de computador. Nesse caso, os aspectos do dispositivo são orientados por rede e outros são orientados a controlador. As classes de controlador e adaptador devem ser instanciadas e uma relação de identidade de dispositivo também deve ser criada para vincular os diferentes aspectos do dispositivo.
...
A classe **CIM \_ adaptador** tem estes tipos de membros:
...
A classe **CIM \_ adaptador** tem esses métodos.



| Método                                                                    | Descrição                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Definido**](reset-method-in-class-cim-networkadapter.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A classe **CIM \_ adaptador** tem essas propriedades.
...
Se **for true**, o adaptador de rede poderá determinar automaticamente a velocidade ou outras características de comunicação da mídia de rede anexada.
...
Velocidade máxima, em bits por segundo (BPS), para o adaptador de rede.
...
Lista de endereços de rede para um adaptador.
...
Endereço de rede embutido em código em um adaptador. O endereço embutido em código pode ser alterado com uma atualização de firmware ou configuração de software. Se for alterado, o campo deverá ser atualizado quando a alteração for feita. Essa propriedade deve ser deixada em branco se não existir nenhum endereço embutido no código para o adaptador de rede.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB. ifSpeed "," MIF. \|Adaptador de rede DMTF 802 porta \| 1,5 "), [**unidades**](/windows/desktop/WmiSdk/standard-qualifiers) (" bits por segundo ")
</dt> </dl>

Estimativa da largura de banda atual, em BPS. Para pontos de extremidade que variam de largura de banda ou para aqueles em que não é possível fazer nenhuma estimativa precisa, essa propriedade deve conter a largura de banda nominal.
...
A classe **CIM \_ adaptador** é derivada do [**\_ LogicalDevice CIM**](cim-logicaldevice.md).

O WMI não implementa essa classe. Para classes derivadas do **CIM \_ adaptador**, consulte [classes Win32](win32-provider.md).
