---
description: A classe CIM \_ OSVersionCheck especifica as versões do sistema operacional que podem dar suporte a um elemento de software.
...
ms.locfileid: "118422133"
---
# <a name="cim_osversioncheck-class"></a>Classe CIM \_ OSVersionCheck

A **classe CIM \_ OSVersionCheck** especifica as versões do sistema operacional que podem dar suporte a um elemento de software. A verificação pode ser executado para um determinado, mínimo, máximo ou um intervalo de versões do sistema operacional. Para especificar uma versão específica do sistema operacional, as versões mínima e máxima devem ser iguais. Para especificar a versão mínima, a versão mínima só deve ser especificada. Para especificar uma versão máxima, a versão máxima só precisa ser especificada. Para especificar um intervalo, as versões mínima e máxima devem ser especificadas.

O tipo de sistema operacional é especificado na propriedade **TargetOperatingSystem** do elemento de software proprietário. Os detalhes das verificações são comparados com os detalhes correspondentes encontrados em um objeto [**CIM \_ OperatingSystem**](cim-operatingsystem.md) referenciado por uma associação [**CIM \_ InstalledOS**](cim-installedos.md) para o objeto [**\_ ComputerSystem CIM**](cim-computersystem.md) que descreve o ambiente. Pelo menos uma **classe CIM \_ OperatingSystem** deve atender aos detalhes da condição para que a verificação seja atendida. Em outras palavras, nem todos os sistemas operacionais no sistema de computador relevante precisam atender à condição. Além disso, **a propriedade OSType** da **classe CIM \_ OperatingSystem** deve corresponder ao tipo da **propriedade TargetOperatingSystem.**
...
A **classe CIM \_ OSVersionCheck** tem estes tipos de membros:
...
A **classe CIM \_ OSVersionCheck** tem esses métodos.



| Método                                                      | Descrição                                                   |
|:------------------------------------------------------------|:--------------------------------------------------------------|
| [**Invoke**](invoke-method-in-class-cim-osversioncheck.md) | Toma uma ação específica. Não implementado pelo WMI.<br/> |
...
A **classe CIM \_ OSVersionCheck** tem essas propriedades.
...
Se **TRUE**, espera-se que a condição exista no ambiente. Por exemplo, um arquivo deve estar em um sistema, portanto, o [**método Invoke**](invoke-method-in-class-cim-check.md) deve retornar **TRUE.**

Se **FALSE**, a condição não deverá existir. Por exemplo, um arquivo não está em um sistema, portanto, o [**método Invoke**](invoke-method-in-class-cim-check.md) deve retornar **FALSE.**
...
Qualificadores: [**maxlen**](/windows/desktop/WmiSdk/standard-qualifiers) (64), [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**CIM \_ OperatingSystem**](cim-operatingsystem.md).**Versão**")
</dt> </dl>

Versão máxima do sistema operacional necessário.

O valor é codificado em uma das seguintes formas:
...
Versão mínima do sistema operacional necessário.
