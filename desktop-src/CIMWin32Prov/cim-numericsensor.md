---
description: A classe CIM NumericSensor representa um sensor numérico que retorna leituras numéricas e, opcionalmente, dá suporte \_ a configurações de limites.
...
ms.locfileid: "119820926"
---
# <a name="cim_numericsensor-class"></a>Classe \_ NUMERICSensor CIM

A **classe CIM \_ NumericSensor** representa um sensor numérico que retorna leituras numéricas e, opcionalmente, dá suporte a configurações de limites.
...
A **classe CIM \_ NumericSensor** tem estes tipos de membros:
...
A **classe \_ NUMERICSensor CIM** tem esses métodos.



| Método                                                                   | Descrição                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-numericsensor.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-numericsensor.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A **classe \_ NUMERICSensor CIM** tem essas propriedades.
...
Precisão do sensor para a propriedade medida. Seu valor é registrado como mais ou menos centésimos de um percentual. Essa propriedade, e as **propriedades Resolução** e **Tolerância,** são usadas para calcular o valor real da propriedade física medida. A precisão pode variar dependendo se o dispositivo é linear em relação ao seu intervalo dinâmico.
...
Classe ou subclasse usada na criação de uma instância. Quando usado com outras propriedades de chave da classe, essa propriedade permite que todas as instâncias da classe e suas subclasses sejam identificadas exclusivamente.
...
Essa propriedade é herdada **de \_ NumericSensor de CIM**.
...
Capacidade do sensor de resolver diferenças na propriedade medida. Esse valor pode variar dependendo se o dispositivo é linear em relação ao seu intervalo dinâmico.
...
Essa propriedade é herdada **de CIM \_ NumericSensor.**
...
A **classe \_ NUMERICSensor CIM** é derivada do [**\_ Sensor CIM.**](cim-sensor.md)

O WMI não implementa essa classe. Para classes derivadas de **CIM \_ NumericSensor**, consulte [Classes Win32](win32-provider.md).
