...
---
# <a name="cim_volatilestorage-class"></a>\_Classe CIM VolatileStorage

A classe **CIM \_ VolatileStorage** representa os recursos e o gerenciamento do armazenamento volátil.
...
A classe **CIM \_ VolatileStorage** tem estes tipos de membros:
...
A classe **CIM \_ VolatileStorage** tem esses métodos.



| Método                                                                     | Descrição                                                                                                                              |
|:---------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-volatilestorage.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volatilestorage.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A classe **CIM \_ VolatileStorage** tem essas propriedades.
...
Informações adicionais sobre o erro. Um exemplo é a verificação de erros e a síndrome de correção (ECC) ou o retorno dos bits de verificação se uma metodologia de erro baseada em CRC for usada. No último caso, se um erro de bit único for reconhecido e o algoritmo CRC for conhecido, o bit exato que falhou poderá ser determinado. Esse tipo de dados (síndrome de ECC, dados de verificação de bits ou de bits de paridade ou outras informações fornecidas pelo fornecedor) está incluído neste campo. Se a propriedade **errorInfo** for igual a 3 ("OK"), essa propriedade não terá significado.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Informações de memória do recurso do sistema DMTF \| 1,5 ")
</dt> </dl>

Se **for true**, essa memória poderá ser armazenada em cache.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Informações de memória do recurso do sistema DMTF \| 1,6 ")
</dt> </dl>

Tipo de cache compatível com a memória. Se a propriedade **armazenável em cache** estiver definida como **false**, essa propriedade não terá significado e deverá ser definida como 4 ("não aplicável").
...
Dados capturados durante o último acesso de memória incorreto. Os dados ocupam os primeiros *n* octetos da matriz necessários para manter o número de bits especificado pela propriedade **ErrorTransferSize** . Se **ErrorTransferSize** for 0, essa propriedade não terá significado.
...
Ordenação de dados armazenados na matriz **ErrorData** . Se **ErrorTransferSize** for 0, essa propriedade não terá significado.
...
Tipo de erro que ocorreu mais recentemente. Os valores 12-14 são indefinidos no esquema CIM, uma vez que a semântica do tipo de erro e se ele foi ou não correto, são misturados no DMI. A propriedade **CorrectableError** indica se ela foi corrigida.
...
Cadeia de caracteres de forma livre que descreve o tipo de detecção de erros e correção com suporte na extensão de armazenamento.
...
Tamanho da transferência de dados em bits que causou o último erro. 0 indica que não há erro. Se a propriedade **errorInfo** for igual a 3, "OK", essa propriedade deverá ser definida como 0.
...
Cadeia de caracteres de forma livre que fornece informações adicionais se a propriedade **ErrorType** for definida como 1 ("other"). Se um valor diferente de 1 (um), essa cadeia de caracteres não tem significado.
...
Endereço inicial, referenciado por um aplicativo ou sistema operacional e mapeado por um controlador de memória, para o objeto de memória. O endereço inicial é especificado em kilobytes.
...
A classe **CIM \_ VolatileStorage** é derivada da [**\_ memória CIM**](cim-memory.md).
