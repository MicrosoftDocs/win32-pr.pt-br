...
| Método                                                               | Descrição                                                                                                                              |
|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-potsmodem.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-potsmodem.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
Países ou regiões em que o modem pode operar.
...
País ou região para o qual o modem está programado no momento. Quando há suporte para vários países ou regiões, essa propriedade define qual deles está selecionado atualmente para uso.
...
Senhas definidas no momento para o modem. Essa matriz pode ser deixada em branco por motivos de segurança.
...
Número máximo de senhas definíveis no próprio modem. Se não houver suporte para esse recurso, insira 0 (zero).
...
Nível de volume dos tons audíveis do modem. Por exemplo, alto, médio ou baixo volume pode ser relatado.
...
A classe **CIM \_ POTSModem** é derivada do [**\_ LogicalDevice CIM**](cim-logicaldevice.md).

O WMI não implementa essa classe. Para classes WMI derivadas do **CIM \_ POTSModem**, consulte [classes Win32](win32-provider.md).
