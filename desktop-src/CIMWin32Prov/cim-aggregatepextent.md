...
---
# <a name="cim_aggregatepextent-class"></a>\_Classe CIM AggregatePExtent

A classe **CIM \_ AggregatePExtent** fornece informações resumidas sobre blocos lógicos endereçáveis, que estão no mesmo grupo de redundância de armazenamento e localizados na mesma mídia física.

A classe **CIM \_ AggregatePExtent** é um agrupamento alternativo para extensões físicas quando apenas informações resumidas são necessárias ou quando a configuração automática é usada. A configuração automática pode resultar em milhares de extensões físicas sendo definidas.
...
A classe **CIM \_ AggregatePExtent** tem estes tipos de membros:
...
A classe **CIM \_ AggregatePExtent** tem esses métodos.



| Método                                                                      | Descrição                                                                                                                                |
|:----------------------------------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-aggregatepextent.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                                 |
| [**SetPowerState**](setpowerstate-method-in-class-cim-aggregatepextent.md) | Define o estado de energia desejado para um dispositivo lógico e quando o dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A classe **CIM \_ AggregatePExtent** tem essas propriedades.
...
Qualificadores: [**override**](/windows/desktop/WmiSdk/standard-qualifiers) ("NumberOfBlocks"), [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Extensão física de agregação DMTF \| 1,2 ")
</dt> </dl>

Número total de blocos (incluindo os blocos de dados de verificação) contidos nessa extensão física agregada. O tamanho do bloco (uma propriedade herdada) deve ser definido com o mesmo valor do dispositivo de acesso à mídia associado a essa extensão.
...
A classe **CIM \_ AggregatePExtent** é derivada de [**\_ StorageExtent CIM**](cim-storageextent.md).
