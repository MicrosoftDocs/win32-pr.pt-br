...
---
# <a name="copyex-method-of-the-cim_datafile-class"></a>Método CopyEx da classe de \_ datafilefiles CIM

O método **CopyEx** copia o arquivo lógico (ou diretório) que é especificado no caminho do objeto para o local especificado pelo parâmetro *filename* . Não haverá suporte para uma cópia se ela exigir a substituição de um arquivo lógico existente. Esse método é uma versão estendida do método [**Copy**](copy-method-in-class-cim-datafile.md) e é herdado de [CIM \_ LogicalFile](cim-logicalfile.md).
...
Cadeia de caracteres que representa o nome do arquivo (ou diretório) em que o método falhou. Esse parâmetro será **NULL** se o método tiver sucesso.
...
O método **CopyEx** no [**\_ arquivo CIM**](cim-datafile.md) é implementado pelo WMI.
