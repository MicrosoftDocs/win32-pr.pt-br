...
---
# <a name="cim_protectedspaceextent-class"></a>\_Classe CIM ProtectedSpaceExtent

A classe **CIM \_ ProtectedSpaceExtent** representa endereços de bloco lógico endereçáveis, que são tratados como uma única extensão de armazenamento, mas estão localizados em uma única extensão física. Extensões de espaço protegido excluem os blocos lógicos mapeados como dados de verificação e contêm informações de mapeamento de profundidade de dados do usuário. Uma possibilidade alternativa, se a configuração automática for usada, é instanciar ou estender a classe [**CIM \_ AggregatePSExtent**](cim-aggregatepsextent.md) .
...
A classe **CIM \_ ProtectedSpaceExtent** tem estes tipos de membros:
...
A classe **CIM \_ ProtectedSpaceExtent** tem esses métodos.



| Método                                                                          | Descrição                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-protectedspaceextent.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A classe **CIM \_ ProtectedSpaceExtent** tem essas propriedades.
...
Tamanho do bloco, em bytes, dos blocos que formam a extensão de armazenamento. Se o tamanho do bloco variável, o tamanho máximo do bloco, em bytes, deverá ser especificado. Se o tamanho do bloco for desconhecido ou se um conceito de bloco não for válido (por exemplo, para extensões de agregação, memória ou discos lógicos), insira um 1 (um).
...
Número total de blocos consecutivos, cada um deles bloqueando o tamanho do valor contido na propriedade **BlockSize** , que formam a extensão de armazenamento. O tamanho total da extensão de armazenamento pode ser calculado multiplicando o valor da propriedade **BlockSize** pelo valor dessa propriedade. Se o valor de **BlockSize** for 1 (um), essa propriedade será o tamanho total da extensão de armazenamento.
...
Qualificadores: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|Extensão de espaço protegido por DMTF \| 1,6 "), [**unidades**](/windows/desktop/WmiSdk/standard-qualifiers) (" bytes ")
</dt> </dl>

Para uma classe **CIM \_ ProtectedSpaceExtent** que é alocada para uma classe de [**\_ volume do CIM**](cim-volumeset.md) , essa propriedade é o número de bytes de dados de usuário colocados na extensão de espaço protegido antes de passar para a próxima extensão de espaço protegido no conjunto de volumes. Caso contrário, essa extensão de espaço protegido é considerada como não alocada e essa propriedade deve ser definida como 0.
...
A classe **CIM \_ ProtectedSpaceExtent** é derivada de [**\_ StorageExtent CIM**](cim-storageextent.md).
