---
description: A classe CIM \_ CDROMDrive representa uma unidade CD-ROM no computador.
ms.assetid: 044c9687-fc25-4a8c-b2ef-bd7ea332af7b
ms.tgt_platform: multiple
title: CIM_CDROMDrive classe (Provedores WMI CIMWin32)
ms.topic: reference
ms.date: 05/31/2018
topic_type:
- APIRef
...
ms.locfileid: "119700966"
---
# <a name="cim_cdromdrive-class-cimwin32-wmi-providers"></a>CIM_CDROMDrive classe (Provedores WMI CIMWin32)

A **classe CIM \_ CDROMDrive** representa uma unidade CD-ROM no computador.

> [!Note]  
> O nome da unidade não corresponde à letra da unidade lógica atribuída ao dispositivo, que é o nome do dispositivo de armazenamento lógico que depende da unidade.
...
A **classe CIM \_ CDROMDrive** tem estes tipos de membros:
...
A **classe CIM \_ CDROMDrive** tem esses métodos.



| Método                                                                | Descrição                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Redefinir**](reset-method-in-class-cim-cdromdrive.md)                 | Solicita uma redefinição do dispositivo lógico. Não implementado pelo WMI.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-cdromdrive.md) | Define o estado de energia desejado para um dispositivo lógico e quando um dispositivo deve ser colocado nesse estado. Não implementado pelo WMI.<br/> |
...
A **classe CIM \_ CDROMDrive** tem essas propriedades.
...
Distingue um dispositivo que pode acessar os dois lados da mídia de lado duplo de um dispositivo que lê apenas um único lado e requer que a mídia seja transformada.
...
Indica que a mídia não precisa ser explicitamente ejetada do dispositivo antes de ser acessada por um elemento selador.
...
Matriz de cadeias de caracteres de forma livre que fornece explicações detalhadas para acessar os recursos do dispositivo indicados na matriz **Recursos.**

> [!Note]  
> Cada entrada dessa matriz está relacionada à entrada na matriz **Recursos,** localizada no mesmo índice.
...
Cadeia de caracteres de forma livre que indica o algoritmo ou a ferramenta usada para compactar o arquivo lógico. Se não for possível descrever o esquema de compactação (porque ele é desconhecido), use o seguinte: Se , use "Desconhecido". Se , use "Compactado". , use "Não Compactado".
...
**Esse dispositivo está funcionando corretamente.**  acima (0)
...
**Esse dispositivo não está configurado corretamente.** (1)
...
**Windows pode carregar o driver para este dispositivo.** (2)
...
**O driver para esse dispositivo pode estar corrompido ou seu sistema pode estar com pouca memória ou outros recursos.** (3)
...
**Esse dispositivo não está funcionando corretamente. Um de seus drivers ou seu registro pode estar corrompido.** (4)
...
**O driver para esse dispositivo precisa de um recurso que Windows pode gerenciar.** (5)
...
**A configuração de inicialização para esse dispositivo entra em conflito com outros dispositivos.**  (6)
...
**Este dispositivo não pode iniciar.** (10)
...
**Esse dispositivo não pode encontrar recursos gratuitos suficientes que ele possa usar.** (12)
...
**Windows pode verificar os recursos desse dispositivo.** (13)
...
**Esse dispositivo não pode funcionar corretamente até que você reinicie o computador.** (14)
...
**Esse dispositivo não está funcionando corretamente porque provavelmente há um problema de nova enumeração.** (15)
...
**Windows pode identificar todos os recursos que esse dispositivo usa.** (16)
...
**Esse dispositivo está solicitando um tipo de recurso desconhecido.** (17)
...
**Falha do sistema: tente alterar o driver para este dispositivo. Se isso não funcionar, consulte sua documentação de hardware. Windows está removendo esse dispositivo.** (21)
...
**Esse dispositivo está desabilitado.** (22)
...
**Falha do sistema: tente alterar o driver para este dispositivo. Se isso não funcionar, consulte sua documentação de hardware.** (23)
...
**Esse dispositivo não está presente, não está funcionando corretamente ou não tem todos os drivers instalados.** (24)
...
**Windows ainda está configurando esse dispositivo.** (25)
...
**Windows ainda está configurando esse dispositivo.** (26)
...
**Esse dispositivo não tem configuração de log válida.** (27)
...
**Esse dispositivo não está funcionando corretamente porque Windows pode carregar os drivers necessários para esse dispositivo.** (31)
...
Cadeia de caracteres de forma livre que descreve os tipos de detecção de erro e correção com suporte do dispositivo.
...
Tamanho máximo do bloco, em bytes, para a mídia acessada pelo dispositivo.
...
Tamanho máximo, em quilobytes, de mídia com suporte neste dispositivo. Kilobytes são interpretados como o número de bytes multiplicado por 1000 (não o número de bytes multiplicado por 1024).
...
Tamanho mínimo do bloco, em bytes, para a mídia acessada pelo dispositivo.
...
Se **for true**, o dispositivo de acesso à mídia precisará de limpeza. A limpeza manual ou automática é possível é indicada na propriedade da matriz **Capabilities** .
...
Há suporte para o método **SetPowerState.** Esse método é encontrado na classe [**\_ LogicalDevice cim**](cim-logicaldevice.md) pai e pode ser implementado. Para obter mais informações, [consulte Designando classes Managed Object Format (MOF).](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes)
...
Essa propriedade não indica se os recursos de gerenciamento de energia estão habilitados no momento ou, se habilitados, quais recursos têm suporte. Para obter mais informações, consulte a **matriz PowerManagementCapabilities.**
...
A **classe CIM \_ CDROMDrive** é derivada de [**CIM \_ MediaAccessDevice**](cim-mediaaccessdevice.md).

O WMI não implementa essa classe. Para obter mais informações sobre classes derivadas de **CIM \_ CDROMDrive**, consulte [Classes Win32](win32-provider.md).
