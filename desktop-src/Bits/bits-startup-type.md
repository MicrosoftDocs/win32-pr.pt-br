...
O **tipo de inicialização** para bits é o início automático atrasado (se houver trabalhos bits ativos) ou início da demanda (se não houver trabalhos ativos). Versões do Windows anteriores à atualização de novembro usaram apenas o tipo de inicialização de início automático atrasado; versões anteriores ao vista usaram o tipo de inicialização manual.
