...
ms.locfileid: "119579376"
---
# <a name="example-common-classes"></a>Exemplo: classes comuns

Você pode usar os exemplos de código neste tópico como um ponto de partida para muitos aplicativos Serviço de Transferência Inteligente em Segundo Plano (BITS) que executam inicialização COM, precisam de tratamento de erros e recebimento de notificações de retorno de chamada.


O exemplo de código a seguir define uma classe de exceção para tratar erros.
...
A classe myexception é uma classe de exceção genérica que aceita um código HRESULT e uma cadeia de caracteres de erro.


O exemplo de código a seguir define uma classe auxiliar de aquisição de recursos para a função [CoInitializeEx](/windows/win32/api/combaseapi/nf-combaseapi-coinitializeex) .
...
A classe CCoInitializer lida com alocação de recursos e desalocação para inicialização COM. Essa classe permite que o destruidor seja chamado quando a classe sai do escopo. Essa classe elimina a necessidade de o método [CoUninitialize](/windows/win32/api/combaseapi/nf-combaseapi-couninitialize) ser chamado após cada bloco de exceção.
...
A classe CNotifyInterface derivada da interface [**IBackgroundCopyCallback**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopycallback) . A classe CNotifyInterface implementa a interface IUnknown. Para obter mais informações, consulte [IUnknown]( /windows/win32/api/unknwn/nn-unknwn-iunknown).

O CNotifyInterface usa os seguintes métodos para receber a notificação de que um trabalho foi concluído, foi modificado ou está em um estado de erro: [**JobTransferred**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopycallback-jobtransferred), [**JobModification**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopycallback-jobmodification)e [**JobError**](/windows/desktop/api/Bits/nf-bits-ibackgroundcopycallback-joberror). Todos esses métodos usam um objeto de trabalho [**método ibackgroundcopyjob**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopyjob) .

Este exemplo usa o [CoTaskMemFree](/windows/win32/api/combaseapi/nf-combaseapi-cotaskmemfree) para liberar recursos de memória.


O exemplo de código a seguir é a implementação da interface de retorno de chamada [**IBackgroundCopyCallback**](/windows/desktop/api/Bits/nn-bits-ibackgroundcopycallback) .
