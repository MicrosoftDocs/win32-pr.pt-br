---
title: Estruturas de entrada do teclado
description: Estruturas de entrada do teclado
ms.assetid: 77cfccd8-7e47-4cdf-99a0-c1992a257fef
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 43c629be089241ec270706db71330c44842d2d33fb54f0193465a0a1950291fd
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 08/11/2021
ms.locfileid: "118757668"
---
# <a name="keyboard-input-structures"></a>Estruturas de entrada do teclado

## <a name="in-this-section"></a>Nesta seção

- [**HARDWAREINPUT**](/windows/win32/api/winuser/ns-winuser-hardwareinput)
- [**ENTRADA**](/windows/win32/api/winuser/ns-winuser-input)
- [**KEYBDINPUT**](/windows/win32/api/winuser/ns-winuser-keybdinput)
- [**LASTINPUTINFO**](/windows/win32/api/winuser/ns-winuser-lastinputinfo)
- [**MOUSEINPUT**](/windows/win32/api/winuser/ns-winuser-mouseinput)
