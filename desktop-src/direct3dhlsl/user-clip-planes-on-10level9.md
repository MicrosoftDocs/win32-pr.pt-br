...
ao escrever um aplicativo de Windows Store, você deve usar os níveis de recurso do 10Level9 ([nível de recurso](/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro) 9 \_ x) para que o aplicativo possa ser executado no nível do recurso 9 \_ x e superior. Como seu aplicativo dá suporte ao nível de recurso 9 \_ x e superior, você também deve usar a capacidade comum de aplicar os planos de corte no espaço do clipe.
...
Um plano de recorte é definido por um vetor com 4 componentes. Os três primeiros componentes definem um vetor x, y, z que emana da origem no espaço que desejamos cortar. Esse vetor implica em um plano, perpendicular ao vetor e executado por meio da origem. Windows mantém todos os pixels no lado do vetor do plano e corta todos os pixels por trás do plano. o componente w em diante empurra o plano de volta e faz com que Windows clipes menos (um w negativo faz Windows para cortar mais) ao longo da linha de vetor. Se os componentes x, y, z comporem um vetor de unidade (normalizado), w enviará por push as unidades do plano w de volta.
