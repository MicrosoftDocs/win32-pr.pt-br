...
Mínimo não assinado por Atomic para memória. Retorna o valor na memória antes da operação de máximo.



| IMM \_ Atomic \_ umin dst0 \[ . \_ \_ máscara de componente único \] , dst1, dstAddress \[ . swizzle \] , \[ \_ componente src0. Select\] |
|--------------------------------------------------------------------------------------------------------------|
...
Esta instrução executa um componente único sem sinal de 32 bits de operando no mínimo *src0* com *dst1* em 32 bits por endereço de componente *dstAddress*.
...
O valor em *dst1* Memory antes de min é retornado para *dst0*.
...
Se a invocação do sombreador estiver inativa, por exemplo, se o pixel que foi descartado anteriormente em sua execução ou se uma invocação de pixel/amostra existir apenas para servir como um auxiliar para um pixel/amostra real para derivações, essa instrução não alterará a memória *dst1* e o valor retornado será indefinido.
